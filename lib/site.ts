...
const categoryToolPresets: Record<string, ToolCardData[]> = {
  ...
  "text": [
    ...
    { title: { ko: "텍스트 공백·줄바꿈 정리기", en: "Text Whitespace & Line Break Cleaner", ja: "テキスト空白・改行整理ツール" }, description: { ko: "공백과 줄바꿈을 정리하는 다음 텍스트 도구입니다.", en: "Clean up whitespace and line breaks.", ja: "空白と改行を整理する次のテキストツールです。" }, status: "NEXT", active: false },
    { title: { ko: "대소문자·문장 형식 변환기", en: "Text Case & Sentence Converter", ja: "大文字・小文字・文形式変換ツール" }, description: { ko: "대소문자와 문장 형식을 변환하는 다음 텍스트 도구입니다.", en: "Convert letter case and sentence formatting.", ja: "大文字・小文字と文形式を変換する次のテキストツールです。" }, status: "NEXT", active: false },
  ],
  "image-edit": [
    {
      title: tool008Titles,
...
export function getCategoryToolCards(categorySlug: string, locale: Locale): ToolCardData[] {
  ...
  if (categorySlug === "text") {
    return cards.map((item, index) => index === 0 ? { ...item, href: `/${locale}/${tool036Slug}` } : item);
  }
