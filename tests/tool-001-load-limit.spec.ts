...
test.describe("001 부하·경계 검수",()=>{
 ...
 test("10개 허용·11개 차단",async({page})=>{await page.goto("/ko/jpg-png-webp-image-converter");const b=await fs.readFile(fixture);for(let i=0;i<11;i+=1)await page.getByTestId("converter-file-input").setInputFiles({name:`a-${i}.jpg`,mimeType:"image/jpeg",buffer:Buffer.concat([b,Buffer.from([i])])});await expect(page.getByTestId("converter-file-card")).toHaveCount(10)});
});
