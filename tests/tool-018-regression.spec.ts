...
test.describe('018 route, locale, SEO and protected-route regression', () => {
  for (const locale of ['ko','en','ja'] as const) {
    test(`${locale} page renders localized H1, metadata, canonical, hreflang and structured data`, async ({ page }) => {
      ...
      await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', `https://www.fixlgs.com/tools/${locale}/image-metadata-checker`);
      await expect(page.locator('link[rel="alternate"][hreflang="ko"]')).toHaveAttribute('href', 'https://www.fixlgs.com/tools/ko/image-metadata-checker');
      await expect(page.locator('link[rel="alternate"][hreflang="en"]')).toHaveAttribute('href', 'https://www.fixlgs.com/tools/en/image-metadata-checker');
      await expect(page.locator('link[rel="alternate"][hreflang="ja"]')).toHaveAttribute('href', 'https://www.fixlgs.com/tools/ja/image-metadata-checker');
      await expect(page.locator('link[rel="alternate"][hreflang="x-default"]')).toHaveAttribute('href', 'https://www.fixlgs.com/tools/en/image-metadata-checker');
