...
const route='break-even-point-calculator';
const koHeadline='손익분기점의 핵심은 공헌이익입니다';
for(const locale of ['ko','en','ja'] as const){
  test(`TOOL069 completed design contract ${locale}`,async({page})=>{
    ...
    await expect(faq.locator('.toolbox-faq-actions button')).toHaveCount(0);
