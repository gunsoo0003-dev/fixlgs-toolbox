...
const pageCopy = {
  ko: {
    ...
    guideTitle: "스토어 제출용 스크린샷을 만들 때 중요한 기준", guide: [
      ["01", "실제 앱 화면 우선", "앱 UI를 왜곡하거나 존재하지 않는 기능처럼 보이게 만들지 않고 실제 사용자 경험이 명확히 보이도록 구성합니다."],
      ["02", "No Stretch", "세로 화면을 가로 규격에 억지로 늘리지 않고 원본 비율을 유지한 contain·cover 방식으로 화면을 배치합니다."],
      ["03", "스토어별 preset", "Apple의 고정 accepted size와 Google Play의 범위형 규칙을 구분해 기본 preset으로 선택할 수 있게 합니다."],
      ["04", "다국어 분리", "이미지와 crop은 공유하고 제목·설명은 한국어·영어·일본어별로 따로 저장해 긴 문구로 인한 잘림을 줄입니다."],
      ["05", "첫 1~3장 우선", "검색 결과와 제품 페이지 첫 화면에서 핵심 기능이 먼저 보이도록 가장 중요한 화면을 앞쪽에 배치합니다."],
      ["06", "부분 실패 보존", "여러 규격·언어를 출력할 때 한 결과가 실패해도 성공한 결과는 유지하고 실패 항목만 다시 확인할 수 있게 합니다."],
    ],
  ...
  en: {
    ...
    guideTitle: "Key standards for store screenshot production", guide: [
      ["01", "Use real app screens", "Show the actual experience clearly without distorting the UI or implying features that do not exist."],
      ["02", "No Stretch", "Preserve the source aspect ratio with contain or cover instead of stretching portrait screens into landscape output."],
      ["03", "Store-specific presets", "Keep Apple's accepted fixed sizes separate from Google Play's range-based rules and expose simple default presets."],
      ["04", "Separate localization", "Share image and crop state while keeping title and description text independent for Korean, English, and Japanese."],
      ["05", "Prioritize screenshots 1–3", "Place the most important product features first so the opening store views communicate the app quickly."],
      ["06", "Keep partial success", "When one language or size fails, preserve successful outputs and identify only the failed result for retry."],
    ],
  ...
  ja: {
    ...
    guideTitle: "ストア提出用スクリーンショットの重要基準", guide: [
      ["01", "実際のアプリ画面", "UIを歪めたり存在しない機能に見せたりせず、実際のユーザー体験が明確に伝わる構成にします。"],
      ["02", "No Stretch", "縦画面を横規格へ無理に引き伸ばさず、元画像比率を保ったcontain・coverで配置します。"],
      ["03", "ストア別preset", "Appleの固定accepted sizeとGoogle Playの範囲ルールを分けて管理します。"],
      ["04", "多言語分離", "画像とcropは共有し、韓国語・英語・日本語のタイトルと説明は別々に保存します。"],
      ["05", "最初の1〜3枚を優先", "最初に主要機能が見えるよう、重要な画面を前方へ配置します。"],
      ["06", "部分失敗を保持", "1件失敗しても成功した結果は保持し、失敗した組み合わせだけ確認できるようにします。"],
    ],
