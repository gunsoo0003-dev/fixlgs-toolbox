...
const copy = {
  ko: {
    ...
    next:"다음 작업", related:"관련 도구", available:"사용 가능", how:"사용 방법", guide:"글자 수와 문서 통계를 이렇게 활용하세요", guideDesc:"글자 수 제한 확인부터 문서 구조 점검, 바이트·읽기시간 확인, 파일 편집과 저장까지 실제 사용 상황에 맞춰 정리했습니다.", caution:"주의사항", faq:"자주 묻는 질문",
  ...
  en: {
    ...
    next:"Next work", related:"Related tools", available:"Available", how:"How to use", guide:"Character & document statistics guide", guideDesc:"From checking character limits to reviewing structure, bytes, reading time, file editing, and saving, these notes explain the tool in practical situations.", caution:"Important notes", faq:"Frequently asked questions",
  ...
  ja: {
    ...
    next:"次の作業", related:"関連ツール", available:"利用可能", how:"使い方", guide:"文字数と文書統計をこう活用します", guideDesc:"文字数制限の確認から文書構造、バイト数、読了時間、ファイル編集・保存まで、実際の利用場面に合わせて整理しました。", caution:"注意事項", faq:"よくある質問",
...
export function CharacterDocumentCounterPage({ locale }: { locale: Locale }) {
  const t = copy[locale];
  const url = `https://www.fixlgs.com/tools/${locale}/character-document-counter`;
  const related = [
