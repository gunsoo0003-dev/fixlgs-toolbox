...
const copy = {
  ko:{back:"PDF 도구",title:"PDF 분할·페이지 추출기",desc:"PDF를 페이지 범위로 나누거나 필요한 페이지만 골라 새 PDF 또는 개별 PDF로 저장하세요.",local:"페이지 복사·분할·ZIP 생성은 브라우저 안에서 처리됩니다.",how:"사용 방법",steps:["PDF 파일 1개를 선택합니다. 총 페이지 수와 페이지 미리보기를 확인합니다.","페이지 범위 분할, 특정 페이지 추출, 페이지별 개별 PDF, 홀수·짝수 분리 중 원하는 방식을 고릅니다.","범위 또는 페이지 번호를 입력하거나 특정 페이지 추출 모드에서 썸네일을 눌러 선택합니다.","실행 전에 예상 결과 파일 수와 포함 페이지를 확인한 뒤 PDF 분할 또는 페이지 추출을 실행합니다.","결과 PDF를 개별로 받거나 여러 결과를 ZIP으로 한 번에 저장합니다."],guideTitle:"PDF를 나눌 때 실전 기준",guide:[
    ["01","범위 분할과 페이지 추출을 구분","1-20, 21-40처럼 여러 결과 파일이 필요하면 범위 분할을 사용하고, 1,3,7-9처럼 필요한 페이지만 한 문서로 모으려면 특정 페이지 추출을 사용합니다."],
    ["02","화면 번호는 1부터 시작","사용자가 보는 페이지 번호는 1부터 시작합니다. 내부 PDF 인덱스 변환은 도구가 처리하므로 문서에 표시된 페이지 순서 기준으로 입력하면 됩니다."],
    ["03","특정 페이지는 중복 제거·오름차순","같은 페이지를 여러 번 입력하면 한 번만 포함하고, 선택 결과는 오름차순으로 정리합니다. 썸네일 선택과 직접 입력은 같은 선택 상태를 사용합니다."],
    ["04","범위 겹침은 결과마다 유지","1-5와 4-8처럼 범위가 겹치면 4~5페이지는 두 결과 PDF에 각각 포함됩니다. 실행 전에 예상 결과에서 중복 포함 사실을 확인하세요."],
    ["05","페이지를 이미지로 다시 만들지 않음","결과 PDF는 원본 페이지 객체를 새 PDF로 복사하는 방식으로 생성합니다. 페이지를 이미지로 재렌더링해 PDF로 만드는 방식이 아니므로 분할 과정 자체로 불필요한 화질 저하를 만들지 않습니다."],
    ["06","페이지별 분리는 ZIP이 기본","수십~수백 개 파일을 브라우저에서 연속 자동 다운로드하면 차단될 수 있어 페이지별 개별 PDF는 ZIP으로 한 번에 받는 흐름이 편리합니다."],
    ["07","민감 문서는 로컬 처리","계약서·증명서처럼 민감한 PDF도 분할과 추출 과정에서 서버 업로드를 사용하지 않습니다. 탭을 닫으면 작업 중 메모리에 있던 결과도 유지되지 않습니다."],
    ["08","암호·손상 PDF는 우회하지 않음","비밀번호가 설정되었거나 손상된 PDF를 억지로 우회하지 않습니다. 입력 단계에서 읽기 실패를 구분해 안내하고 정상 PDF로 다시 작업할 수 있게 합니다."]],
  ...
  en:{back:"PDF Tools",title:"Split & Extract PDF",desc:"Split a PDF by page range or extract only the pages you need into a new PDF or separate page files.",local:"Page copying, splitting, and ZIP creation run inside your browser.",how:"How to use",steps:["Choose one PDF and review its page count and page thumbnails.","Choose Split by Page Range, Extract Selected Pages, One PDF per Page, or Split Odd / Even Pages.","Enter ranges or page numbers, or click thumbnails in selected-page mode.","Review the expected file count and included pages before running the split or extraction.","Download individual result PDFs or save multiple outputs together as a ZIP."],guideTitle:"Practical rules for splitting PDFs",guide:[
    ["01","Choose range split vs extraction","Use range split when you need several sections such as 1-20 and 21-40. Use selected-page extraction when you only need pages such as 1,3,7-9 in a new document."],
    ["02","Page numbers start at 1","The interface uses human-readable page numbers beginning at 1. Internal zero-based PDF indexes are handled by the tool."],
    ["03","Selections are deduplicated and sorted","Repeated selected pages are included once and selected-page output is sorted ascending. Thumbnail clicks and text input share the same selection state."],
    ["04","Overlapping split ranges stay independent","If ranges such as 1-5 and 4-8 overlap, pages 4 and 5 are intentionally included in both output PDFs. Check the expected plan before processing."],
    ["05","Pages are copied, not rasterized","Result PDFs copy original PDF page objects into new documents rather than rebuilding the pages as images, avoiding unnecessary image re-rendering during the split itself."],
    ["06","ZIP is convenient for per-page output","Browsers may block many automatic downloads, so one-PDF-per-page output can be collected into a single ZIP."],
    ["07","Sensitive documents stay local","Contracts and certificates are processed in browser memory without a server-upload workflow. Unsaved results disappear when the page is closed or refreshed."],
    ["08","Encrypted or damaged PDFs are not bypassed","The tool does not bypass passwords or force damaged files through processing. It reports an input error and keeps the workflow recoverable."]],
  ...
  ja:{back:"PDFツール",title:"PDF 分割・ページ抽出ツール",desc:"PDFをページ範囲で分けたり、必要なページだけを選んで新しいPDFまたは個別PDFとして保存できます。",local:"ページのコピー・分割・ZIP生成はブラウザ内で処理されます。",how:"使い方",steps:["PDFファイルを1つ選び、総ページ数とページプレビューを確認します。","ページ範囲で分割、指定ページを抽出、ページごとに個別PDF、奇数・偶数ページを分割から方式を選びます。","範囲やページ番号を入力するか、指定ページ抽出ではサムネイルをクリックして選択します。","実行前に予想ファイル数と含まれるページを確認して分割または抽出を実行します。","結果PDFを個別に保存するか、複数結果をZIPでまとめて保存します。"],guideTitle:"PDFを分割するときの実用基準",guide:[
    ["01","範囲分割とページ抽出を使い分ける","1-20、21-40のように複数の区間が必要なら範囲分割、1,3,7-9のように必要なページだけをまとめるなら指定ページ抽出を使います。"],
    ["02","画面のページ番号は1から","ユーザーが指定するページ番号は1から始まります。内部PDFの0始まりインデックス変換はツール側で処理します。"],
    ["03","重複を除き昇順に整理","同じページを複数回入力しても1回だけ含め、指定ページ抽出は昇順に整理します。サムネイル選択と直接入力は同じ選択状態です。"],
    ["04","重なる範囲は各結果に残る","1-5と4-8のように範囲が重なる場合、4〜5ページは両方の結果PDFに含まれます。実行前の予想結果を確認してください。"],
    ["05","ページを画像として再生成しない","結果PDFは元のPDFページを新しいPDFへコピーして作成し、画像として再レンダリングしてPDF化する方式ではありません。"],
    ["06","ページ別出力はZIPが便利","多数の自動ダウンロードはブラウザに止められる場合があるため、ページごとの個別PDFはZIPでまとめて保存できます。"],
    ["07","機密文書もローカル処理","契約書や証明書もサーバーへアップロードせずブラウザ内で処理します。保存前にタブを閉じると結果は保持されません。"],
    ["08","暗号・破損PDFを無理に処理しない","パスワードを回避したり破損PDFを強制処理せず、入力エラーを明確に表示して正常なPDFで再作業できるようにします。"]],
...
export function SplitExtractPdfPage({locale}:{locale:Locale}){
  ...
  return <ToolboxSubpageShell locale={locale} appName={t.title}>
    ...
    <section className="toolbox-tool-detail-body"><div>
      ...
      <section className="toolbox-tool-format-guide toolbox-tool-expert-post toolbox-tool-expert-post--wide-head toolbox-tool-expert-post--compact-copy"><div className="toolbox-tool-format-guide-head"><p>PRACTICAL GUIDE</p><h2>{t.guideTitle}</h2><span>{t.desc}</span></div><div className="toolbox-tool-format-body"><div className="toolbox-tool-direction-grid toolbox-tool-practical-grid">{t.guide.map(([n,title,desc])=><article key={n}><strong>{n}</strong><h4>{title}</h4><p>{desc}</p></article>)}</div></div></section>
