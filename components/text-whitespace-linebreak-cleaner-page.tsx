...
const copy = {
  ko: {
    ...
    next:"다음 작업", related:"관련 도구", available:"사용 가능", how:"사용 방법", guide:"공백과 줄바꿈은 이렇게 정리하세요", guideDesc:"일반 공백과 탭의 차이, 각 줄의 앞뒤 정리, 빈 줄 처리, LF·CRLF 출력까지 실제 복붙 문서에서 헷갈리기 쉬운 기준을 정리했습니다.", caution:"주의사항", faq:"자주 묻는 질문",
  ...
  en: {
    ...
    next:"Next work", related:"Related tools", available:"Available", how:"How to use", guide:"Whitespace and line-ending cleanup guide", guideDesc:"These notes explain regular spaces, tabs, per-line trimming, blank lines, and exact LF/CRLF output for pasted text.", caution:"Important notes", faq:"Frequently asked questions",
  ...
  ja: {
    ...
    next:"次の作業", related:"関連ツール", available:"利用可能", how:"使い方", guide:"空白と改行をこう整理します", guideDesc:"通常スペースとタブの違い、各行の前後、空行、LF・CRLF出力など、貼り付けテキストで迷いやすい基準を整理しました。", caution:"注意事項", faq:"よくある質問",
...
export function TextWhitespaceLinebreakCleanerPage({ locale }: { locale: Locale }) {
  const t = copy[locale];
  const url = `https://www.fixlgs.com/tools/${locale}/text-whitespace-linebreak-cleaner`;
  const related = [
