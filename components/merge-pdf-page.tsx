...
export function MergePdfPage({ locale }: { locale: Locale }) {
  const t = copy[locale];
  const url = `https://www.fixlgs.com/tools/${locale}/merge-pdf`;
  const related = [
  ...
  const jsonLd = {
    ...
    "@graph": [
      ...
      { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "TOOLBOX", item: `https://www.fixlgs.com/tools/${locale}` }, { "@type": "ListItem", position: 2, name: t.back, item: `https://www.fixlgs.com/tools/${locale}/category/pdf` }, { "@type": "ListItem", position: 3, name: t.title, item: url }] },
  ...
  return (
    <ToolboxSubpageShell locale={locale} appName={t.title}>
      ...
      <section className="toolbox-tool-detail-body"><div>
        ...
        <ToolNavigation locale={locale} currentTool={28} />
