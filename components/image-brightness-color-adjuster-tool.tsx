...
export function ImageBrightnessColorAdjusterTool({locale}:{locale:Locale}){
 ...
 async function load(f:File){
  ...
  try{const loaded=await loadBrowserImage(f,"from-image");if(token!==loadToken.current){loaded.close();return}if(loaded.width*loaded.height>MAX_PIXELS||loaded.width>MAX_SIDE||loaded.height>MAX_SIDE){setError(t.tooLarge);setErrorKind("size");setStatus("");loaded.close();return}const scale=Math.min(1,1400/loaded.width,900/loaded.height),oc=document.createElement("canvas");oc.width=Math.max(1,Math.round(loaded.width*scale));oc.height=Math.max(1,Math.round(loaded.height*scale));const ctx=oc.getContext("2d",{willReadFrequently:true});if(!ctx){loaded.close();throw new Error("canvas")}ctx.drawImage(loaded.source,0,0,oc.width,oc.height);const img=ctx.getImageData(0,0,oc.width,oc.height);let hasAlpha=false;for(let i=3;i<img.data.length;i+=4){if(img.data[i]<255){hasAlpha=true;break}}source.current=img;setTransparent(hasAlpha);setDimensions({width:loaded.width,height:loaded.height});setFile(f);setFilename(safeName(f.name.replace(/\.[^.]+$/,"")+"-adjusted"));valuesRef.current=defaults;setValues(defaults);applyHistory([defaults],0);setView("adjusted");setZoom(1);setStatus("");loaded.close()}catch(e){setStatus("");setError(e instanceof RangeError?t.memory:t.error);setErrorKind(e instanceof RangeError?"memory":"decode")}
 }
 ...
 async function download(){if(!file||busy)return;const token=++operationToken.current;setBusy(true);setError("");setErrorKind("");setStatus(t.working);let loaded:Awaited<ReturnType<typeof loadBrowserImage>>|null=null;try{loaded=await loadBrowserImage(file,"from-image");if(token!==operationToken.current)return;const c=document.createElement("canvas");c.width=loaded.width;c.height=loaded.height;const ctx=c.getContext("2d",{willReadFrequently:true});if(!ctx)throw new Error("canvas");if(resolvedExt==="jpg"){ctx.fillStyle=bgColor;ctx.fillRect(0,0,c.width,c.height)}ctx.drawImage(loaded.source,0,0);const img=renderPixels(ctx.getImageData(0,0,c.width,c.height),values);ctx.putImageData(img,0,0);const mime=resolvedExt==="png"?"image/png":resolvedExt==="webp"?"image/webp":"image/jpeg",blob=await new Promise<Blob|null>(res=>c.toBlob(res,mime,quality/100));if(token!==operationToken.current)return;if(!blob)throw new Error("blob");const href=URL.createObjectURL(blob);try{const a=document.createElement("a");a.href=href;a.download=`${safeName(filename)}.${resolvedExt}`;document.body.appendChild(a);a.click();a.remove();setResult({size:blob.size,format:resolvedExt.toUpperCase(),width:c.width,height:c.height});setStatus(t.ready)}finally{setTimeout(()=>URL.revokeObjectURL(href),0)}}catch(e){if(token===operationToken.current){setStatus("");setError(e instanceof RangeError?t.memory:t.error);setErrorKind(e instanceof RangeError?"memory":"decode")}}finally{loaded?.close();if(token===operationToken.current)setBusy(false)}}
 if(!file)return (
    ...
    <section className="toolbox-workbench">
      ...
      >
        ...
        <div className="toolbox-upload-focus">
          ...
          <h2>{t.drop}</h2>
          <p>{t.local}</p>
          <button type="button" onClick={()=>openFilePicker(input.current)} data-testid="tool009-select">{t.select}</button>
      ...
      <div className="toolbox-upload-active">
        <div className="toolbox-upload-active-head">
          ...
          <div className="toolbox-upload-active-actions">
            ...
            <button type="button" onClick={()=>openFilePicker(input.current)}>＋ {t.replace}</button>
          </div>
        </div>
