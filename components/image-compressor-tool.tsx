...
// 004 운영 안전선: 10개, 파일당 15MB, 전체 50MB, 이미지당 30MP, 전체 80MP.
const labels={
...
async function loadImage(file:Blob){const url=URL.createObjectURL(await materializeImageBlob(file));try{const img=new Image();img.decoding="async";await new Promise<void>((res,rej)=>{img.onload=()=>res();img.onerror=()=>rej(new Error("decode"));img.src=url});return {img,width:img.naturalWidth,height:img.naturalHeight}}finally{URL.revokeObjectURL(url)}}
...
async function loadOrientedDrawable(file:Blob){
 if(typeof createImageBitmap==="function"){
  try{const bitmap=await createImageBitmap(await materializeImageBlob(file),{imageOrientation:"none"});return {source:bitmap as CanvasImageSource,width:bitmap.width,height:bitmap.height,close:()=>bitmap.close()}}catch{}
 }
