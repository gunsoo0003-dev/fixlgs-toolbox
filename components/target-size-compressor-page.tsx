...
export function TargetSizeCompressorPage({ locale }: { locale: Locale }) {
  ...
  return <ToolboxSubpageShell locale={locale} appName={appName}>
    ...
    <section className="toolbox-tool-detail-body"><div><TargetSizeCompressorTool locale={locale}/><section className="toolbox-next-work"><div><p>NEXT WORK</p><h2>{next}</h2></div><div className="toolbox-next-work-grid">{[
      {n:"004",name:locale==="ko"?"이미지 압축기":locale==="en"?"Image Compressor":"画像圧縮ツール",href:`/${locale}/image-compressor`},
      {n:"006",name:locale==="ko"?"이미지 크기 변경기":locale==="en"?"Image Resizer":"画像サイズ変更ツール"},
      {n:"001",name:locale==="ko"?"JPG·PNG·WebP 이미지 변환기":locale==="en"?"JPG, PNG & WebP Converter":"JPG・PNG・WebP画像変換",href:`/${locale}/jpg-png-webp-image-converter`}
    ].map((c)=>c.href?<Link key={c.n} href={c.href} className="toolbox-next-work-card"><span>{c.n}</span><h3>{c.name}</h3><div className="toolbox-next-work-card-foot"><span>{available}</span><strong>↗</strong></div></Link>:<div key={c.n} className="toolbox-next-work-card is-disabled"><span>{c.n}</span><h3>{c.name}</h3><div className="toolbox-next-work-card-foot"><span>{soon}</span><strong>·</strong></div></div>)}</div></section></div></section>
