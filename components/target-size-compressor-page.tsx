...
export function TargetSizeCompressorPage({ locale }: { locale: Locale }) {
  ...
  const jsonLd = [
    { "@context": "https://schema.org", "@type": "WebApplication", name: appName, applicationCategory: "MultimediaApplication", operatingSystem: "Any", offers: { "@type": "Offer", price: "0", priceCurrency: "USD" }, url: `https://www.fixlgs.com/tools/${locale}/target-size-image-compressor`, description: t.desc },
    { "@context": "https://schema.org", "@type": "FAQPage", mainEntity: t.faqs.map(([q, a]) => ({ "@type": "Question", name: q, acceptedAnswer: { "@type": "Answer", text: a } })) },
    { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "TOOLBOX", item: `https://www.fixlgs.com/tools/${locale}` }, { "@type": "ListItem", position: 2, name: appName, item: `https://www.fixlgs.com/tools/${locale}/target-size-image-compressor` }] },
  ];
