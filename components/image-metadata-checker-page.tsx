...
const copy = {
  ko: {
    ...
    ], more: 'FAQ 더보기', collapse: 'FAQ 접기', how: '사용 방법', faqTitle: '자주 묻는 질문', related: '관련 도구', available: '사용 가능', next: '다음 작업',
  ...
  en: {
    ...
    ], more: 'Show more FAQs', collapse: 'Show fewer FAQs', how: 'How to use', faqTitle: 'Frequently asked questions', related: 'Related tools', available: 'Available', next: 'Next work',
  ...
  ja: {
    ...
    ], more: 'FAQをもっと見る', collapse: 'FAQを閉じる', how: '使い方', faqTitle: 'よくある質問', related: '関連ツール', available: '利用可能', next: '次の作業',
...
export function ImageMetadataCheckerPage({ locale }: { locale: Locale }) {
  const t = copy[locale];
  const url = `https://www.fixlgs.com/tools/${locale}/image-metadata-checker`;
  const related = [
  ...
  const jsonLd = {
    '@context': 'https://schema.org', '@graph': [
      ...
      { '@type': 'BreadcrumbList', itemListElement: [ { '@type': 'ListItem', position: 1, name: 'TOOLBOX', item: `https://www.fixlgs.com/tools/${locale}` }, { '@type': 'ListItem', position: 2, name: t.back, item: `https://www.fixlgs.com/tools/${locale}/category/image-edit` }, { '@type': 'ListItem', position: 3, name: t.title, item: url } ] },
