...
const copy = {
  ko: {
    ...
    next:"다음 작업", related:"관련 도구", available:"사용 가능", how:"사용 방법", guide:"대소문자와 문장 형식은 이렇게 변환하세요", guideDesc:"대문자·소문자, 단순 제목형, 문장형, 첫 글자 대문자의 차이와 고유명사·공백 구조에 대한 기준을 정리했습니다.", caution:"주의사항", faq:"자주 묻는 질문",
  ...
  en: {
    ...
    next:"Next work", related:"Related tools", available:"Available", how:"How to use", guide:"Case and sentence-format conversion guide", guideDesc:"These notes explain uppercase, lowercase, simple title case, sentence case, first-letter capitalization, and how the tool preserves whitespace structure.", caution:"Important notes", faq:"Frequently asked questions",
  ...
  ja: {
    ...
    next:"次の作業", related:"関連ツール", available:"利用可能", how:"使い方", guide:"大文字・小文字と文形式の変換ガイド", guideDesc:"大文字・小文字、単純なタイトル形式、文頭形式、最初の文字だけ大文字にする違いと、空白構造の扱いを整理しました。", caution:"注意事項", faq:"よくある質問",
...
export function CaseSentenceFormatConverterPage({ locale }: { locale: Locale }) {
  const t = copy[locale];
  const url = `https://www.fixlgs.com/tools/${locale}/case-sentence-format-converter`;
  const related = [
  ...
  const jsonLd = { "@context":"https://schema.org", "@graph":[
    ...
    { "@type":"BreadcrumbList", itemListElement:[{"@type":"ListItem",position:1,name:"TOOLBOX",item:`https://www.fixlgs.com/tools/${locale}`},{"@type":"ListItem",position:2,name:t.back,item:`https://www.fixlgs.com/tools/${locale}/category/text`},{"@type":"ListItem",position:3,name:t.title,item:url}] },
  ...
  return <ToolboxSubpageShell locale={locale} appName={t.title}>
    ...
    <section className="toolbox-tool-detail-body"><div>
      ...
      <ToolNavigation locale={locale} currentTool={38} />
