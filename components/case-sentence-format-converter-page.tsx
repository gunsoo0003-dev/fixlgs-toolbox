...
export function CaseSentenceFormatConverterPage({ locale }: { locale: Locale }) {
  ...
  return <ToolboxSubpageShell locale={locale} appName={t.title}>
    ...
    <section className="toolbox-tool-detail-body"><div>
      ...
      <section className="toolbox-next-work"><div className="toolbox-next-work-head"><p>NEXT WORK</p><h2>{t.next}</h2></div><div className="toolbox-next-work-grid"><Link className="toolbox-next-work-card" href={`/${locale}/list-sorter-duplicate-remover`}><span>039</span><h3>{locale === "ko" ? "목록 정렬·중복 제거기" : locale === "ja" ? "リスト並べ替え・重複削除ツール" : "List Sort & Duplicate Remover"}</h3><div className="toolbox-next-work-card-foot"><span>{t.available}</span><strong>↗</strong></div></Link></div></section>
