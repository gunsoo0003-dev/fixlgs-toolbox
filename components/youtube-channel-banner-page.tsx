...
export function YoutubeChannelBannerPage({ locale }: { locale: Locale }) {
  ...
  return <ToolboxSubpageShell locale={locale} appName={t.title}>
    ...
    <section className="toolbox-tool-format-guide tool020-use-cases"><div className="toolbox-tool-format-guide-head"><p>USE CASES</p><h2>{locale === "ko" ? "활용 예시" : locale === "en" ? "Use cases" : "活用例"}</h2></div><div className="toolbox-tool-format-body"><div className="toolbox-tool-format-grid">{t.examples.map(([title, description], i) => <article key={title}><strong>{String(i + 1).padStart(2, "0")}</strong><h3>{title}</h3><p>{description}</p></article>)}</div></div></section>
    <section className="toolbox-tool-format-guide toolbox-tool-expert-post tool020-expert-post">
