...
async function encodeAvif(imageData:ImageData,quality:number){const mod=await remoteImport("https://esm.sh/@jsquash/avif@2.1.1");const buffer=await mod.encode(imageData,{cqLevel:Math.round(63-(quality/100)*55),speed:8});return new Blob([buffer],{type:"image/avif"})}
  ...
  <div className="toolbox-tool-workflow">
   <section className="toolbox-workbench">
    ...
    >
     ...
     <input ref={inputRef} type="file" hidden multiple accept=".heic,.heif,.avif,.jpg,.jpeg,.png,image/heic,image/heif,image/avif,image/jpeg,image/png" onChange={e=>{if(e.target.files)void addFiles(e.target.files);e.currentTarget.value=""}}/>
