...
const outDir = path.join(desktop, `TOOLBOX_001_REAL_ANDROID_DEEP_V9_${stamp}`);
...
async function ensureDeviceReady(page, label='device ready') {
  ...
  if (isKeyguardShowing(snap)) {
    ...
    throw new Error('Device is still on a secure lock screen. Unlock the Galaxy once, then rerun; V9 keeps it awake afterward.');
  }
...
function dumpUi() {
  const remote='/sdcard/tool001-v9-window.xml';
...
async function saveDeviceScreen(name) {
  const remote='/sdcard/tool001-v9-screen.png';
...
async function prepareDeviceFiles(count) {
  ...
  for(let i=1;i<=count;i++){
    const name=`TOOL001_V9_${token}_${String(i).padStart(2,'0')}.png`;
...
function browserInit() {
  const KEY = '__TOOL001_REAL_ANDROID_V9__';
  if (window[KEY]) return;
  const state = window[KEY] = {
    version:'V9', bornEpoch:Date.now(), bornPerf:performance.now(), seq:0,
...
async function waitForAttempt(page, expected, timeout) {
  ...
  while(Date.now()-start<timeout){
    const s=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V9__?({count:window.__TOOL001_REAL_ANDROID_V9__.attempts.length,attempts:window.__TOOL001_REAL_ANDROID_V9__.attempts.map(a=>({attemptIndex:a.attemptIndex,done:a.done,fileCount:a.fileCount})),activeProbes:window.__TOOL001_REAL_ANDROID_V9__.activeProbes}):({count:0,attempts:[],activeProbes:0})).catch(()=>({count:0,attempts:[],activeProbes:0}));
...
async function main(){
  log('=== TOOL001 REAL ANDROID DEEP DIAGNOSTIC V9 ==='); log('[OUTPUT]',outDir);
  step('ADB 연결 확인');
  ...
  await page.evaluate(sel=>{if(window.__TOOL001_REAL_ANDROID_V9__)window.__TOOL001_REAL_ANDROID_V9__.selector=sel;},selector);
  ...
  for(let i=1;i<=attemptsTarget;i++){
    ...
    try{
      ...
      const p=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V9__)));
      ...
      const p=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V9__?JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V9__)):null).catch(()=>null);
  ...
  const state=await withHeartbeat('collect browser diagnostic state',()=>page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V9__))),20000);
  ...
  write('result.json',{meta:{version:'V9',generatedAt:new Date().toISOString(),url,selector,attemptsTarget,attemptsObserved:state.attempts.length,node:process.version,hostPlatform:process.platform},environment:state.environment,attempts:state.attempts,timeline:state.timeline,domFinal});
  write('timeline.json',state.timeline);write('attempts.json',state.attempts);write('dom-final.json',domFinal);
  const s=['TOOL001_REAL_ANDROID_DEEP_DIAGNOSTIC_V9',`URL=${url}`,`ATTEMPTS_TARGET=${attemptsTarget}`,`ATTEMPTS_OBSERVED=${state.attempts.length}`,`USER_AGENT=${state.environment?.userAgent||''}`,`PLATFORM=${state.environment?.platform||''}`,`MAX_TOUCH_POINTS=${state.environment?.maxTouchPoints??''}`];
