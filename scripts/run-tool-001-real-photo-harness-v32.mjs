...
const labels=String(args.cases||'PHOTO_01,PHOTO_02,PHOTO_03,PHOTO_04,PHOTO_05,PHOTO_06,PHOTO_07').split(',').map(x=>x.trim()).filter(Boolean);
...
const outDir=path.join(desktop,`TOOLBOX_001_REAL_PHOTO_V32_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function dumpUi(){const remote='/sdcard/tool001-v32-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
async function autoCommitManualSelection(caseNo,page,startChange){const deadline=Date.now()+8000;let sawSelection=false;let commitTaps=0;let lastXml='';while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__)));if(st.changes.length>startChange)return{committed:true,mode:'picker-auto-close-change',sawSelection,commitTaps};let open=false;try{lastXml=dumpUi();open=pickerDetectedFromXml(lastXml);}catch{}if(!open){await sleep(250);const st2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__)));if(st2.changes.length>startChange)return{committed:true,mode:'picker-closed-change',sawSelection,commitTaps};await sleep(500);continue;}const sig=pickerSelectionSignals(lastXml);if(sig.detected){sawSelection=true;write(`case-${caseNo}-selection-observed.xml`,lastXml);const actions=photoPickerActionControls(lastXml);write(`case-${caseNo}-commit-controls.json`,actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds})));if(actions.length){for(const action of actions){const b=action.bounds;adb('shell','input','tap',String(b.x),String(b.y));commitTaps++;log(`[PHOTO_PICKER] case ${caseNo}: auto commit "${action.exact}" at ${b.x},${b.y}`);await pcScreen(`case-${caseNo}-after-auto-commit-${commitTaps}.png`);if(await pickerClosedStable(page,7000)){const st3=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__)));if(st3.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit',sawSelection:true,commitTaps,action:action.exact};await sleep(1500);const st4=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__)));if(st4.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit-delayed-change',sawSelection:true,commitTaps,action:action.exact};return{committed:false,mode:'picker-closed-no-change',sawSelection:true,commitTaps,action:action.exact};}}}}
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V32__'; if(window[KEY])return;
...
async function ensureChromeInteractive(page,caseNo,tag='guard'){
  ...
  // V32: NEVER infer overflow menu from toolbar "새 탭".
...
async function autoSelectPickerMedia(caseNo,label,round,page,startChange){
  ...
  while(Date.now()<dl){
    const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__))).catch(()=>null);
    ...
    if(!snap.detected){
      await sleep(300);
      const obs2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__))).catch(()=>null);
...
async function waitProduct(page,startCapture,before,timeoutMs=8000){const deadline=Date.now()+timeoutMs;let snap=before,cap=[];while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__)));cap=st.capture.slice(startCapture);snap=await domSnapshot(page);const terminal=cap.some(x=>x.phase==='picker-file-pass'||x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail');const cardChanged=snap.cardCount!==before.cardCount;if(terminal&&(cardChanged||cap.some(x=>x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail')))break;await sleep(350);}return{capture:cap,after:snap};}
async function collectFailureEvidence(caseNo,label,page,reason){let xml='';try{xml=dumpUi();}catch{}write(`case-${caseNo}-${label}-failure-picker.xml`,xml);await pcScreen(`case-${caseNo}-${label}-failure-device.png`);await page.screenshot({path:path.join(outDir,`case-${caseNo}-${label}-failure-page.png`),fullPage:true}).catch(()=>{});const snap=await domSnapshot(page).catch(()=>null);const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__))).catch(()=>null);write(`case-${caseNo}-${label}-failure-evidence.json`,{reason,pickerOpen:pickerDetectedFromXml(xml),selectionSignals:pickerSelectionSignals(xml).selectedNodes.length,commitControls:photoPickerActionControls(xml).map(a=>({exact:a.exact,label:a.label,bounds:a.bounds})),foreground:foregroundSnapshot(),observer:obs,dom:snap});}
...
function checkpointSummary(r){
  ...
  return{
    ...
    FILE_ACQUIRED:has('picker-selection-received'),
    READ_START:has('capture-bytes-start')||!!firstReader,
    SNAPSHOT_SUCCESS:!!owned,
    PRODUCT_STATE_CARD_PREVIEW:statePass&&(r.previewCount||0)>0,
    FIRST_READER:firstReader,
    FIRST_FAILURE:cap.find(x=>/fail$/.test(x.phase||'')||x.phase==='product-add-fail')||null
  };
}
...
async function main(){
  log('=== TOOL001 REAL PHOTO HARNESS V32 FACTS-ONLY ===');log('[OUTPUT]',outDir);
  ...
  step('실기기 사용자흐름 검수 안내');log('');log('*** 검수기는 사용자 흐름만 자동 재현하고 사실만 기록합니다. ***');log('사용자 입력 없음: Photo Picker의 실제 사진 1~7번을 순서대로 자동 클릭합니다.');log('');log('GOOD/BAD를 미리 가정하지 않습니다. 각 사진의 실제 결과만 기록합니다.');log('');
  const results=[];
  for(let i=0;i<labels.length;i++){
    ...
    await ensureExpectedToolPage(page,caseNo,'case-start');await page.bringToFront();const before=await domSnapshot(page);const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__)));let result={case:caseNo,label,pass:false,error:null,before,commit:null};
    try{
      ...
      const changeDeadline=Date.now()+8000;let change=null;while(Date.now()<changeDeadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V32__)));if(st.changes.length>st0.changes.length){change=st.changes.at(-1);break;}await sleep(250);}if(!change)throw new Error('NO_FILE_CHANGE_AFTER_VERIFIED_COMMIT');
      const r=await timed(`CASE ${caseNo} product capture + preview`,()=>waitProduct(page,st0.capture.length,before,8000),9000);
  ...
  step('검수 사실기록 생성');
  const summary=['TOOL001_REAL_PHOTO_HARNESS_V32_FACTS_ONLY',`URL=${url}`,`CASES=${results.length}`];
  ...
  write('result.json',{version:'V32',mode:'HARNESS_FACTS_ONLY',url,generatedAt:new Date().toISOString(),results});
  step('최종 사실표');
