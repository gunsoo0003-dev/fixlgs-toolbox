...
const outDir = path.join(desktop, `TOOLBOX_001_REAL_ANDROID_DEEP_V6_${stamp}`);
...
async function prepareDeviceFiles(count) {
  ...
  for(let i=1;i<=count;i++){
    const name=`TOOL001_V6_${token}_${String(i).padStart(2,'0')}.png`;
...
function browserInit() {
  const KEY = '__TOOL001_REAL_ANDROID_V6__';
  if (window[KEY]) return;
  const state = window[KEY] = {
    version:'V6', bornEpoch:Date.now(), bornPerf:performance.now(), seq:0,
...
async function waitForAttempt(page, expected, timeout) {
  ...
  while(Date.now()-start<timeout){
    const s=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V6__?({count:window.__TOOL001_REAL_ANDROID_V6__.attempts.length,attempts:window.__TOOL001_REAL_ANDROID_V6__.attempts.map(a=>({attemptIndex:a.attemptIndex,done:a.done,fileCount:a.fileCount})),activeProbes:window.__TOOL001_REAL_ANDROID_V6__.activeProbes}):({count:0,attempts:[],activeProbes:0})).catch(()=>({count:0,attempts:[],activeProbes:0}));
...
async function main(){
  log('=== TOOL001 REAL ANDROID DEEP DIAGNOSTIC V6 ==='); log('[OUTPUT]',outDir);
  step('ADB 연결 확인');
  ...
  await page.evaluate(sel=>{if(window.__TOOL001_REAL_ANDROID_V6__)window.__TOOL001_REAL_ANDROID_V6__.selector=sel;},selector);
  ...
  for(let i=1;i<=attemptsTarget;i++){
    ...
    try{
      ...
      const p=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V6__)));
      ...
      const p=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V6__?JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V6__)):null).catch(()=>null);
  ...
  const state=await withHeartbeat('collect browser diagnostic state',()=>page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V6__))),20000);
  ...
  write('result.json',{meta:{version:'V6',generatedAt:new Date().toISOString(),url,selector,attemptsTarget,attemptsObserved:state.attempts.length,node:process.version,hostPlatform:process.platform},environment:state.environment,attempts:state.attempts,timeline:state.timeline,domFinal});
