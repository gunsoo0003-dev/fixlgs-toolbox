...
const runner=path.join(here,'run-tool-001-v48-chrome-webview-ab.mjs');
...
check('webview-real-web-button',/OPEN_WEB_FILE/.test(s) && /onclick="document\.getElementById\('file'\)\.click\(\)"/.test(s));
check('webview-button-exact-tap',/tapExactUiText\('OPEN_WEB_FILE'\)/.test(s));
...
check('desktop-result-zip',/TOOLBOX_001_V48_CHROME_WEBVIEW_AB_REAL_DEVICE_/.test(s) && /Compress-Archive/.test(s));
