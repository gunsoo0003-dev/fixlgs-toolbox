...
const finalDir = resolve(tmpdir(), `010_final_검수결과_${stamp}_${process.pid}`);
const desktopZip = resolve(desktop, `010_final_검수결과_${stamp}.zip`);
...
const steps = [
  { name: "validator", args: ["run", "check:tool010-validator"], type: "source" },
  { name: "source", args: ["run", "check:tool010-source"], type: "source" },
  { name: "ja-terms", args: ["run", "check:ja-terms"], type: "source" },
  { name: "build", args: ["run", "build"], type: "build" },
  { name: "common", args: ["run", "test:toolbox:common-additive"], type: "playwright" },
  { name: "core", args: ["run", "test:toolbox:010-core"], type: "playwright" },
  { name: "additional", args: ["run", "test:toolbox:010-additional"], type: "playwright" },
  { name: "regression", args: ["run", "test:toolbox:010-regression"], type: "playwright" },
  {
    name: "boundary",
    args: ["exec", "playwright", "test", "tests/tool-010-limit.spec.ts", "--workers=1", "--project=desktop-chromium"],
    type: "playwright",
  },
  {
    name: "auto-limit",
    args: ["run", "test:toolbox:010-auto-limit"],
    type: "limit",
  },
];
...
function buildMaster() {
  ...
  const master = {
    ...
    overallStatus,
    rule: "기존 001~008 검수기는 보존하고, 추가 공통검수 + 010 고유검수 + 내부 단계별 한계검수를 연결",
  ...
  mkdirSync(finalDir, { recursive: true });
  safeWrite(resolve(summaryDir, "010-final-summary.json"), JSON.stringify(master, null, 2));
  safeWrite(resolve(summaryDir, "010-final-summary.txt"), `${text.join("\n")}\n`);
  safeWrite(resolve(finalDir, "README_먼저확인.txt"), [
    "1. 01_최종요약/010-final-summary.txt를 먼저 확인하세요.",
  ...
  mkdirSync(playwrightResults, { recursive: true });
  safeWrite(resolve(playwrightResults, "tool-010-validation-master.json"), JSON.stringify(master, null, 2));
  safeWrite(resolve(playwrightResults, "tool-010-validation-master.txt"), `${text.join("\n")}\n`);
...
function createDesktopZip() {
  ...
  return { finalDir: null, desktopZip, zipCreated: true };
}
...
try {
  ...
  for (const name of [
    "tool-010-auto-limit-report.txt",
    "tool-010-auto-limit-report.json",
...
console.log(`\n010 FINAL RESULT: ${overallStatus.toUpperCase()}`);
console.log(`바탕화면 결과 ZIP: ${desktopZip}`);
