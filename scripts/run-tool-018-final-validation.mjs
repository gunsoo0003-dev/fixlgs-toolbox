...
const definitions = [
  ['validator', ['run','check:tool018-validator']],
  ['source', ['run','check:tool018-source']],
  ['design', ['run','check:tool018-design']],
  ['harness', ['run','check:tool018-harness']],
  ['ja-terms', ['run','check:ja-terms']],
  ['build', ['run','build']],
  ['common', ['exec','--','playwright','test','tests/common-tool-additive.spec.ts','--workers=1','--project=desktop-018','--config=playwright.tool018-runtime.config.ts']],
  ['core', ['run','test:toolbox:018-core-only']],
  ['boundary', ['run','test:toolbox:018-boundary-only']],
  ['regression', ['run','test:toolbox:018-regression-only']],
  ['service-limit', ['run','test:toolbox:018-limit-only']],
];
...
const startedAt=new Date(),steps=[];
let prerequisiteFailed=false;

for(const [name,args] of definitions){
  if(prerequisiteFailed){
    steps.push({name,status:'skipped',exitCode:null,durationSeconds:0,counts:{passed:0,failed:0,skipped:1},command:'skipped',stdout:'',stderr:'선행 단계 실패로 후속 검수 미실행'});
    continue;
  }
  ...
  if(code===0&&counts.passed===0)counts.passed=1;
  if(code!==0&&counts.failed===0)counts.failed=1;
...
const skipped=steps.some(x=>x.status==='skipped');
const status=failed||skipped?'failed':'passed';
const packaged=createValidationResultPackage({toolNumber:'018',validationType:'final',status,startedAt,endedAt:new Date(),steps});
let finalZipPath=packaged.zipPath;
if(packaged.zipCreated){
  finalZipPath=resolve(dirname(packaged.zipPath),'018_final_검수결과.zip');
  rmSync(finalZipPath,{force:true});
  renameSync(packaged.zipPath,finalZipPath);
}
cleanupProjectValidationArtifacts();
console.log(`018 FINAL RESULT: ${status.toUpperCase()}`);
console.log(`FAIL ${steps.filter(x=>x.status==='failed').length} / SKIP ${steps.filter(x=>x.status==='skipped').length}`);
