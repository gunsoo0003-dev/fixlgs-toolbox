...
const CATEGORIES=[
  {id:'C01_PROVIDER_URI',photo:'PHOTO_01',name:'Provider/URI 수명·권한'},
  {id:'C02_INPUT_DOM',photo:'PHOTO_02',name:'Input/DOM 수명'},
  {id:'C03_CONCURRENT_READ',photo:'PHOTO_03',name:'중복·동시 read'},
  {id:'C04_PICKER_RETURN_TIMING',photo:'PHOTO_04',name:'Photo Picker 복귀/read 타이밍'},
  {id:'C05_REACT_LIFECYCLE',photo:'PHOTO_05',name:'React lifecycle/DOM 교체'},
  {id:'C06_ABORT_RESIDUE',photo:'PHOTO_06',name:'이전 작업/abort 잔존'},
  {id:'C07_CHROME_ANDROID_FILE',photo:'PHOTO_07',name:'Chrome/Android File 처리'},
  {id:'C08_PRE_READ_DECODE',photo:'PHOTO_08',name:'선행 validation/decode 접근'}
];
const SETS=Math.max(1,Number(args.sets||2));
const REPEATS=Math.max(1,Number(args.repeats||3));
const plan=[];
for(let set=1;set<=SETS;set++)for(const cat of CATEGORIES)for(let repeat=1;repeat<=REPEATS;repeat++)plan.push({...cat,set,repeat,label:`${cat.photo}_${cat.id}_S${set}_R${repeat}`});
...
const outDir=path.join(desktop,`TOOLBOX_001_NOTREADABLE_PYRAMID_V35_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function dumpUi(){const remote='/sdcard/tool001-v35-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
async function autoCommitManualSelection(caseNo,page,startChange){const deadline=Date.now()+8000;let sawSelection=false;let commitTaps=0;let lastXml='';while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__)));if(st.changes.length>startChange)return{committed:true,mode:'picker-auto-close-change',sawSelection,commitTaps};let open=false;try{lastXml=dumpUi();open=pickerDetectedFromXml(lastXml);}catch{}if(!open){await sleep(250);const st2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__)));if(st2.changes.length>startChange)return{committed:true,mode:'picker-closed-change',sawSelection,commitTaps};await sleep(500);continue;}const sig=pickerSelectionSignals(lastXml);if(sig.detected){sawSelection=true;write(`case-${caseNo}-selection-observed.xml`,lastXml);const actions=photoPickerActionControls(lastXml);write(`case-${caseNo}-commit-controls.json`,actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds})));if(actions.length){for(const action of actions){const b=action.bounds;adb('shell','input','tap',String(b.x),String(b.y));commitTaps++;log(`[PHOTO_PICKER] case ${caseNo}: auto commit "${action.exact}" at ${b.x},${b.y}`);await pcScreen(`case-${caseNo}-after-auto-commit-${commitTaps}.png`);if(await pickerClosedStable(page,7000)){const st3=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__)));if(st3.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit',sawSelection:true,commitTaps,action:action.exact};await sleep(1500);const st4=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__)));if(st4.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit-delayed-change',sawSelection:true,commitTaps,action:action.exact};return{committed:false,mode:'picker-closed-no-change',sawSelection:true,commitTaps,action:action.exact};}}}}
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V35__'; if(window[KEY])return;
  const s=window[KEY]={changes:[],capture:[],nativeReads:[],consoleErrors:[],events:[],lifecycle:[],nav:[],inputLifecycle:[],aborts:[],decodeOps:[],objectUrls:[],readConcurrency:[],domMutations:[]}; let activeReads=0; const readEnter=(kind,extra={})=>{activeReads++;s.readConcurrency.push({at:Date.now(),kind,phase:'start',activeReads,...extra});}; const readExit=(kind,extra={})=>{s.readConcurrency.push({at:Date.now(),kind,phase:'end',activeReads,...extra});activeReads=Math.max(0,activeReads-1);};
  // V35 non-product instrumentation: observe native browser reads without consuming the File ourselves.
  ...
  try {
    ...
    if(!orig.__tool001v35Wrapped){
      const wrapped=function(blob){
        ...
        s.nativeReads.push(meta); readEnter('FileReader',{id,size:meta.size,type:meta.type});
        const done=(phase)=>{
      ...
      Object.defineProperty(wrapped,'__tool001v35Wrapped',{value:true});
      proto.readAsArrayBuffer=wrapped;
    }
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V35_FILEREADER_OBSERVER:'+String(e?.message||e)}); }
  try {
    const proto=Blob.prototype, orig=proto.stream;
    if(typeof orig==='function'&&!orig.__tool001v35Wrapped){
      const wrapped=function(){
        s.nativeReads.push({at:Date.now(),phase:'blob-stream-call',size:Number(this?.size||0),type:String(this?.type||'')});
        return orig.call(this);
      };
      Object.defineProperty(wrapped,'__tool001v35Wrapped',{value:true});
      proto.stream=wrapped;
    }
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V35_STREAM_OBSERVER:'+String(e?.message||e)}); }
  try {
    ...
    if(typeof orig==='function'&&!orig.__tool001v35Wrapped){
      const wrapped=async function(){
        ...
        s.nativeReads.push({id,at:Date.now(),phase:'arraybuffer-call',size,type}); readEnter('Blob.arrayBuffer',{id,size,type});
      ...
      };Object.defineProperty(wrapped,'__tool001v35Wrapped',{value:true});proto.arrayBuffer=wrapped;
    }
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V35_ARRAYBUFFER_OBSERVER:'+String(e?.message||e)}); }
  try {
    const origAbort=FileReader.prototype.abort;
    if(typeof origAbort==='function'&&!origAbort.__tool001v35Wrapped){const wrapped=function(){s.aborts.push({at:Date.now(),kind:'FileReader.abort',readyState:this.readyState});return origAbort.call(this)};Object.defineProperty(wrapped,'__tool001v35Wrapped',{value:true});FileReader.prototype.abort=wrapped;}
    const AC=window.AbortController;
    if(AC&&AC.prototype){const orig=AC.prototype.abort;if(typeof orig==='function'&&!orig.__tool001v35Wrapped){const wrapped=function(reason){s.aborts.push({at:Date.now(),kind:'AbortController.abort',reason:String(reason||'')});return orig.call(this,reason)};Object.defineProperty(wrapped,'__tool001v35Wrapped',{value:true});AC.prototype.abort=wrapped;}}
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V35_ABORT_OBSERVER:'+String(e?.message||e)}); }
  try {
    const origCIB=window.createImageBitmap;
    if(typeof origCIB==='function'&&!origCIB.__tool001v35Wrapped){const wrapped=function(source,...rest){s.decodeOps.push({at:Date.now(),kind:'createImageBitmap',isBlob:source instanceof Blob,size:Number(source?.size||0),type:String(source?.type||'')});return origCIB.call(this,source,...rest)};Object.defineProperty(wrapped,'__tool001v35Wrapped',{value:true});window.createImageBitmap=wrapped;}
    const origCO=URL.createObjectURL,origRO=URL.revokeObjectURL;
    if(typeof origCO==='function'&&!origCO.__tool001v35Wrapped){const wrapped=function(obj){const u=origCO.call(this,obj);s.objectUrls.push({at:Date.now(),kind:'create',url:String(u),isBlob:obj instanceof Blob,size:Number(obj?.size||0),type:String(obj?.type||'')});return u};Object.defineProperty(wrapped,'__tool001v35Wrapped',{value:true});URL.createObjectURL=wrapped;}
    if(typeof origRO==='function'&&!origRO.__tool001v35Wrapped){const wrapped=function(u){s.objectUrls.push({at:Date.now(),kind:'revoke',url:String(u)});return origRO.call(this,u)};Object.defineProperty(wrapped,'__tool001v35Wrapped',{value:true});URL.revokeObjectURL=wrapped;}
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V35_DECODE_OBSERVER:'+String(e?.message||e)}); }
  try {
    const desc=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value');
    if(desc?.set&&desc?.get&&!desc.set.__tool001v35Wrapped){const origSet=desc.set,origGet=desc.get;const wrapped=function(v){if(this.type==='file')s.inputLifecycle.push({at:Date.now(),kind:'value-set',value:String(v),connected:this.isConnected,testid:this.getAttribute('data-testid')||''});return origSet.call(this,v)};Object.defineProperty(wrapped,'__tool001v35Wrapped',{value:true});Object.defineProperty(HTMLInputElement.prototype,'value',{get:origGet,set:wrapped,configurable:desc.configurable,enumerable:desc.enumerable});}
  ...
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V35_INPUT_DOM_OBSERVER:'+String(e?.message||e)}); }
...
async function ensureChromeInteractive(page,caseNo,tag='guard'){
  ...
  // V35: NEVER infer overflow menu from toolbar "새 탭".
...
async function autoSelectPickerMedia(caseNo,label,round,page,startChange){
  ...
  while(Date.now()<dl){
    const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__))).catch(()=>null);
    ...
    if(!snap.detected){
      await sleep(300);
      const obs2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__))).catch(()=>null);
...
async function waitProduct(page,startCapture,startNative,before,timeoutMs=6500){const deadline=Date.now()+timeoutMs;let snap=before,cap=[],nr=[];while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__)));cap=st.capture.slice(startCapture);nr=(st.nativeReads||[]).slice(startNative);snap=await domSnapshot(page);const preview=snap.cards.flatMap(c=>c.imgs||[]).some(img=>img.complete&&img.naturalWidth>0&&img.naturalHeight>0);const cardChanged=snap.cardCount!==before.cardCount;const capFail=cap.some(x=>x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail'||x.phase==='product-add-fail');const nativeFail=nr.some(x=>/error|throw|abort/.test(x.phase||'')&&/(NotReadableError|could not be read|UPLOAD_FILE_CHANGED|ERR_)/i.test(String(x.error||'')));if((cardChanged&&preview)||capFail||nativeFail)break;await sleep(220);}return{capture:cap,nativeReads:nr,after:snap};}
async function collectFailureEvidence(caseNo,label,page,reason){let xml='';try{xml=dumpUi();}catch{}write(`case-${caseNo}-${label}-failure-picker.xml`,xml);await pcScreen(`case-${caseNo}-${label}-failure-device.png`);await page.screenshot({path:path.join(outDir,`case-${caseNo}-${label}-failure-page.png`),fullPage:true}).catch(()=>{});const snap=await domSnapshot(page).catch(()=>null);const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__))).catch(()=>null);write(`case-${caseNo}-${label}-failure-evidence.json`,{reason,pickerOpen:pickerDetectedFromXml(xml),selectionSignals:pickerSelectionSignals(xml).selectedNodes.length,commitControls:photoPickerActionControls(xml).map(a=>({exact:a.exact,label:a.label,bounds:a.bounds})),foreground:foregroundSnapshot(),observer:obs,dom:snap});}
...
async function main(){
  log('=== TOOL001 REAL PHOTO HARNESS V35 FACTS-ONLY ===');log('[OUTPUT]',outDir);
  ...
  step('실기기 사용자흐름 검수 안내');log('');log('*** 검수기는 사용자 흐름만 자동 재현하고 사실만 기록합니다. ***');log('사용자 입력 없음: NotReadable 8대 원인군을 PHOTO_01~08 슬롯에 배정하고 각 3회 × 동일 세트 2회 자동 검수합니다.');log('');log('총 48회 실제 사용자 흐름을 빠르게 재현하고, 1세트/2세트 반복 상관관계로 불량 가지를 압축합니다.');log('');
  const results=[];
  for(let i=0;i<labels.length;i++){
    ...
    await ensureExpectedToolPage(page,caseNo,'case-start');await page.bringToFront();const before=await domSnapshot(page);const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__))); adb('logcat','-c'); let result={case:caseNo,label,categoryId:planEntry.id,categoryName:planEntry.name,photo:planEntry.photo,set:planEntry.set,repeat:planEntry.repeat,pass:false,error:null,before,commit:null};
    try{
      ...
      const changeDeadline=Date.now()+8000;let change=null;while(Date.now()<changeDeadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V35__)));if(st.changes.length>st0.changes.length){change=st.changes.at(-1);break;}await sleep(250);}if(!change)throw new Error('NO_FILE_CHANGE_AFTER_VERIFIED_COMMIT');
      ...
      write(`case-${caseNo}-${label}-native-reads.json`,nativeReads); write(`case-${caseNo}-${label}-root-cause-telemetry.json`,{inputLifecycle,aborts,decodeOps,objectUrls,readConcurrency,domMutations,lifecycle,events});
      write(`case-${caseNo}-${label}-logcat-evidence.txt`,logcatEvidence.join('\n'));
      const userPass=newCards>0&&previewCount>0;
      result={...result,change,capture:r.capture,nativeReads,inputLifecycle,aborts,decodeOps,objectUrls,readConcurrency,domMutations,lifecycle,events,logcatEvidence,readerTimeline:readers,byteSignature:sniff||null,inspection,providerReceived,productFail,after:r.after,capturePass:!!capPass,captureFail:capFail||null,newCards,previewCount,previewDims:previewImgs.map(x=>[x.naturalWidth,x.naturalHeight]),uiMessage:message,pass:userPass};
  ...
  step('검수 사실기록 생성');
  const summary=['TOOL001_NOTREADABLE_PYRAMID_V35',`URL=${url}`,`CASES=${results.length}`];
  for(const r of results){
    const f=r.change?.files?.[0]||{};
    const lastReader=r.readerTimeline?.at(-1)||{};
    summary.push(
      '',
      `[${r.label}]`,
      `PHOTO_SLOT=${r.commit?.slot??''}`,
      `FLOW_PASS=${r.pass}`,
      `COMMIT_MODE=${r.commit?.mode||''}`,
      `CHANGE_SEEN=${!!r.change}`,
      `INPUT_FILES=${r.change?.files?.length??''}`,
      `TRUSTED_CHANGE=${r.change?.trusted??''}`,
      `FILE_NAME=${f.name||''}`,
      `FILE_TYPE=${f.type||''}`,
      `FILE_SIZE=${f.size??''}`,
      `CAPTURE_PASS=${r.capturePass??false}`,
      `NATIVE_READS=${JSON.stringify(r.nativeReads||[])}`,
      `LOGCAT_EVIDENCE=${JSON.stringify((r.logcatEvidence||[]).slice(-20))}`, 
      `CAPTURE_FAIL=${r.captureFail?.error||''}`,
      `LAST_READER_PHASE=${lastReader.phase||''}`,
      `LAST_READER=${lastReader.reader||''}`,
      `LAST_READER_ERROR=${lastReader.error||''}`,
      `NEW_CARDS=${r.newCards??0}`,
      `PREVIEW_COUNT=${r.previewCount??0}`,
      `CHECKPOINTS=${JSON.stringify(r.checkpoints||checkpointSummary(r))}`, 
      `ERROR=${r.error||''}`
    );
  }
  write('summary.txt',summary.join('\n'));
  write('result.json',{version:'V35',mode:'NOTREADABLE_PYRAMID_8X3X2',url,sets:SETS,repeats:REPEATS,categories:CATEGORIES,generatedAt:new Date().toISOString(),results});
  step('최종 사실표');
  ...
  const matrix=CATEGORIES.map(cat=>{
    ...
    const bySet=[];for(let set=1;set<=SETS;set++){const x=rows.filter(r=>r.set===set);bySet.push({set,pass:x.filter(r=>r.pass).length,fail:x.filter(r=>!r.pass&&!/HARNESS_/.test(r.error||'')).length,harness:x.filter(r=>/HARNESS_/.test(r.error||'')).length,results:x.map(r=>({repeat:r.repeat,pass:r.pass,error:r.error||'',notReadable:!!r.pyramid?.notReadable,evidence:r.pyramid?.categoryEvidence?.[cat.id]??null}))});}
    const valid=rows.filter(r=>!/HARNESS_/.test(r.error||'')); const failCount=valid.filter(r=>!r.pass).length; const passCount=valid.filter(r=>r.pass).length; const bothSetsFail=bySet.length>=2&&bySet.every(s=>s.fail>0);
    const rank=bothSetsFail?'STRONG_BRANCH':failCount>0?'SUSPECT_BRANCH':valid.length===SETS*REPEATS&&passCount===SETS*REPEATS?'CLEAN_6_OF_6_CONFIRM_LATER':'INSUFFICIENT';
    return{id:cat.id,name:cat.name,photo:cat.photo,passCount,failCount,validCount:valid.length,bySet,rank};
  });
  write('pyramid-matrix.json',{version:'V35',sets:SETS,repeats:REPEATS,totalPlanned:plan.length,matrix});
  const mt=['TOOL001 NOTREADABLE PYRAMID V35','8 categories × 3 repeats × 2 sets',''];for(const m of matrix){mt.push(`${m.id} | ${m.name} | ${m.photo} | SET1 P${m.bySet[0]?.pass||0}/F${m.bySet[0]?.fail||0}/H${m.bySet[0]?.harness||0} | SET2 P${m.bySet[1]?.pass||0}/F${m.bySet[1]?.fail||0}/H${m.bySet[1]?.harness||0} | TOTAL P${m.passCount}/F${m.failCount} | ${m.rank}`)}write('pyramid-matrix.txt',mt.join('\n'));
  write('verdict.json',{allFlowPass:allPass,firstDivergence:firstDivergence(results),passCount:results.filter(r=>r.pass).length,failCount:results.filter(r=>!r.pass).length,matrix,strongBranches:matrix.filter(x=>x.rank==='STRONG_BRANCH').map(x=>x.id),suspectBranches:matrix.filter(x=>x.rank==='SUSPECT_BRANCH').map(x=>x.id),cleanCandidates:matrix.filter(x=>x.rank==='CLEAN_6_OF_6_CONFIRM_LATER').map(x=>x.id)});
  log('=== PYRAMID MATRIX ===');for(const m of matrix)log(`${m.id} ${m.photo}: SET1 P${m.bySet[0]?.pass||0}/F${m.bySet[0]?.fail||0} | SET2 P${m.bySet[1]?.pass||0}/F${m.bySet[1]?.fail||0} => ${m.rank}`);
  step('결과 ZIP 생성');restoreNotificationOverlays();await context.close().catch(()=>{});if(typeof device.close==='function')await device.close().catch(()=>{});adb('shell','svc','power','stayon','false');const zip=path.join(desktop,`${path.basename(outDir)}.zip`);const pr=spawnSync('powershell.exe',['-NoProfile','-Command',`Compress-Archive -Path '${outDir.replaceAll("'","''")}\\*' -DestinationPath '${zip.replaceAll("'","''")}' -Force`],{encoding:'utf8',shell:false});if(pr.status!==0)log('[WARN] ZIP creation failed',pr.stderr||pr.stdout||'');else log('[PASS] RESULT ZIP',zip);log('=== FINAL SUMMARY ===');for(const r of results)log(`${r.label}: ${r.pass?'PASS':'FAIL'} size=${r.change?.files?.[0]?.size??''} type=${r.change?.files?.[0]?.type??''} capture=${r.capturePass?'PASS':'FAIL'} preview=${r.previewCount??0} error=${r.error||''}`);log(`REPEAT_RESULT=PASS:${results.filter(r=>r.pass).length} FAIL:${results.filter(r=>!r.pass).length}`);log(`FIRST_DIVERGENCE=${firstDivergence(results)}`);log(`FINAL_HARNESS_FLOW=${allPass?'PASS':'MIXED_OR_FAIL'}`);log('PYRAMID_MATRIX=pyramid-matrix.txt');
}
