...
const runner=path.join(here,'run-tool-001-v51-chrome-webview-ab.mjs');
...
check('webview-load-datawithbaseurl-local',/webView\.loadDataWithBaseURL\(/.test(s) && /https:\/\/diag\.local\//.test(s) && !/webView\.loadData\(/.test(s));
check('webview-keep-screen-on-only',/WindowManager\.LayoutParams\.FLAG_KEEP_SCREEN_ON/.test(s));
...
check('desktop-result-zip',/TOOLBOX_001_V51_CHROME_WEBVIEW_AB_REAL_DEVICE_/.test(s) && /Compress-Archive/.test(s));
