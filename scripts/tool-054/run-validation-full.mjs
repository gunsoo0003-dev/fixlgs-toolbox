...
const allowed=new Set(["preflight", "core-only", "feature-only", "background-only", "boundary-only", "regression-only", "limit-only", "final"]);
if(!allowed.has(mode)){console.error(`UNKNOWN_MODE=${mode}`);process.exit(2)}
const isWin=process.platform==='win32';
const resultDir=process.env.TOOL054_RESULT_DIR?path.resolve(process.env.TOOL054_RESULT_DIR):path.join(process.env.USERPROFILE||os.homedir(),'Desktop');
fs.mkdirSync(resultDir,{recursive:true});
const stamp=new Date().toISOString().replace(/[:.]/g,'-');
const txt=path.join(resultDir,`054_${mode}_검수결과_${stamp}.txt`);
const rows=[];
