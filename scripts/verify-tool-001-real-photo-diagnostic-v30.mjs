#!/usr/bin/env node
import fs from 'fs';
const runner=fs.readFileSync(new URL('./run-tool-001-real-photo-diagnostic-v30.mjs',import.meta.url),'utf8');
...
const checks={
  version:/REAL_PHOTO_V30|DIAGNOSTIC_V30|version:'V30'/.test(runner),
  ...
  auto_commit_after_user_selection:/AUTO_COMMIT_AFTER_USER_SELECTION/.test(runner)&&/AUTO COMMIT/.test(runner),
  zero_user_action:!/ACTION REQUIRED/.test(runner)&&/무인 자동 진행/.test(runner),
  auto_media_candidates:/pickerMediaCandidates/.test(runner)&&/autoSelectPickerMedia/.test(runner),
  auto_media_safe_grid:/b\.cy<420/.test(runner)&&/b\.cy>2050/.test(runner)&&/AUTO_MEDIA_UNSAFE_BOUNDS/.test(runner),
  auto_media_repeat_mapping:/KNOWN_GOOD_REPEAT/.test(runner)&&/KNOWN_BAD_REPEAT_2/.test(runner)&&/caseAutoMediaIndex/.test(runner),
  auto_media_selection_confirmed:/AUTO_MEDIA_SELECTION_NOT_CONFIRMED/.test(runner)&&/pickerSelectionSignals/.test(runner),
  no_drag_handle:/drag\|handle\|손잡이/.test(runner),
  short_selection_timeout:/const selectionDeadline=Date\.now\(\)\+20000/.test(runner),
  commit_live_rescan:/commit-controls-r/.test(runner)&&/await sleep\(180\)/.test(runner),
  commit_revalidate_before_tap:/picker-not-active/.test(runner)&&/strictChromeOverflowMenuOpen/.test(runner),
  commit_single_tap:/do not tap again/.test(runner)&&/commitTaps:1/.test(runner),
  commit_safe_bounds:/b\.cy<1000/.test(runner)&&/bottom half/.test(runner),
  commit_picker_close_change_watch:/AUTO_COMMIT_PICKER_CLOSED_CHANGE/.test(runner)&&/waitChange\(6500\)/.test(runner),
  commit_not_exposed_evidence:/COMMIT_CONTROL_NOT_EXPOSED_AFTER_LIVE_RESCAN/.test(runner),
  auto_close_path:/SELECTION_AUTO_CLOSED_CHANGE/.test(runner),
...
console.log(JSON.stringify({STATIC_SELFTEST:missing.length?'FAIL':'PASS',version:'V30',missing,...checks},null,2));
