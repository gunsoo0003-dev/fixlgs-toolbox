...
function runPlaywrightPreflight() {
  ...
  if (existsSync(localCli)) {
    ...
    args = [localCli, 'test', 'tests/tool-013-preflight.spec.ts', '--workers=1'];
  } else {
    const npmCli = process.env.npm_execpath;
    if (npmCli && existsSync(npmCli)) {
      exe = process.execPath;
      args = [npmCli, 'exec', '--', 'playwright', 'test', 'tests/tool-013-preflight.spec.ts', '--workers=1'];
    } else if (process.platform === 'win32') {
      exe = 'cmd.exe';
      args = ['/d', '/s', '/c', 'npm', 'exec', '--', 'playwright', 'test', 'tests/tool-013-preflight.spec.ts', '--workers=1'];
    } else {
      exe = 'npm';
      args = ['exec', '--', 'playwright', 'test', 'tests/tool-013-preflight.spec.ts', '--workers=1'];
    }
  }
...
let failed = false;
const staticCode = runNodeScript('scripts/check-tool-013-harness.mjs');
if (staticCode !== 0) {
  failed = true;
  steps.push({
  ...
  console.error('HARNESS_ERROR: 013 static selector/DOM preflight failed. Feature tests were not started.');
} else {
  const runtimeCode = runPlaywrightPreflight();
  if (runtimeCode !== 0) {
    failed = true;
    console.error('HARNESS_ERROR: 013 runtime preflight failed. Treat as harness/connection error until selector, route, timing, or environment cause is resolved. Do not classify this as feature FAIL.');
  }
}
...
console.log(`013 PREFLIGHT RESULT: ${status.toUpperCase()}`);
console.log(`결과 ZIP: ${packaged.zipPath}`);
if (!failed) console.log('013 PREFLIGHT: PASS — harness can reach the route and required initial/ready-state DOM. Full feature validation was not run.');
