#!/usr/bin/env node
import fs from 'fs';
const runner=fs.readFileSync(new URL('./run-tool-001-real-photo-diagnostic-v25.mjs',import.meta.url),'utf8');
...
const checks={
  version:/REAL_PHOTO_V25|DIAGNOSTIC_V25|version:'V25'/.test(runner),
...
console.log(JSON.stringify({STATIC_SELFTEST:missing.length?'FAIL':'PASS',version:'V25',missing,...checks},null,2));
