...
const outDir = fs.mkdtempSync(path.join(os.tmpdir(), `tool033-${mode}-`));
const results = [];
...
function copyTreeIfPresent(src, dest) {
...
function isPlaywrightStep(args) {
  return args.some(a => String(a).includes('playwright.tool033.config.ts'));
}

function clearRuntimeEvidence() {
  for (const rel of ['test-results', 'playwright-report']) {
    try { fs.rmSync(path.join(root, rel), { recursive: true, force: true }); } catch {}
  }
}

function captureRuntimeEvidence(index, name) {
  const evidenceRoot = path.join(outDir, 'evidence', `${String(index).padStart(2, '0')}-${safeFilePart(name)}`);
  let copied = false;
  copied = copyTreeIfPresent(path.join(root, 'test-results'), path.join(evidenceRoot, 'test-results')) || copied;
  copied = copyTreeIfPresent(path.join(root, 'playwright-report'), path.join(evidenceRoot, 'playwright-report')) || copied;
  return copied;
}
...
async function runStep(index, total, name, cmd, args) {
  ...
  return await new Promise((resolve) => {
    ...
    const finish = (code, error) => {
      ...
      try { if (isPlaywrightStep(args)) evidenceCaptured = captureRuntimeEvidence(index, name); } catch (e) { stderr += `\nEVIDENCE_COPY_FAIL: ${e instanceof Error ? e.message : String(e)}\n`; }
      results.push({ name, status: code, error, started, ended, elapsedSec, evidenceCaptured });
    ...
    let child;
    try {
      if (isPlaywrightStep(args)) clearRuntimeEvidence();
...
const staticSteps = [
  ['source', node, ['scripts/tool-033/check-source.mjs']],
  ['harness', node, ['scripts/tool-033/check-harness.mjs']],
  ['checker-contract', node, ['scripts/tool-033/check-checker-contract.mjs']],
  ['design-static', node, ['scripts/tool-033/check-design-static.mjs']],
  ['limits-static', node, ['scripts/tool-033/check-limit-contract.mjs']],
  ['css-protection', node, ['scripts/tool-033/check-css-protection.mjs']],
];

const runtime = (spec, name) => [name, node, [pwCli, 'test', spec, '--workers=1', '--config=playwright.tool033.config.ts']];
let steps = [...staticSteps];
if (mode === 'core-only') steps.push(runtime('tests/tool-033-core.spec.ts', 'core'));
if (mode === 'boundary-only') steps.push(runtime('tests/tool-033-boundary.spec.ts', 'boundary'));
if (mode === 'feature-only') steps.push(runtime('tests/tool-033-feature.spec.ts', 'feature'));
if (mode === 'regression-only') steps.push(runtime('tests/tool-033-regression.spec.ts', 'regression'));
if (mode === 'limit-only') steps.push(runtime('tests/tool-033-limit.spec.ts', 'limit'));
if (mode === 'preflight') steps.push(runtime('tests/tool-033-preflight.spec.ts', 'harness-minimal-runtime'));
if (mode === 'final') {
  steps.push(['typescript', node, [tscCli, '--noEmit']]);
  steps.push(['production-build', node, ['scripts/tool-033/runtime-workspace.mjs', 'build']]);
  steps.push(runtime('tests/tool-033-preflight.spec.ts', 'preflight'));
  steps.push(runtime('tests/tool-033-core.spec.ts', 'core'));
  steps.push(runtime('tests/tool-033-boundary.spec.ts', 'boundary'));
  steps.push(runtime('tests/tool-033-feature.spec.ts', 'feature'));
  steps.push(runtime('tests/tool-033-regression.spec.ts', 'regression'));
  steps.push(runtime('tests/tool-033-limit.spec.ts', 'limit'));
}
...
try {
  ...
  if (missing.length) {
    ...
    console.log(`TOOL 033 ${mode.toUpperCase()} | total steps=${steps.length}`);
...
const summary = `TOOL 033 ${mode}\nPASS=${pass}\nFAIL=${fail}\nSKIP=0\nSTATUS=${ok ? 'PASS' : 'FAIL'}\n`;
try { fs.writeFileSync(path.join(outDir, 'summary.txt'), summary); } catch {}
try { fs.writeFileSync(path.join(outDir, 'summary.json'), JSON.stringify({ tool: '033', mode, pass, fail, skip: 0, status: ok ? 'PASS' : 'FAIL', results }, null, 2)); } catch {}

const desktop = path.join(process.env.USERPROFILE || os.homedir(), 'Desktop');
const outBase = process.env.TOOL033_RESULT_DIR ? path.resolve(process.env.TOOL033_RESULT_DIR) : desktop;
let zipPath = path.join(outBase, `033_${mode}_검수결과.zip`);
try {
