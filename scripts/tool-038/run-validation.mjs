...
const outDir=fs.mkdtempSync(path.join(os.tmpdir(),`tool038-${mode}-`));
const results=[];
...
const outBase=process.env.TOOL038_RESULT_DIR?path.resolve(process.env.TOOL038_RESULT_DIR):desktop;
fs.mkdirSync(outBase,{recursive:true});
const zipPath=path.join(outBase,`038_${mode}_검수결과.zip`);
const liveTxtPath=path.join(outBase,`038_${mode}_진행중.txt`);
const liveJsonPath=path.join(outBase,`038_${mode}_진행중.json`);
...
const staticSteps=[['source',node,['scripts/tool-038/check-source.mjs']],['logic-contract',node,['scripts/tool-038/check-logic.mjs']],['functional-fixtures',node,['--experimental-strip-types','scripts/tool-038/check-fixtures.mjs']]];
const spec=(f,n)=>[n,node,[pw,'test',f,'--workers=1','--config=playwright.tool038.config.ts']];let steps=[...staticSteps];
if(mode==='preflight')steps.push(spec('tests/tool-038-preflight.spec.ts','preflight-runtime'));
if(mode==='core-only')steps.push(spec('tests/tool-038-core.spec.ts','core'));
if(mode==='boundary-only')steps.push(spec('tests/tool-038-boundary.spec.ts','boundary'));
if(mode==='feature-only')steps.push(spec('tests/tool-038-feature.spec.ts','feature'));
if(mode==='regression-only')steps.push(spec('tests/tool-038-regression.spec.ts','regression'));
if(mode==='limit-only')steps.push(spec('tests/tool-038-limit.spec.ts','limit'));
if(mode==='final')steps.push(['typescript',node,[tsc,'--noEmit']],['production-build',node,[next,'build']],spec('tests/tool-038-preflight.spec.ts','preflight'),spec('tests/tool-038-core.spec.ts','core'),spec('tests/tool-038-boundary.spec.ts','boundary'),spec('tests/tool-038-feature.spec.ts','feature'),spec('tests/tool-038-regression.spec.ts','regression'),spec('tests/tool-038-limit.spec.ts','limit'));
try{
  ...
  else{console.log(`TOOL 038 ${mode.toUpperCase()} | total steps=${steps.length}`);for(let i=0;i<steps.length&&!interrupted;i++){const [name,cmd,args]=steps[i];await runStep(i+1,steps.length,name,cmd,args)}}
