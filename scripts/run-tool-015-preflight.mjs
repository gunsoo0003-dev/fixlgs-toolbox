...
function runRuntime(){
  ...
  let exe,args;
  if(existsSync(local)){exe=process.execPath;args=[local,'test','tests/tool-015-preflight.spec.ts','--workers=1'];}
  else{
    const npmCli=process.env.npm_execpath;
    if(npmCli&&existsSync(npmCli)){exe=process.execPath;args=[npmCli,'exec','--','playwright','test','tests/tool-015-preflight.spec.ts','--workers=1'];}
    else if(process.platform==='win32'){exe='cmd.exe';args=['/d','/s','/c','npm','exec','--','playwright','test','tests/tool-015-preflight.spec.ts','--workers=1'];}
    else{exe='npm';args=['exec','--','playwright','test','tests/tool-015-preflight.spec.ts','--workers=1'];}
  }
...
let failed=false;
const staticCode=runNode('scripts/check-tool-015-harness.mjs');
if(staticCode!==0){
  failed=true;
  ...
  console.error('[HARNESS_ERROR] 015 static harness failed. Runtime feature tests were not started.');
}else{
  const runtimeCode=runRuntime();
  if(runtimeCode!==0){failed=true;console.error('[HARNESS_ERROR] 015 runtime preflight failed. Resolve route/selector/timing/environment before classifying feature failures.');}
}

const endedAt=new Date(),status=failed?'failed':'passed';
const pkg=createValidationResultPackage({toolNumber:'015',validationType:'preflight',status,startedAt,endedAt,steps});
cleanupProjectValidationArtifacts();
let finalZip=pkg.zipPath;
if(pkg.zipCreated){
  const fixed=resolve(resolveDesktopPath(),'015_preflight_검수결과.zip');
...
console.log(`015 PREFLIGHT RESULT: ${status.toUpperCase()}`);
