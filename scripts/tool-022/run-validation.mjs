...
const outDir=fs.mkdtempSync(path.join(os.tmpdir(),`tool022-${mode}-`));
const results=[];
...
for(const [name,script] of [
 ['source','scripts/check-tool-022-source.mjs'],['harness','scripts/check-tool-022-harness.mjs'],['design','scripts/check-tool-022-design.mjs'],['validator','scripts/check-tool-022-validator.mjs'],['transpile','scripts/check-tool-022-transpile.mjs']
]) ok=run(name,'node',[script])&&ok;

const runtimeSpec={
 'preflight':'tests/tool-022-preflight.spec.ts','core-only':'tests/tool-022-core.spec.ts','boundary-only':'tests/tool-022-boundary.spec.ts','regression-only':'tests/tool-022-regression.spec.ts','limit-only':'tests/tool-022-limit.spec.ts'
};
...
if(!fs.existsSync(pwCli)){
  ...
  for(const [name,spec] of Object.entries(runtimeSpec)) ok=run(`playwright-${name}`,'node',[pwCli,'test',spec,'--workers=1','--config=playwright.tool022-runtime.config.ts'])&&ok;
}else{
  ok=run('playwright-runtime','node',[pwCli,'test',runtimeSpec[mode],'--workers=1','--config=playwright.tool022-runtime.config.ts'])&&ok;
}
...
const summary=`TOOL 022 ${mode}\nPASS=${pass}\nFAIL=${fail}\nSTATUS=${ok?'PASS':'FAIL'}\nDURATION_MS=${totalDurationMs}\nSLOWEST=${slowest.map(x=>`${x.name}:${x.durationMs}ms`).join(', ')}\n`;
fs.writeFileSync(path.join(outDir,'summary.txt'),summary);
fs.writeFileSync(path.join(outDir,'summary.json'),JSON.stringify({tool:'022',mode,pass,fail,status:ok?'PASS':'FAIL',totalDurationMs,slowest,results},null,2));
const zipName=`022_${mode}_검수결과.zip`;
const desktopDir=path.join(os.homedir(),'Desktop'); const defaultResultDir=fs.existsSync(desktopDir)?desktopDir:root;
const resultDir=process.env.TOOL022_RESULT_DIR?path.resolve(process.env.TOOL022_RESULT_DIR):defaultResultDir;
