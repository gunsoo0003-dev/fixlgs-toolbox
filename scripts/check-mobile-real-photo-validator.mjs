...
const scriptPath=process.env.MOBILE_VALIDATOR_SCRIPT || path.join(root,'scripts','run-mobile-real-photo-001-025.mjs');
...
const must=[
  ...
  'recoverToChrome','HARNESS_DOWNLOAD_FILE_NOT_OBSERVED','DOWNLOAD_BEFORE.json','DOWNLOAD_REDOWNLOAD_CONFIRM','TOOL005 CURRENT_RESULT_USE','TOOL003_UNSUPPORTED_INPUT','tool023-dropzone','tool025-file-input','tool025-dropzone','tool025-workspace-dropzone','TOOL025_INDIVIDUAL_DOWNLOAD','TOOL025_A4_DOWNLOAD','resolveTool025Base','HARNESS_WEB_ROUTE_NOT_AVAILABLE','HARNESS_TOOL025_BASE_UNAVAILABLE','no server spawn','cleanupAndroidResources','ANDROID_DEVICE_CLOSE','[EXIT] NORMAL','BLOCKED_STEPS','archiveResultFolder','RESULT ZIP TARGET','RETEST_HARNESS_INVALID_ORIGINAL_PRODUCT_FAIL_PRESERVED'
];
...
if(toolNums.length!==25)errors.push(`tool definitions != 25 (${toolNums.length})`);
for(let i=1;i<=25;i++)if(!toolNums.includes(i))errors.push(`missing TOOL${String(i).padStart(3,'0')}`);
if(new Set(toolNums).size!==toolNums.length)errors.push('duplicate tool definition');
for(const x of ['TOOLBOX_MOBILE_REALPHOTO_001_025_${stamp}','fatal-harness.json','RESULT ZIP TARGET','archiveResultFolder(outDir,zipPath)']) if(!script.includes(x))errors.push(`missing result/archive contract: ${x}`);
