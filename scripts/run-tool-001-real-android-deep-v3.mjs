...
const outDir = path.join(desktop, `TOOLBOX_001_REAL_ANDROID_DEEP_V3_${stamp}`);
...
function browserInit() {
  const KEY = '__TOOL001_REAL_ANDROID_V3__';
  if (window[KEY]) return;
  const state = window[KEY] = {
    version:'V3', bornEpoch:Date.now(), bornPerf:performance.now(), seq:0,
...
async function waitForAttempt(page, expected, timeout) {
  ...
  while(Date.now()-start<timeout){
    const s=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V3__?({count:window.__TOOL001_REAL_ANDROID_V3__.attempts.length,attempts:window.__TOOL001_REAL_ANDROID_V3__.attempts.map(a=>({attemptIndex:a.attemptIndex,done:a.done,fileCount:a.fileCount})),activeProbes:window.__TOOL001_REAL_ANDROID_V3__.activeProbes}):({count:0,attempts:[],activeProbes:0})).catch(()=>({count:0,attempts:[],activeProbes:0}));
...
async function main(){
  log('=== TOOL001 REAL ANDROID DEEP DIAGNOSTIC V3 ==='); log('[OUTPUT]',outDir);
  step('ADB 연결 확인');
  ...
  step('파일 입력요소 확인');
  await withHeartbeat('waitForSelector(file input)',()=>page.waitForSelector(selector,{timeout:30000}),35000);
  await page.evaluate(sel=>{if(window.__TOOL001_REAL_ANDROID_V3__)window.__TOOL001_REAL_ANDROID_V3__.selector=sel;},selector);
  await page.screenshot({path:path.join(outDir,'before.png'),fullPage:true}).catch(()=>{});

  step(`실기기 파일 선택 ${attemptsTarget}회 진단 대기`);
  for(let i=1;i<=attemptsTarget;i++){
    ...
    try{await waitForAttempt(page,i,waitChangeMs);log(`[ATTEMPT ${i}] change + file probe COMPLETE`);await sleep(settleMs);await page.screenshot({path:path.join(outDir,`attempt-${String(i).padStart(2,'0')}-after.png`),fullPage:true}).catch(()=>{});const p=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V3__)));write('timeline-live.json',p.timeline);write('attempts-live.json',p.attempts);}catch(e){log(`[ATTEMPT ${i}] TIMEOUT/ERROR`,e.message);const p=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V3__?JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V3__)):null).catch(()=>null);if(p){write('timeline-live.json',p.timeline);write('attempts-live.json',p.attempts);}break;}
  }

  step('브라우저/파일 진단 결과 수집');
  const state=await withHeartbeat('collect browser diagnostic state',()=>page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V3__))),20000);
  ...
  write('result.json',{meta:{version:'V3',generatedAt:new Date().toISOString(),url,selector,attemptsTarget,attemptsObserved:state.attempts.length,node:process.version,hostPlatform:process.platform},environment:state.environment,attempts:state.attempts,timeline:state.timeline,domFinal});
  write('timeline.json',state.timeline);write('attempts.json',state.attempts);write('dom-final.json',domFinal);
  const s=['TOOL001_REAL_ANDROID_DEEP_DIAGNOSTIC_V3',`URL=${url}`,`ATTEMPTS_TARGET=${attemptsTarget}`,`ATTEMPTS_OBSERVED=${state.attempts.length}`,`USER_AGENT=${state.environment?.userAgent||''}`,`PLATFORM=${state.environment?.platform||''}`,`MAX_TOUCH_POINTS=${state.environment?.maxTouchPoints??''}`];
