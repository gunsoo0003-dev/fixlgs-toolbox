...
const page=fs.readFileSync('components/tool-080-rental-yield-calculator-page.tsx','utf8');
const css=fs.readFileSync('components/tool-080-rental-yield-calculator.module.css','utf8');
const seq=[
...
check('ToolNavigation current tool',page.includes('ToolNavigation')&&page.includes('currentTool={80}'));
check('expert shared wide head',page.includes('toolbox-tool-expert-post--wide-head'));
const expertTags=["GROSS", "CAPITAL", "MANAGEMENT", "INTEREST", "NET", "SCOPE"];
...
check('actual tool number',page.includes('080'));
...
console.log(`RESULT TOOL080 DESIGN PASS=${pass} FAIL=${fail}`);
