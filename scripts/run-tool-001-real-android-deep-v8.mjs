...
const outDir = path.join(desktop, `TOOLBOX_001_REAL_ANDROID_DEEP_V8_${stamp}`);
...
function currentFocusLine() {
  ...
  return (txt.split(/\r?\n/).find(x=>/mCurrentFocus|mFocusedApp/.test(x)) || '').trim();
}
function screenStateText() {
  ...
  return { power, display, policy, focus: currentFocusLine() };
}
function isScreenAwake(snapshot) {
  const t = `${snapshot?.power||''}\n${snapshot?.display||''}`;
  return /mWakefulness=Awake/i.test(t) || /Display Power:\s*state=ON/i.test(t) || /state=ON/i.test(t);
}
function isKeyguardShowing(snapshot) {
  const t = snapshot?.policy || '';
  return /isStatusBarKeyguard=true/i.test(t) || /mShowingLockscreen=true/i.test(t) || /keyguard.*showing=true/i.test(t) || /showing=true.*keyguard/i.test(t);
}
async function configureStayAwake() {
  originalStayAwake = adbText('shell','settings','get','global','stay_on_while_plugged_in').trim();
  const r = adb('shell','settings','put','global','stay_on_while_plugged_in','3'); // AC + USB
...
async function ensureDeviceReady(page, label='device ready') {
  ...
  adb('shell','input','keyevent','KEYCODE_WAKEUP');
  await sleep(350);
  // Android 15: dismiss-keyguard safely requests dismissal. It does not bypass a PIN/password.
  ...
  if (isKeyguardShowing(snap)) {
    // A swipe can clear AOD/non-secure lock screens; secure locks remain locked and are reported below.
  ...
  if (isKeyguardShowing(snap)) {
    write('device-keyguard-blocked.txt', `${snap.focus}\n\n${snap.policy}`);
    throw new Error('Device is still on a secure lock screen. Unlock the Galaxy once, then rerun; V8 will keep it awake afterward.');
  }
  // Bring the already-controlled Chrome task/page back to foreground if another system UI was on top.
  if (!/com\.android\.chrome/i.test(snap.focus)) {
  ...
  if (!/com\.android\.chrome/i.test(snap.focus)) {
    throw new Error(`Chrome is not foreground after recovery: ${snap.focus}`);
  }
  log(`[PASS] device awake + unlocked + Chrome foreground (${snap.focus})`);
  return snap;
}
...
function dumpUi() {
  adb('shell','uiautomator','dump','/sdcard/tool001-v8-window.xml');
  const xml = adbText('shell','cat','/sdcard/tool001-v8-window.xml');
  return xml;
}
...
async function saveDeviceScreen(name) {
  const remote='/sdcard/tool001-v8-screen.png';
...
async function prepareDeviceFiles(count) {
  ...
  for(let i=1;i<=count;i++){
    const name=`TOOL001_V8_${token}_${String(i).padStart(2,'0')}.png`;
...
async function clickUploadButton(page, attemptIndex) {
  ...
  const geom = await btn.evaluate(el => {
    ...
    return {x:r.left,y:r.top,width:r.width,height:r.height,innerWidth:window.innerWidth,innerHeight:window.innerHeight,dpr:window.devicePixelRatio||1,text:(el.textContent||'').trim()};
  });
  ...
  // Primary path in V8: real Android OS tap. This avoids Playwright actionability/stability checks entirely.
  ...
  const sw=Number(m[1]), sh=Number(m[2]);
  const scale=sw/Math.max(1,geom.innerWidth);
  const contentPxH=Math.round(geom.innerHeight*scale);
  const topOffset=Math.max(0,sh-contentPxH);
  const ax=Math.max(1,Math.min(sw-1,Math.round(cssX*scale)));
  const ay=Math.max(1,Math.min(sh-1,Math.round(topOffset+cssY*scale)));
  log(`[TOUCH] button="${geom.text}" css=(${cssX},${cssY}) physical=(${ax},${ay}) screen=${sw}x${sh} viewport=${geom.innerWidth}x${geom.innerHeight} topOffset≈${topOffset}`);
  const r=adb('shell','input','tap',String(ax),String(ay));
  if(r.status!==0) throw new Error(`ADB tap failed: ${r.stderr||r.stdout||''}`);
  log(`[PASS] ADB real-device tap sent at (${ax},${ay})`);
  return {method:'adb.tap',cssX,cssY,x:ax,y:ay,screen:{sw,sh},viewport:{w:geom.innerWidth,h:geom.innerHeight},topOffset};
}

function browserInit() {
  const KEY = '__TOOL001_REAL_ANDROID_V8__';
  if (window[KEY]) return;
  const state = window[KEY] = {
    version:'V8', bornEpoch:Date.now(), bornPerf:performance.now(), seq:0,
...
async function waitForAttempt(page, expected, timeout) {
  ...
  while(Date.now()-start<timeout){
    const s=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V8__?({count:window.__TOOL001_REAL_ANDROID_V8__.attempts.length,attempts:window.__TOOL001_REAL_ANDROID_V8__.attempts.map(a=>({attemptIndex:a.attemptIndex,done:a.done,fileCount:a.fileCount})),activeProbes:window.__TOOL001_REAL_ANDROID_V8__.activeProbes}):({count:0,attempts:[],activeProbes:0})).catch(()=>({count:0,attempts:[],activeProbes:0}));
...
async function main(){
  log('=== TOOL001 REAL ANDROID DEEP DIAGNOSTIC V8 ==='); log('[OUTPUT]',outDir);
  step('ADB 연결 확인');
  ...
  await page.evaluate(sel=>{if(window.__TOOL001_REAL_ANDROID_V8__)window.__TOOL001_REAL_ANDROID_V8__.selector=sel;},selector);
  ...
  for(let i=1;i<=attemptsTarget;i++){
    ...
    try{
      ...
      const p=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V8__)));
      ...
      const p=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V8__?JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V8__)):null).catch(()=>null);
  ...
  const state=await withHeartbeat('collect browser diagnostic state',()=>page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V8__))),20000);
  ...
  write('result.json',{meta:{version:'V8',generatedAt:new Date().toISOString(),url,selector,attemptsTarget,attemptsObserved:state.attempts.length,node:process.version,hostPlatform:process.platform},environment:state.environment,attempts:state.attempts,timeline:state.timeline,domFinal});
  write('timeline.json',state.timeline);write('attempts.json',state.attempts);write('dom-final.json',domFinal);
  const s=['TOOL001_REAL_ANDROID_DEEP_DIAGNOSTIC_V8',`URL=${url}`,`ATTEMPTS_TARGET=${attemptsTarget}`,`ATTEMPTS_OBSERVED=${state.attempts.length}`,`USER_AGENT=${state.environment?.userAgent||''}`,`PLATFORM=${state.environment?.platform||''}`,`MAX_TOUCH_POINTS=${state.environment?.maxTouchPoints??''}`];
