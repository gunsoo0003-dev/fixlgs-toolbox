...
const outDir=path.join(desktop,`TOOLBOX_001_V40_THREE_CAUSE_AB_REAL_DEVICE_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
async function main(){
  ...
  const MODES=[
    {id:'CAUSE_A_FIRST_REFERENCE',captureMode:'FIRST_GETFILE_IMMEDIATE',label:'첫 getFile 참조 즉시 read'},
    {id:'CAUSE_B_FRESH_REACQUIRE',captureMode:'FRESH_REACQUIRE_IMMEDIATE',label:'같은 handle에서 새 File 재획득 후 즉시 read'},
    {id:'CAUSE_C_STABILIZED_REACQUIRE',captureMode:'STABILIZED_REACQUIRE',label:'500ms 안정화 후 새 File 재획득 read'},
  ];
  const plan=[];for(let round=1;round<=REPEATS;round++)for(const mode of MODES)plan.push({...mode,round});
  log('=== TOOL001 V40 THREE CAUSE A/B REAL DEVICE ===');log('[OUTPUT]',outDir);
  step('SELF-CHECK');if(adb('version').status!==0)throw new Error('ADB_NOT_AVAILABLE');if(!/\tdevice\b/.test(adbText('devices')))throw new Error('ADB_DEVICE_NOT_READY');write('self-check.txt','SELF_CHECK=PASS\nMODE=V40_THREE_CAUSE_10X_EACH\nTOTAL=30\n');log('[PASS] SELF-CHECK');
  ...
  step('V40 관찰기 주입');await page.addInitScript(browserInit);
  ...
  const support=await page.evaluate(()=>({ua:navigator.userAgent,showOpenFilePicker:typeof window.showOpenFilePicker==='function'}));write('v40-feature-support.json',support);log(`[FEATURE] showOpenFilePicker=${support.showOpenFilePicker}`);if(!support.showOpenFilePicker)throw new Error('V40_STABLE_PICKER_UNSUPPORTED_ON_DEVICE');
  step('남은 3개 원인 후보 각 10회 A/B 검수');const results=[];
  for(let i=0;i<plan.length;i++){
    const item=plan[i],caseNo=i+1;log('');log(`===== V40 ${item.id} ROUND ${item.round}/${REPEATS} CASE ${caseNo}/${plan.length} =====`);
    await ensureExpectedToolPage(page,caseNo,'case-start');await page.bringToFront();
    await page.evaluate(mode=>{window.__TOOL001_V40_CAPTURE_MODE__=mode;},item.captureMode);
    ...
    const r={case:caseNo,round:item.round,mode:item.id,captureMode:item.captureMode,label:item.label,pass:false,error:'',capture:[],observer:{},summary:null,trace:null,newCards:0,previewCount:0};
    try{
    ...
    r.v40Events=(r.capture||[]).filter(x=>String(x.phase||'').startsWith('v40-'));
    r.v40ReadFail=r.v40Events.find(x=>x.phase==='v40-read-fail')||null;
    r.v40ReadPass=r.v40Events.some(x=>x.phase==='v40-read-pass');
    r.v40GetFilePass=r.v40Events.filter(x=>x.phase==='v40-getfile-pass').length;
    ...
    const traceLines=[`CASE=${caseNo}`,`MODE=${item.id}`,`ROUND=${item.round}`,`USER_VISIBLE=${r.pass?'SUCCESS':'FAIL'}`,`LAST_SUCCESS_STEP=${r.trace.lastSuccessStep}`,`FIRST_FAIL_STEP=${r.trace.firstFailStep}`,`ERROR_NAME=${r.trace.errorName||''}`,`NOTREADABLE_EVENTS=${r.trace.notReadableEvents}`,`V40_GETFILE_PASS_COUNT=${r.v40GetFilePass}`,`V40_READ_PASS=${r.v40ReadPass}`,`V40_READ_FAIL=${r.v40ReadFail?JSON.stringify(r.v40ReadFail):''}`,`CARD_ADDED=${r.trace.cardAdded}`,`VISIBLE_PREVIEW=${r.trace.visiblePreview}`];write(`case-${caseNo}-v40-trace.txt`,traceLines.join('\n'));await pcScreen(`case-${caseNo}-device.png`);
    log(`[${r.pass?'PASS':'FAIL'}] ${item.id} R${item.round} NR=${r.trace.notReadableEvents} getFilePass=${r.v40GetFilePass} readPass=${r.v40ReadPass} card=${r.newCards} preview=${r.previewCount}`);results.push(r);
    if(i<plan.length-1){await page.goto(url,{waitUntil:'domcontentloaded',timeout:45000});await sleep(350);}
  }
  step('3개 원인 후보 비교 집계');
  const modeSummary=MODES.map(m=>{const rows=results.filter(r=>r.mode===m.id);const pass=rows.filter(r=>r.pass).length;const fail=rows.length-pass;const nr=rows.reduce((a,r)=>a+(r.trace?.notReadableEvents||0),0);const readPass=rows.filter(r=>r.v40ReadPass).length;return{id:m.id,label:m.label,total:rows.length,pass,fail,readPass,notReadableEvents:nr,rate:`${pass}/${rows.length}`,candidate:pass===rows.length?'100_PERCENT_CANDIDATE':'NOT_100_PERCENT'};});
  const winners=modeSummary.filter(x=>x.pass===x.total);
  const txt=['TOOL001 V40 THREE CAUSE A/B',`TOTAL=${results.length}`,`PHOTO=PHOTO_01`,`REPEATS_PER_MODE=${REPEATS}`,'','MODE_SUMMARY',...modeSummary.map(x=>`${x.id} PASS=${x.pass}/${x.total} FAIL=${x.fail} READ_PASS=${x.readPass}/${x.total} NOTREADABLE_EVENTS=${x.notReadableEvents} RESULT=${x.candidate}`),'',`WINNER_COUNT=${winners.length}`,`WINNERS=${winners.map(x=>x.id).join(',')||'NONE'}`,'',winners.length===1?`INTERPRETATION=ONE_BRANCH_STANDS_OUT:${winners[0].id}`:winners.length>1?'INTERPRETATION=MULTIPLE_100_PERCENT_BRANCHES':'INTERPRETATION=NO_100_PERCENT_BRANCH_COMMON_BOUNDARY_REMAINS','','ROWS',...results.map(r=>`CASE_${String(r.case).padStart(2,'0')} ${r.mode} R${r.round} ${r.pass?'PASS':'FAIL'} NR=${r.trace?.notReadableEvents||0} GET=${r.v40GetFilePass} READ=${r.v40ReadPass?'PASS':'FAIL'} ERROR=${r.v40ReadFail?.error||r.trace?.errorName||'-'}`)];
  write('v40-three-cause-summary.txt',txt.join('\n'));write('result.json',{version:'V40-THREE-CAUSE-AB',url,generatedAt:new Date().toISOString(),modeSummary,results});log(txt.join('\n'));
  step('결과 ZIP 생성');restoreNotificationOverlays();await context.close().catch(()=>{});if(typeof device.close==='function')await device.close().catch(()=>{});adb('shell','svc','power','stayon','false');const zip=path.join(desktop,`${path.basename(outDir)}.zip`);const pr=spawnSync('powershell.exe',['-NoProfile','-Command',`Compress-Archive -Path '${outDir.replaceAll("'","''")}\\*' -DestinationPath '${zip.replaceAll("'","''")}' -Force`],{encoding:'utf8',shell:false});if(pr.status!==0)log('[WARN] ZIP creation failed',pr.stderr||pr.stdout||'');else log('[PASS] RESULT ZIP',zip);log('SUMMARY=v40-three-cause-summary.txt');
}
