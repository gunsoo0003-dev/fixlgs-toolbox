...
const artifactNames = [
  ...
  'tool-008-auto-limit-report.txt',
  'tool-008-auto-limit-report.json',
  'tool-008-validation-master.txt',
  'tool-008-validation-master.json',
  'tool-008-build.log',
  'tool-008-core.log',
  'tool-008-limit.log',
];
...
const steps = [
  ...
  { name: 'core', args: ['run', 'test:toolbox:008-core'], summary: true },
  { name: 'limit', args: ['run', 'test:toolbox:008-limit'], summary: true },
];
...
for (const step of steps) {
  ...
  const logPath = resolve(outDir, `tool-008-${step.name}.log`);
  ...
  if (step.summary) {
    ...
    if (existsSync(genericJson)) copyFileSync(genericJson, resolve(outDir, `tool-008-${step.name}-summary.json`));
    if (existsSync(genericTxt)) copyFileSync(genericTxt, resolve(outDir, `tool-008-${step.name}-summary.txt`));
  ...
  if (status === 0) console.log(`[PASS] 008 ${step.name}`);
  else {
    console.error(`[FAIL] 008 ${step.name}: exit ${status}${run.signal ? ` / signal ${run.signal}` : ''}`);
...
const counts = {
  total: (core?.counts?.total || 0) + (limit?.counts?.total || 0),
  passed: (core?.counts?.passed || 0) + (limit?.counts?.passed || 0),
  failed: (core?.counts?.failed || 0) + (limit?.counts?.failed || 0),
  skipped: (core?.counts?.skipped || 0) + (limit?.counts?.skipped || 0),
};
const failed = results.some(x => x.status !== 0) || counts.failed > 0;
const missingReports = results.filter(x => ['core','limit'].includes(x.name) && !x.summary).map(x => x.name);
...
const lines = [
  ...
  '',
  '[한계탐색 결과]',
  existsSync(resolve(outDir, 'tool-008-auto-limit-report.txt')) ? '생성됨' : '생성되지 않음',
];

writeFileSync(resolve(outDir, 'tool-008-validation-master.json'), JSON.stringify(master, null, 2), 'utf8');
writeFileSync(resolve(outDir, 'tool-008-validation-master.txt'), lines.join('\n') + '\n', 'utf8');
...
console.log(`\n008 FINAL RESULT: ${overallStatus.toUpperCase()}`);
console.log(`통합 보고서: ${resolve(outDir, 'tool-008-validation-master.txt')}`);
