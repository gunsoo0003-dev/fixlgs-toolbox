...
const outDir = fs.mkdtempSync(path.join(os.tmpdir(), `tool027-${mode}-`));
const results = [];
...
const staticSteps = [
  ['runner-contract', node, ['scripts/tool-027/check-runner-contract.mjs']],
  ['source', node, ['scripts/tool-027/check-source.mjs']],
  ['logic', node, ['scripts/tool-027/check-logic.mjs']],
  ['integration', node, ['scripts/tool-027/check-integration.mjs']],
  ['design-transplant', node, ['scripts/tool-027/check-design-transplant.mjs']],
  ['content', node, ['scripts/tool-027/check-content.mjs']],
  ['common-design', node, ['scripts/check-toolbox-common-design.mjs']],
];

const runtime = (spec, name) => [name, node, [pwCli, 'test', spec, '--workers=1', '--config=playwright.tool027.config.ts']];
let steps = [...staticSteps];
if (mode === 'core-only') steps.push(runtime('tests/tool-027-core.spec.ts', 'core'));
if (mode === 'boundary-only') steps.push(runtime('tests/tool-027-boundary.spec.ts', 'boundary'));
if (mode === 'feature-only') {
  steps.push(runtime('tests/tool-027-feature.spec.ts', 'feature'));
  steps.push(runtime('tests/tool-027-design-state.spec.ts', 'feature-design-state'));
}
if (mode === 'regression-only') steps.push(runtime('tests/tool-027-regression.spec.ts', 'regression'));
if (mode === 'limit-only') steps.push(runtime('tests/tool-027-limit.spec.ts', 'limit'));
if (mode === 'preflight') steps.push(runtime('tests/tool-027-preflight.spec.ts', 'harness-minimal-runtime'));
if (mode === 'final') {
  steps.push(['typescript', node, [tscCli, '--noEmit']]);
  steps.push(['production-build', node, ['scripts/tool-027/runtime-workspace.mjs', 'build']]);
  steps.push(runtime('tests/tool-027-core.spec.ts', 'core'));
  steps.push(runtime('tests/tool-027-boundary.spec.ts', 'boundary'));
  steps.push(runtime('tests/tool-027-feature.spec.ts', 'feature'));
  steps.push(runtime('tests/tool-027-design-state.spec.ts', 'feature-design-state'));
  steps.push(runtime('tests/tool-027-regression.spec.ts', 'regression'));
  steps.push(runtime('tests/tool-027-limit.spec.ts', 'limit'));
}
...
try {
  ...
  try {
    ...
    const lockedRoot = lock.packages?.['']?.dependencies?.['pdfjs-dist'];
    const installedPath = path.join(root, 'node_modules', 'pdfjs-dist', 'package.json');
    const installed = fs.existsSync(installedPath) ? JSON.parse(fs.readFileSync(installedPath, 'utf8')).version : null;
    if (declared !== expectedPdfJs) dependencyProblem = `package.json pdfjs-dist must be ${expectedPdfJs}, got ${declared ?? 'missing'}`;
    else if (lockedRoot !== expectedPdfJs) dependencyProblem = `package-lock is not synchronized for pdfjs-dist ${expectedPdfJs}`;
    else if (installed !== expectedPdfJs) dependencyProblem = `installed pdfjs-dist must be ${expectedPdfJs}, got ${installed ?? 'missing'}`;
  } catch (e) {
    dependencyProblem = `dependency metadata check failed: ${e instanceof Error ? e.message : String(e)}`;
  }

  if (dependencyProblem) missing.push(dependencyProblem);

  if (missing.length) {
    const msg = `ENVIRONMENT_FAIL: ${missing.join('; ')}. Run npm install pdfjs-dist@5.4.54 --save, then npm install before validation.\n`;
    ...
    console.log(`TOOL 027 ${mode.toUpperCase()} | total steps=${steps.length}`);
...
const summary = `TOOL 027 ${mode}\nPASS=${pass}\nFAIL=${fail}\nSKIP=0\nSTATUS=${ok ? 'PASS' : 'FAIL'}\n`;
try { fs.writeFileSync(path.join(outDir, 'summary.txt'), summary); } catch {}
try { fs.writeFileSync(path.join(outDir, 'summary.json'), JSON.stringify({ tool: '027', mode, pass, fail, skip: 0, status: ok ? 'PASS' : 'FAIL', results }, null, 2)); } catch {}

const desktop = path.join(process.env.USERPROFILE || os.homedir(), 'Desktop');
const outBase = process.env.TOOL027_RESULT_DIR ? path.resolve(process.env.TOOL027_RESULT_DIR) : desktop;
let zipPath = path.join(outBase, `027_${mode}_검수결과.zip`);
try {
