...
const outDir=fs.mkdtempSync(path.join(os.tmpdir(),`tool036-${mode}-`));
const results=[];
...
const outBase=process.env.TOOL036_RESULT_DIR?path.resolve(process.env.TOOL036_RESULT_DIR):desktop;
fs.mkdirSync(outBase,{recursive:true});
const zipPath=path.join(outBase,`036_${mode}_검수결과.zip`);
const liveTxtPath=path.join(outBase,`036_${mode}_진행중.txt`);
const liveJsonPath=path.join(outBase,`036_${mode}_진행중.json`);
...
const staticSteps=[['source',node,['scripts/tool-036/check-source.mjs']],['harness-static',node,['scripts/tool-036/check-harness.mjs']],['design-code',node,['scripts/tool-036/check-design-static.mjs']],['css-protection',node,['scripts/tool-036/check-css-protection.mjs']],['localization',node,['scripts/tool-036/check-localization.mjs']],['functional-fixtures',node,['--experimental-strip-types','scripts/tool-036/check-functional-fixtures.mjs']],['main-integration',node,['scripts/tool-036/check-main-integration.mjs']]];
const spec=(f,n)=>[n,node,[pw,'test',f,'--workers=1','--config=playwright.tool036.config.ts']];let steps=[...staticSteps];
if(mode==='preflight')steps.push(spec('tests/tool-036-preflight.spec.ts','preflight-runtime'));
if(mode==='core-only')steps.push(spec('tests/tool-036-core.spec.ts','core'));
if(mode==='boundary-only')steps.push(spec('tests/tool-036-boundary.spec.ts','boundary'));
if(mode==='feature-only')steps.push(spec('tests/tool-036-feature.spec.ts','feature'));
if(mode==='regression-only')steps.push(spec('tests/tool-036-regression.spec.ts','regression'));
if(mode==='limit-only')steps.push(spec('tests/tool-036-limit.spec.ts','limit'));
if(mode==='final')steps.push(['typescript',node,[tsc,'--noEmit']],['production-build',node,[next,'build']],spec('tests/tool-036-preflight.spec.ts','preflight'),spec('tests/tool-036-core.spec.ts','core'),spec('tests/tool-036-boundary.spec.ts','boundary'),spec('tests/tool-036-feature.spec.ts','feature'),spec('tests/tool-036-regression.spec.ts','regression'),spec('tests/tool-036-limit.spec.ts','limit'));
try{
  ...
  else{console.log(`TOOL 036 ${mode.toUpperCase()} | total steps=${steps.length}`);for(let i=0;i<steps.length&&!interrupted;i++){const [name,cmd,args]=steps[i];await runStep(i+1,steps.length,name,cmd,args)}}
