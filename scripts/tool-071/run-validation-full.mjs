...
const txt=path.join(resultDir,`071_final_검수결과_${stamp}.txt`);
const rows=[];
...
function browser(){return isWin?run('browser','npx playwright test --config=playwright.tool071.config.ts --workers=1 --reporter=list',[],{shell:true}):run('browser','npx',['playwright','test','--config=playwright.tool071.config.ts','--workers=1','--reporter=list'])}

npmRun('static','tool071:static');
run('secret-scan',process.execPath,['scripts/tool-071/check-secret-scan.mjs']);
if(deps()){
  browser();
...
const summary=`TOOL071 MODE=final\nPASS=${pass}\nFAIL=${fail}\nMAIN_WORK=${missingDeps?1:0}\nENVIRONMENT_BLOCK=0\nSTATUS=${status}\n`;
