...
const defs = {
  'core-only': ['run', 'test:toolbox:013-core'],
  'boundary-only': ['run', 'test:toolbox:013-boundary'],
};
...
const pkg=createValidationResultPackage({toolNumber:'013',validationType:mode,status:code===0?'passed':'failed',startedAt,endedAt:new Date(),steps});
cleanupProjectValidationArtifacts();
console.log(`013 ${mode.toUpperCase()} RESULT: ${code===0?'PASSED':'FAILED'}`);
