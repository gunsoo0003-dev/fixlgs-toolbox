...
/*
  FIXLGS TOOLBOX 001~035 Android Chrome real-photo smoke validator V24
...
let BASE = String(args['base-url'] || 'https://www.fixlgs.com/tools').replace(/\/$/, '');
...
const outDir = path.join(desktop, `TOOLBOX_MOBILE_REALPHOTO_001_035_${stamp}`);
...
const TOOLS = [
  ...
  t(34,'pdf-password-metadata','[data-testid="tool034-file-input"]',['[data-testid="tool034-workspace"]'],{kind:'pdf-password-metadata',click:'[data-testid="tool034-set-password"]',result:'[data-testid="tool034-result"]',download:'[data-testid="tool034-download"]'},{uploadControls:['[data-testid="tool034-dropzone"] button'],uploadText:/PDF 선택|Choose PDF|PDFを選択/i,fixedGalleryInputUnsupported:true,inputKind:'pdf-document'}),
  t(35,'pdf-text-image-extractor','[data-testid="tool035-file-input"]',['[data-testid="tool035-workspace"]'],{kind:'click-result',click:'[data-testid="tool035-extract"]',result:'[data-testid="tool035-results"]'},{uploadControls:['[data-testid="tool035-dropzone"] button'],uploadText:/PDF 선택|Choose PDF|PDFを選択/i,fixedGalleryInputUnsupported:true,inputKind:'pdf-document'}),
...
async function runToolWorkflow(page,tool,caseId){
  ...
  if(w.kind==='pdf-password-metadata'){
    const password='Tool034!Mobile#2026';
    const newPassword=page.locator('[data-testid="tool034-new-password"]').first();
    const confirm=page.locator('[data-testid="tool034-confirm-password"]').first();
    await newPassword.waitFor({state:'visible',timeout:SAFETY_RESULT_MS});
