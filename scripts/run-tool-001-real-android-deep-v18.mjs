...
const outDir = path.join(desktop, `TOOLBOX_001_REAL_ANDROID_DEEP_V18_${stamp}`);
...
async function assertInteractiveUnlocked(label='lock-check') {
  ...
  if (s.blocked) {
    ...
    throw new Error(`SECURE_LOCK_ACTIVE at ${label}. Unlock the Galaxy once and rerun; V18 keeps the display awake after startup.`);
  }
  return s;
}
...
async function ensureDeviceReady(page, label='device ready') {
  ...
  if (isKeyguardShowing(snap)) {
    ...
    throw new Error('Device is still on a secure lock screen. Unlock the Galaxy once, then rerun; V18 keeps it awake afterward.');
  }
...
function dumpUi() {
  const remote='/sdcard/tool001-v18-window.xml';
...
function parseMediaStoreRows(raw) {
  ...
  for(const line of String(raw||'').split(/\r?\n/)){
    ...
    const sizem=line.match(/(?:^|[\s,])size=(\d+)/i);
    if(!idm || !namem) continue;
    rows.push({line,id:Number(idm[1]),name:namem[1].trim(),dateAdded:datem?Number(datem[1]):0,size:sizem?Number(sizem[1]):null});
  }
  return rows;
}
function queryImageMediaStore() {
  return adbText('shell','content','query','--uri','content://media/external/images/media','--projection','_id:_display_name:date_added:size');
}
function assertMediaStoreTargetNewest(filename, attemptIndex, label='check') {
  const raw=queryImageMediaStore();
  const rows=parseMediaStoreRows(raw);
  const targetRows=rows.filter(r=>r.name.toLowerCase()===String(filename).toLowerCase());
  if(!targetRows.length){
    write(`attempt-${String(attemptIndex).padStart(2,'0')}-mediastore-${label}.txt`,raw);
    throw new Error(`PHOTO_PICKER_MEDIASTORE_TARGET_MISSING ${filename}`);
  }
  const sorted=[...rows].sort((a,b)=>(b.dateAdded-a.dateAdded)||(b.id-a.id));
  const target=[...targetRows].sort((a,b)=>(b.dateAdded-a.dateAdded)||(b.id-a.id))[0];
  const newest=sorted[0]||null;
  const proof={label,filename,target,newest,rowCount:rows.length,pass:!!newest&&newest.id===target.id};
...
async function findAndTapFile(filename, attemptIndex, page=null, timeoutMs=45000) {
  ...
  while(Date.now()-started<timeoutMs){
    ...
    if((snap.packages||[]).includes('com.google.android.photopicker') ||
      ...
      const mediaProof=assertMediaStoreTargetNewest(filename,attemptIndex,'picker-pre-tap');
      const mediaHit=mediaProof.target.line;
      ...
      // Photo Picker sorts Photos by newest first. With V18 only one canonical copy is created,
...
function makeTestPng(localPath, targetBytes=512*1024) {
  // Valid 1x1 PNG with a large ancillary tEXt chunk. This exercises provider/file
  // transfer size without requiring a huge pixel surface or excessive decode memory.
  const base=Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9ZQmcAAAAASUVORK5CYII=','base64');
  const iend=base.subarray(base.length-12);
  const prefix=base.subarray(0,base.length-12);
  const fixed=prefix.length+iend.length+12;
  const dataLen=Math.max(8,targetBytes-fixed);
  const textData=Buffer.alloc(dataLen,0x41); Buffer.from('tool001\0','ascii').copy(textData,0);
  const text=pngChunk('tEXt',textData);
  const buf=Buffer.concat([prefix,text,iend]);
...
async function prepareDeviceFiles(count) {
  ...
  for(let i=1;i<=count;i++){
    ...
    const name=`TOOL001_V18_${safeMb}MB_${token}_${String(i).padStart(2,'0')}.png`;
...
async function stageDeviceFile(tf, attemptIndex) {
  ...
  let mediaRaw='', media='', indexed=false, proof=null;
  const started=Date.now();
  while(Date.now()-started<10000){
    await sleep(500);
    mediaRaw=queryImageMediaStore();
    const rows=parseMediaStoreRows(mediaRaw);
    const hit=rows.find(row=>row.name.toLowerCase()===tf.name.toLowerCase());
    media=hit?.line||'';
...
${mediaRaw}`);
  ...
  proof=assertMediaStoreTargetNewest(tf.name,attemptIndex,'after-stage');
  log(`[PASS] staged ONE canonical Photo Picker media item ${tf.name} -> ${remote}`);
  log(`[PASS] MediaStore indexed and newest target id=${proof.target.id} ${tf.name}`);
  return {exists,media,mediaRaw,indexed,remote,targetId:proof.target.id};
}
...
async function runSelfCheck() {
  log('[SELF-CHECK] V18 validator integrity check starting');
  ...
  write('self-check.txt', [
    'SELF_CHECK=PASS',
    'BUILD=V18_MEDIASTORE_TARGET_LOCK_AND_SIZE_PROOF',
...
function browserInit() {
  const KEY = '__TOOL001_REAL_ANDROID_V18__';
  if (window[KEY]) return;
  const state = window[KEY] = {
    version:'V18', bornEpoch:Date.now(), bornPerf:performance.now(), seq:0,
  ...
  async function observeProduct(file,attemptIndex) {
    ...
    while(Date.now()<deadline){
      ...
      if(cardAdded && previewAdded){ r.productCapturePass=true; r.reason='card-and-preview'; r.domAfter=now; break; }
...
async function waitForAttempt(page, expected, timeout, expectedBytes=null) {
  ...
  while(Date.now()-start<timeout){
    const s=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V18__?({count:window.__TOOL001_REAL_ANDROID_V18__.attempts.length,attempts:window.__TOOL001_REAL_ANDROID_V18__.attempts.map(a=>({attemptIndex:a.attemptIndex,done:a.done,fileCount:a.fileCount,fileSize:a.files?.[0]?.size??null,fileName:a.files?.[0]?.name??null})),activeProbes:window.__TOOL001_REAL_ANDROID_V18__.activeProbes}):({count:0,attempts:[],activeProbes:0})).catch(()=>({count:0,attempts:[],activeProbes:0}));
...
async function main(){
  log('=== TOOL001 REAL ANDROID DEEP DIAGNOSTIC V18 ==='); log('[OUTPUT]',outDir);
  step('검수기 SELF-CHECK');
  ...
  await page.evaluate(sel=>{if(window.__TOOL001_REAL_ANDROID_V18__)window.__TOOL001_REAL_ANDROID_V18__.selector=sel;},selector);
  ...
  for(let i=1;i<=attemptsTarget;i++){
    ...
    try{
      ...
      await withHeartbeat(`attempt ${i}: stage fresh PNG on Galaxy`,()=>stageDeviceFile(tf,i),25000);
      await withHeartbeat(`attempt ${i}: prove target remains newest before picker`,()=>Promise.resolve(assertMediaStoreTargetNewest(tf.name,i,'pre-activation')),10000);
      ...
      await withHeartbeat(`attempt ${i}: wait trusted change + non-invasive product capture`,()=>waitForAttempt(page,i,Math.min(waitChangeMs,45000),tf.testBytes),Math.min(waitChangeMs,45000)+10000,5000);
      write(`attempt-${String(i).padStart(2,'0')}-pipeline-stage.json`,{attempt:i,stage:'PRODUCT_CAPTURE_OBSERVED',pass:true,at:new Date().toISOString()});
      log(`[ATTEMPT ${i}] NATIVE_PICKER + change + product capture observation COMPLETE`);
      ...
      const p=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V18__)));
      ...
      const p=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V18__?JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V18__)):null).catch(()=>null);
  ...
  const state=await withHeartbeat('collect browser diagnostic state',()=>page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V18__))),20000);
  ...
  write('result.json',{meta:{version:'V18',generatedAt:new Date().toISOString(),url,selector,attemptsTarget,sizeSweepMb,attemptsObserved:state.attempts.length,node:process.version,hostPlatform:process.platform},environment:state.environment,attempts:state.attempts,timeline:state.timeline,domFinal});
  write('timeline.json',state.timeline);write('attempts.json',state.attempts);write('dom-final.json',domFinal);
  const s=['TOOL001_REAL_ANDROID_DEEP_DIAGNOSTIC_V18',`URL=${url}`,`ATTEMPTS_TARGET=${attemptsTarget}`,`SIZE_SWEEP_MB=${sizeSweepMb.join(',')}`,`ATTEMPTS_OBSERVED=${state.attempts.length}`,`USER_AGENT=${state.environment?.userAgent||''}`,`PLATFORM=${state.environment?.platform||''}`,`MAX_TOUCH_POINTS=${state.environment?.maxTouchPoints??''}`];
  for(const a of state.attempts){const p=a.probes?.[0];s.push('',`[ATTEMPT_${a.attemptIndex}]`,`EXPECTED_SIZE_MB=${sizeSweepMb[a.attemptIndex-1]??''}`,`EXPECTED_FILE_SIZE=${deviceFiles?.[a.attemptIndex-1]?.testBytes??''}`,`TRUSTED_CHANGE=${a.trusted}`,`FILE_COUNT=${a.fileCount}`,`FILE_NAME=${p?.file?.name||a.files?.[0]?.name||''}`,`FILE_TYPE=${p?.file?.type||a.files?.[0]?.type||''}`,`FILE_SIZE=${p?.file?.size??a.files?.[0]?.size??''}`,`ORIGINAL_FILE_PROBE=SKIPPED_NON_INVASIVE`,`PRODUCT_CAPTURE_PASS=${p?.productCapturePass??''}`,`PRODUCT_CAPTURE_REASON=${p?.reason??''}`,`CARD_COUNT_BEFORE=${p?.domBefore?.cardCount??''}`,`CARD_COUNT_AFTER=${p?.domAfter?.cardCount??''}`,`PREVIEW_COUNT_AFTER=${p?.domAfter?.cardLoadedPreviewCount??''}`,`NON_INVASIVE=${p?.nonInvasive??''}`,`DOM_ERRORS=${(p?.domAfter?.matchedErrors||[]).join(',')}`,`OBSERVE_MS=${p?.totalMs??''}`,`ORIGINAL_FILE_READ_ATTEMPTED=${p?.originalFileReadAttempted??''}`,`DOM_BEFORE_LOADED_IMAGES=${p?.domBefore?.loadedImageCount??''}`,`DOM_AFTER_LOADED_IMAGES=${p?.domAfter?.loadedImageCount??''}`,`DOM_AFTER_BLOB_IMAGES=${p?.domAfter?.blobImageCount??''}`,`DOM_AFTER_CANVAS_COUNT=${p?.domAfter?.canvases?.length??''}`,`DOM_ERROR_MATCHES=${(p?.domAfter?.matchedErrors||[]).join(',')}`);}
  const attemptVerdicts=state.attempts.map(a=>{
    const p=a.probes?.[0];
    const pass=!!(a.done && a.trusted && p?.nonInvasive && p?.originalFileReadAttempted===false && p?.productCapturePass);
    return {attemptIndex:a.attemptIndex,pass,trusted:!!a.trusted,fileCount:a.fileCount,nonInvasive:!!p?.nonInvasive,productCapturePass:!!p?.productCapturePass,reason:p?.reason||null};
  });
...
main().catch(async e=>{
  ...
  try{adb('shell','rm','-f','/sdcard/Download/TOOL001_V18_*.png');adb('shell','rm','-f','/sdcard/Pictures/TOOL001_V18_*.png');adb('shell','rm','-f','/sdcard/tool001-v17-screen.png');adb('shell','rm','-f','/sdcard/tool001-v18-screen.png');}catch{}
