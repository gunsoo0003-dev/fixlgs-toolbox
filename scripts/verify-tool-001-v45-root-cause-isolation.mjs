...
const runner=path.join(here,'run-tool-001-v45-root-cause-isolation.mjs');
const s=fs.readFileSync(runner,'utf8');
const kt=fs.readFileSync(path.join(here,'tool-001-v45-native-template','MainActivity.kt'),'utf8');
...
check('native-task-reset-standard-flags',/'-W','-S','-f','0x10008000'/.test(s) && !/--activity-new-task|--activity-clear-task/.test(s));
...
check('passive-system-evidence',/uriPermissionWmLockErrors/.test(s) && /mediaProviderLowerFsOpens/.test(s) && /chromeNoPersistedWritePermission/.test(s));
check('diagnostic-apk-auto-remove-after-zip',/await makeZip\(\);\s*cleanupNativeApk\(\);/.test(s) && /adb\('uninstall',pkg\)/.test(s));
