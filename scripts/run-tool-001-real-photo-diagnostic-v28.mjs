...
const outDir=path.join(desktop,`TOOLBOX_001_REAL_PHOTO_V28_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function dumpUi(){const remote='/sdcard/tool001-v28-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
async function autoCommitManualSelection(caseNo,page,startChange){const deadline=Date.now()+90000;let sawSelection=false;let commitTaps=0;let lastXml='';while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__)));if(st.changes.length>startChange)return{committed:true,mode:'picker-auto-close-change',sawSelection,commitTaps};let open=false;try{lastXml=dumpUi();open=pickerDetectedFromXml(lastXml);}catch{}if(!open){await sleep(250);const st2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__)));if(st2.changes.length>startChange)return{committed:true,mode:'picker-closed-change',sawSelection,commitTaps};await sleep(500);continue;}const sig=pickerSelectionSignals(lastXml);if(sig.detected){sawSelection=true;write(`case-${caseNo}-selection-observed.xml`,lastXml);const actions=photoPickerActionControls(lastXml);write(`case-${caseNo}-commit-controls.json`,actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds})));if(actions.length){for(const action of actions){const b=action.bounds;adb('shell','input','tap',String(b.x),String(b.y));commitTaps++;log(`[PHOTO_PICKER] case ${caseNo}: auto commit "${action.exact}" at ${b.x},${b.y}`);await pcScreen(`case-${caseNo}-after-auto-commit-${commitTaps}.png`);if(await pickerClosedStable(page,7000)){const st3=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__)));if(st3.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit',sawSelection:true,commitTaps,action:action.exact};await sleep(1500);const st4=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__)));if(st4.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit-delayed-change',sawSelection:true,commitTaps,action:action.exact};return{committed:false,mode:'picker-closed-no-change',sawSelection:true,commitTaps,action:action.exact};}}}}
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V28__'; if(window[KEY])return;
...
async function ensureChromeInteractive(page,caseNo,tag='guard'){
  ...
  // V28: NEVER infer overflow menu from toolbar "새 탭".
...
async function waitForSelectionAndCommit(caseNo,label,page,startChange,context){
  ...
  async function waitChange(ms){
    ...
    while(Date.now()<dl){
      const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__)));
  ...
  async function currentObserver(){
    return page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__))).catch(()=>null);
  }

  for(let round=1;round<=3;round++){
    await ensureExpectedToolPage(page,caseNo,`native-before-activation-r${round}`);
    ...
    session.push({
      round,
      activation:{method:activation.method,strategies:activation.strategies},
      mode:'NATIVE_USER_COMMIT'
    });
    write(`case-${caseNo}-activation-session.json`,session);

    log(`[PICKER READY] CASE ${caseNo} ${label}: 실제 Android Photo Picker 확인.`);
    log(`[ACTION REQUIRED] ${caseInstruction(label)}`);
    log(`[ACTION REQUIRED] 사진 1장을 탭하세요. Picker가 그대로 열려 있으면 화면의 완료/추가/선택 버튼을 직접 1회 누르세요.`);
    log(`[ACTION REQUIRED] 사진 선택 뒤 자동으로 Chrome으로 돌아오면 추가로 누르지 마세요.`);

    const deadline=Date.now()+45000;
    let sawSelection=false;
    let selectionAt=0;
    let manualCommitReminderShown=false;
    ...
    while(Date.now()<deadline){
      const now=Date.now();
      const observer=await currentObserver();
      if(observer?.changes?.length>startChange){
        const st=await pageState(`native-change-r${round}`);
        timeline.push({at:now,event:'change',pageState:st});
        write(`case-${caseNo}-native-picker-timeline-r${round}.json`,timeline);
        return{
          committed:true,
          mode:'NATIVE_USER_FLOW_CHANGE',
          sawSelection,
          commitTaps:0,
          userCommit:true,
          round,
      ...
      const sig=pickerOpen ? pickerSelectionSignals(lastXml) : {selectedNodes:[],actionControls:[],selectedByText:false,detected:false};
      const actions=pickerOpen ? photoPickerActionControls(lastXml) : [];
      ...
      timeline.push({
        ...
        commitControls:actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds}))
      });
      ...
      if(pickerOpen && sig.detected && !sawSelection){
        sawSelection=true;
        selectionAt=now;
        write(`case-${caseNo}-native-selection-observed-r${round}.xml`,lastXml);
        write(`case-${caseNo}-native-selection-controls-r${round}.json`,actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds})));
        log(`[SELECTION] CASE ${caseNo}: 사진 선택 상태 감지. 검수기는 Picker를 터치하지 않습니다.`);
      }

      // Natural-user flow: if picker remains open after a selection, explicitly ask the user
      // to press the OS-provided positive action. The harness never taps it.
      if(sawSelection && pickerOpen && !manualCommitReminderShown && now-selectionAt>=2500){
        manualCommitReminderShown=true;
        log(`[ACTION REQUIRED] CASE ${caseNo}: Picker가 아직 열려 있습니다. 화면에 완료/추가/선택 버튼이 보이면 직접 1회 누르세요.`);
        await pcScreen(`case-${caseNo}-native-user-commit-reminder-r${round}.png`);
        write(`case-${caseNo}-native-user-commit-reminder-r${round}.xml`,lastXml);
      }

      if(lastPickerOpen && !pickerOpen){
        const closedState=await pageState(`native-picker-closed-r${round}`);
        timeline.push({at:Date.now(),event:'picker-closed',pageState:closedState});
        log(`[PICKER CLOSED] CASE ${caseNo}: Chrome 반환 감시 시작.`);
        const ch=await waitChange(8000);
        if(ch){
          write(`case-${caseNo}-native-picker-timeline-r${round}.json`,timeline);
          return{
            committed:true,
            mode:'NATIVE_USER_FLOW_PICKER_CLOSED_CHANGE',
            sawSelection,
            commitTaps:0,
            userCommit:true,
            round,
        ...
        const after=await pageState(`native-picker-closed-no-change-r${round}`);
        const obs=await currentObserver();
        write(`case-${caseNo}-native-picker-timeline-r${round}.json`,timeline);
        write(`case-${caseNo}-native-picker-closed-no-change-r${round}.json`,{
          sawSelection,
          after,
          observer:obs,
      ...
      // If picker vanished before a selection was ever observed, retry activation.
      if(!pickerOpen && !sawSelection){
        const ch=await waitChange(1500);
        if(ch){
          write(`case-${caseNo}-native-picker-timeline-r${round}.json`,timeline);
          return{
            committed:true,
            mode:'NATIVE_AUTO_RETURN_CHANGE',
            sawSelection:false,
            commitTaps:0,
            userCommit:true,
            round,
        ...
        session.push({round,pickerLostBeforeSelection:true});
        write(`case-${caseNo}-activation-session.json`,session);
        break;
      }
    ...
    write(`case-${caseNo}-native-picker-timeline-r${round}.json`,timeline);
    if(sawSelection){
      await pcScreen(`case-${caseNo}-native-user-flow-timeout-r${round}.png`);
      write(`case-${caseNo}-native-user-flow-timeout-r${round}.xml`,lastXml);
      return{
        committed:false,
        mode:'NATIVE_USER_COMMIT_TIMEOUT',
        sawSelection:true,
        commitTaps:0,
        userCommit:true,
        round,
...
async function waitProduct(page,startCapture,before,timeoutMs=90000){const deadline=Date.now()+timeoutMs;let snap=before,cap=[];while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__)));cap=st.capture.slice(startCapture);snap=await domSnapshot(page);const terminal=cap.some(x=>x.phase==='picker-file-pass'||x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail');const cardChanged=snap.cardCount!==before.cardCount;if(terminal&&(cardChanged||cap.some(x=>x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail')))break;await sleep(350);}return{capture:cap,after:snap};}
async function collectFailureEvidence(caseNo,label,page,reason){let xml='';try{xml=dumpUi();}catch{}write(`case-${caseNo}-${label}-failure-picker.xml`,xml);await pcScreen(`case-${caseNo}-${label}-failure-device.png`);await page.screenshot({path:path.join(outDir,`case-${caseNo}-${label}-failure-page.png`),fullPage:true}).catch(()=>{});const snap=await domSnapshot(page).catch(()=>null);const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__))).catch(()=>null);write(`case-${caseNo}-${label}-failure-evidence.json`,{reason,pickerOpen:pickerDetectedFromXml(xml),selectionSignals:pickerSelectionSignals(xml).selectedNodes.length,commitControls:photoPickerActionControls(xml).map(a=>({exact:a.exact,label:a.label,bounds:a.bounds})),foreground:foregroundSnapshot(),observer:obs,dom:snap});}
...
function classifyStage(r){
  ...
  if(r.commit?.mode==='NATIVE_USER_COMMIT_TIMEOUT')return 'PICKER_NATIVE_COMMIT_TIMEOUT';
...
async function main(){
  ...
  step('실사진 비교검수 안내');log('');log('*** 검수기가 테스트 사진을 만들지 않습니다. ***');log('각 CASE에서 안내된 GOOD/BAD 실제 사진을 Galaxy Photo Picker에서 1장만 탭하세요.');log('검수기는 Android Photo Picker를 자동 조작하지 않습니다. 사진 선택 후 Picker가 그대로면 완료/추가/선택 버튼을 직접 1회 누르세요.');log('8개 케이스로 반복성·출처·포맷·provider·result-return·제품검증·preview를 넓게 탐색합니다.');log('');
  const results=[];
  for(let i=0;i<labels.length;i++){
    ...
    await ensureExpectedToolPage(page,caseNo,'case-start');await page.bringToFront();const before=await domSnapshot(page);const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__)));let result={case:caseNo,label,pass:false,error:null,before,commit:null};
    try{
      const commit=await timed(`CASE ${caseNo} verified picker + native user selection/commit`,()=>waitForSelectionAndCommit(caseNo,label,page,st0.changes.length,context),65000);result.commit=commit;result.activation=commit.activation||null;
      if(!commit.committed)throw new Error(`PHOTO_PICKER_COMMIT_FAILED:${commit.mode}`);
      const changeDeadline=Date.now()+8000;let change=null;while(Date.now()<changeDeadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V28__)));if(st.changes.length>st0.changes.length){change=st.changes.at(-1);break;}await sleep(250);}if(!change)throw new Error('NO_FILE_CHANGE_AFTER_VERIFIED_COMMIT');
  ...
  const summary=['TOOL001_REAL_USER_PHOTO_DIAGNOSTIC_V28_NATIVE_USER_FLOW_BROAD_CAUSE_SWEEP',`URL=${url}`,`CASES=${results.length}`,`GOOD_BAD_FIRST_DIVERGENCE=${divergence}`,`TIMING_RACE_EVIDENCE=${timingRace}`,`REPEAT_EVIDENCE=${JSON.stringify(repeatRace)}`];
  ...
  write('summary.txt',summary.join('\n'));write('root-cause-summary.json',{goodBadFirstDivergence:divergence,timingRaceEvidence:timingRace,repeatEvidence:repeatRace,stageCounts:Object.fromEntries([...new Set(results.map(r=>r.rootCauseStage))].map(k=>[k,results.filter(r=>r.rootCauseStage===k).length]))});write('result.json',{version:'V28',url,generatedAt:new Date().toISOString(),uiLimits:{has20MB:ui20,has60MB:ui60},goodBadFirstDivergence:divergence,timingRaceEvidence:timingRace,repeatEvidence:repeatRace,results});
