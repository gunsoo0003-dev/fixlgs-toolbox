...
const page=fs.readFileSync('components/tool-079-dividend-yield-calculator-page.tsx','utf8');
const css=fs.readFileSync('components/tool-079-dividend-yield-calculator.module.css','utf8');
const seq=[
...
check('ToolNavigation current tool',page.includes('ToolNavigation')&&page.includes('currentTool={79}'));
check('expert shared wide head',page.includes('toolbox-tool-expert-post--wide-head'));
const expertTags=["YIELD", "DIVIDEND", "PRICE", "ANNUAL", "PRE-TAX", "SCENARIO"];
...
check('actual tool number',page.includes('079'));
...
console.log(`RESULT TOOL079 DESIGN PASS=${pass} FAIL=${fail}`);
