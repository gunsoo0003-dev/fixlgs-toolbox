...
const definitions = [
  ['validator', ['run', 'check:tool015-validator']],
  ['source', ['run', 'check:tool015-source']],
  ['ja-terms', ['run', 'check:ja-terms']],
  ['build', ['run', 'build']],
  ['common', ['run', 'test:toolbox:common-additive']],
  ['core', ['run', 'test:toolbox:015-core-only']],
  ['boundary', ['run', 'test:toolbox:015-boundary-only']],
  ['regression', ['run', 'test:toolbox:015-regression-only']],
  ['service-limit', ['run', 'test:toolbox:015-limit-only']],
];
...
const pkg = createValidationResultPackage({
  ...
  steps,
  extraFiles: [{
    source: 'test-results/tool-015-limit-report.json',
    destination: 'limit/tool-015-limit-report.json',
  }],
});
let finalZipPath = pkg.zipPath;
if (pkg.zipCreated) {
  finalZipPath = resolve(dirname(pkg.zipPath), '015_final_검수결과.zip');
  rmSync(finalZipPath, { force: true });
  renameSync(pkg.zipPath, finalZipPath);
}
cleanupProjectValidationArtifacts();
console.log(`015 FINAL RESULT: ${status.toUpperCase()}`);
