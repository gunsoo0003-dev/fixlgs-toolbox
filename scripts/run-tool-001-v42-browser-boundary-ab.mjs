...
const chromeRows=await runChromePrecision(chrome);const altRows=await runUiBrowser(alternate);const sum=(rows)=>({total:rows.length,pass:rows.filter(x=>x.pass).length,fail:rows.filter(x=>!x.pass).length,notReadable:rows.reduce((a,x)=>a+(x.notReadable||0),0)});const cs=sum(chromeRows),as=sum(altRows);let interpretation='INCONCLUSIVE';if(cs.pass<cs.total&&as.pass===as.total)interpretation='CHROME_SPECIFIC_STRONGLY_SUSPECTED';else if(cs.pass<cs.total&&as.pass<as.total)interpretation='ANDROID_PROVIDER_OR_SHARED_PLATFORM_BOUNDARY_STRONGLY_SUSPECTED';else if(cs.pass===cs.total&&as.pass<as.total)interpretation='ALTERNATE_BROWSER_SPECIFIC_OR_UI_HARNESS_LIMIT';else if(cs.pass===cs.total&&as.pass===as.total)interpretation='NO_FAILURE_REPRODUCED_THIS_RUN';const txt=[`TOOL001 V42 BROWSER BOUNDARY A/B`,`URL=${url}`,`REPEATS=${repeats}`,`PRIMARY=Chrome ${browserVersion(chrome.pkg)}`,`ALTERNATE=${alternate.label} ${browserVersion(alternate.pkg)}`,'',`CHROME PASS=${cs.pass}/${cs.total} FAIL=${cs.fail} NOTREADABLE=${cs.notReadable}`,`${alternate.id} PASS=${as.pass}/${as.total} FAIL=${as.fail} NOTREADABLE=${as.notReadable}`,'',`INTERPRETATION=${interpretation}`,'',`RULE_1=Chrome fails + alternate all-pass => Chrome-specific path suspected`,`RULE_2=Both fail randomly => Android/provider/shared file-access boundary suspected`,`RULE_3=Harness never retries upload/photo coordinates. One round = one upload tap + one PHOTO01 tap + at most one commit tap.`,`RULE_4=User-visible preview is ground truth; HARNESS_FAIL is never product FAIL.`];write('v42-browser-boundary-summary.txt',txt.join('\n'));write('result.json',{version:'V42R4-BROWSER-BOUNDARY-AB-V39-SOURCE-EXACT',chrome:cs,alternate:{browser:alternate,...as},interpretation,chromeRows,altRows});log(txt.join('\n'));
