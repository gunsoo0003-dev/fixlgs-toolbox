...
const outDir=path.join(desktop,`TOOLBOX_001_REAL_PHOTO_V26_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function dumpUi(){const remote='/sdcard/tool001-v26-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
async function autoCommitManualSelection(caseNo,page,startChange){const deadline=Date.now()+90000;let sawSelection=false;let commitTaps=0;let lastXml='';while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__)));if(st.changes.length>startChange)return{committed:true,mode:'picker-auto-close-change',sawSelection,commitTaps};let open=false;try{lastXml=dumpUi();open=pickerDetectedFromXml(lastXml);}catch{}if(!open){await sleep(250);const st2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__)));if(st2.changes.length>startChange)return{committed:true,mode:'picker-closed-change',sawSelection,commitTaps};await sleep(500);continue;}const sig=pickerSelectionSignals(lastXml);if(sig.detected){sawSelection=true;write(`case-${caseNo}-selection-observed.xml`,lastXml);const actions=photoPickerActionControls(lastXml);write(`case-${caseNo}-commit-controls.json`,actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds})));if(actions.length){for(const action of actions){const b=action.bounds;adb('shell','input','tap',String(b.x),String(b.y));commitTaps++;log(`[PHOTO_PICKER] case ${caseNo}: auto commit "${action.exact}" at ${b.x},${b.y}`);await pcScreen(`case-${caseNo}-after-auto-commit-${commitTaps}.png`);if(await pickerClosedStable(page,7000)){const st3=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__)));if(st3.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit',sawSelection:true,commitTaps,action:action.exact};await sleep(1500);const st4=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__)));if(st4.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit-delayed-change',sawSelection:true,commitTaps,action:action.exact};return{committed:false,mode:'picker-closed-no-change',sawSelection:true,commitTaps,action:action.exact};}}}}
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V26__'; if(window[KEY])return;
...
async function ensureChromeInteractive(page,caseNo,tag='guard'){
  ...
  // V25 BUGFIX: KEYCODE_MENU opens Chrome's overflow menu on Galaxy/Chrome.
  // Never send KEYCODE_MENU as a generic "make interactive" action.
  await page.bringToFront().catch(()=>{});
  let xml='';try{xml=dumpUi();}catch{}
  const menuOpen=/(새 탭|새 시크릿 탭|방문 기록|New tab|New Incognito tab|History)/i.test(xml);
  if(menuOpen){
    log(`[RECOVER] CASE ${caseNo}: Chrome overflow menu detected; closing with BACK`);
  ...
  write(`case-${caseNo}-${tag}-foreground.json`,{fg,ping,chromeOverflowMenuRecovered:menuOpen});
  if(ping?.href)return{fg,ping,chromeOverflowMenuRecovered:menuOpen};
...
async function waitForSelectionAndCommit(caseNo,label,page,startChange,context){
  ...
  async function waitChange(ms){
    const dl=Date.now()+ms;while(Date.now()<dl){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__)));if(st.changes.length>startChange)return st.changes.at(-1);await sleep(200);}return null;
  }
  for(let round=1;round<=3;round++){
    ...
    log(`[ACTION REQUIRED] ${caseInstruction(label)} 완료 버튼은 누르지 마세요. V26가 자동 처리합니다.`);

    const selectionDeadline=Date.now()+30000;let sawSelection=false,commitTaps=0,lastXml='',selectionAt=0;
    while(Date.now()<selectionDeadline){
      const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__)));if(st.changes.length>startChange)return{committed:true,mode:'picker-auto-close-change',sawSelection,commitTaps,round,activation};
...
async function waitProduct(page,startCapture,before,timeoutMs=90000){const deadline=Date.now()+timeoutMs;let snap=before,cap=[];while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__)));cap=st.capture.slice(startCapture);snap=await domSnapshot(page);const terminal=cap.some(x=>x.phase==='picker-file-pass'||x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail');const cardChanged=snap.cardCount!==before.cardCount;if(terminal&&(cardChanged||cap.some(x=>x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail')))break;await sleep(350);}return{capture:cap,after:snap};}
async function collectFailureEvidence(caseNo,label,page,reason){let xml='';try{xml=dumpUi();}catch{}write(`case-${caseNo}-${label}-failure-picker.xml`,xml);await pcScreen(`case-${caseNo}-${label}-failure-device.png`);await page.screenshot({path:path.join(outDir,`case-${caseNo}-${label}-failure-page.png`),fullPage:true}).catch(()=>{});const snap=await domSnapshot(page).catch(()=>null);const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__))).catch(()=>null);write(`case-${caseNo}-${label}-failure-evidence.json`,{reason,pickerOpen:pickerDetectedFromXml(xml),selectionSignals:pickerSelectionSignals(xml).selectedNodes.length,commitControls:photoPickerActionControls(xml).map(a=>({exact:a.exact,label:a.label,bounds:a.bounds})),foreground:foregroundSnapshot(),observer:obs,dom:snap});}
...
async function main(){
  log('=== TOOL001 REAL USER PHOTO DIAGNOSTIC V26 FINAL-INTEGRATED ===');log('[OUTPUT]',outDir);
  ...
  step('실사진 비교검수 안내');log('');log('*** 검수기가 테스트 사진을 만들지 않습니다. ***');log('각 CASE에서 안내된 GOOD/BAD 실제 사진을 Galaxy Photo Picker에서 1장만 탭하세요.');log('완료 버튼은 누르지 않아도 됩니다. V26이 V19 검증 로직으로 자동 commit합니다.');log('8개 케이스로 반복성·출처·포맷·provider·result-return·제품검증·preview를 넓게 탐색합니다.');log('');
  const results=[];
  for(let i=0;i<labels.length;i++){
    ...
    await page.bringToFront();const before=await domSnapshot(page);const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__)));let result={case:caseNo,label,pass:false,error:null,before,commit:null};
    try{
      ...
      const changeDeadline=Date.now()+8000;let change=null;while(Date.now()<changeDeadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V26__)));if(st.changes.length>st0.changes.length){change=st.changes.at(-1);break;}await sleep(250);}if(!change)throw new Error('NO_FILE_CHANGE_AFTER_VERIFIED_COMMIT');
  ...
  const summary=['TOOL001_REAL_USER_PHOTO_DIAGNOSTIC_V26_BROAD_CAUSE_SWEEP',`URL=${url}`,`CASES=${results.length}`,`GOOD_BAD_FIRST_DIVERGENCE=${divergence}`,`TIMING_RACE_EVIDENCE=${timingRace}`,`REPEAT_EVIDENCE=${JSON.stringify(repeatRace)}`];
  ...
  write('summary.txt',summary.join('\n'));write('root-cause-summary.json',{goodBadFirstDivergence:divergence,timingRaceEvidence:timingRace,repeatEvidence:repeatRace,stageCounts:Object.fromEntries([...new Set(results.map(r=>r.rootCauseStage))].map(k=>[k,results.filter(r=>r.rootCauseStage===k).length]))});write('result.json',{version:'V26',url,generatedAt:new Date().toISOString(),uiLimits:{has20MB:ui20,has60MB:ui60},goodBadFirstDivergence:divergence,timingRaceEvidence:timingRace,repeatEvidence:repeatRace,results});
