...
const allowed=new Set(["preflight", "core-only", "feature-only", "boundary-only", "regression-only", "limit-only", "final"]);
if(!allowed.has(mode)){console.error(`UNKNOWN_MODE=${mode}`);process.exit(2)}
const isWin=process.platform==='win32';
const resultDir=process.env.TOOL052_RESULT_DIR?path.resolve(process.env.TOOL052_RESULT_DIR):path.join(process.env.USERPROFILE||os.homedir(),'Desktop');
fs.mkdirSync(resultDir,{recursive:true});
const stamp=new Date().toISOString().replace(/[:.]/g,'-');
const txt=path.join(resultDir,`052_${mode}_검수결과_${stamp}.txt`);
const rows=[];
...
function pw(spec){return isWin?run(spec,`npx playwright test tests/tool-052-${spec}.spec.ts --workers=1 --config=playwright.tool052.config.ts --reporter=list`,[],{shell:true}):run(spec,'npx',['playwright','test',`tests/tool-052-${spec}.spec.ts`,'--workers=1','--config=playwright.tool052.config.ts','--reporter=list'])}
const specs=["preflight", "core", "feature", "boundary", "regression", "limit"];
if(mode==='preflight'){run('static-self-check',process.execPath,['scripts/tool-052/run-static-validation.mjs']);if(deps())pw('preflight')}
else if(mode.endsWith('-only')){const s=mode.slice(0,-5);if(deps())pw(s)}
else{run('secret-scan',process.execPath,['scripts/tool-052/check-secret-scan.mjs']);run('static-self-check',process.execPath,['scripts/tool-052/run-static-validation.mjs']);if(deps()){for(const s of specs)pw(s);if(isWin){run('typescript','npx tsc --noEmit',[],{shell:true});run('production-build','npm run build',[],{shell:true})}else{run('typescript','npx',['tsc','--noEmit']);run('production-build','npm',['run','build'])}}else{for(const x of ['playwright stages','typescript','production-build'])rows.push({name:x,exit:0,kind:'skip',output:'SKIP_ENVIRONMENT dependency preflight failed'})}}
