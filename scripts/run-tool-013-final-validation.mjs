...
const definitions = [
  ['validator', ['run', 'check:tool013-validator']],
  ['source', ['run', 'check:tool013-source']],
  ['ja-terms', ['run', 'check:ja-terms']],
  ['build', ['run', 'build']],
  ['common', ['run', 'test:toolbox:common-additive']],
  ['core', ['run', 'test:toolbox:013-core']],
  ['boundary', ['run', 'test:toolbox:013-boundary']],
  ['regression', ['run', 'test:toolbox:013-regression']],
  ['service-limit', ['exec', 'playwright', 'test', 'tests/tool-013-limit.spec.ts', '--workers=1', '--project=desktop-chromium']],
];
...
const pkg = createValidationResultPackage({
  ...
  steps,
  extraFiles: [{
    source: 'test-results/tool-013-limit-report.json',
    destination: 'limit/tool-013-limit-report.json',
  }],
});
cleanupProjectValidationArtifacts();
console.log(`013 FINAL RESULT: ${status.toUpperCase()}`);
