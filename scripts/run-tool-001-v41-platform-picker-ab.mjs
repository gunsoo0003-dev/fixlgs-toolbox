...
const outDir=path.join(desktop,`TOOLBOX_001_V41_PLATFORM_PICKER_AB_REAL_DEVICE_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function pickerMediaCandidates(xml){
  ...
  const walk=n=>{
    ...
    const blocked=/(드래그 핸들|drag handle|더보기|more|컬렉션|collection|완료|done|추가|add|취소|cancel|뒤로|back|메뉴|menu|최근|recent|downloads?|다운로드)/i.test(desc);
    const mediaLike=/(촬영한 사진|촬영한 동영상|photo|video|image|사진|동영상|\.(?:jpe?g|png|webp)\b)/i.test(desc);
    const photoPickerCell=pkg==='com.google.android.photopicker' && a.clickable==='true' && b && b.top>=700 && b.bottom<=2400 && b.w>=220 && b.h>=220 && !blocked && mediaLike;
    const documentCell=/(documentsui|myfiles|filepicker)/i.test(pkg) && a.clickable==='true' && b && b.top>=250 && b.bottom<=2400 && b.w>=120 && b.h>=80 && !blocked && (mediaLike || /thumbnail|grid|item|document/i.test(`${a['resource-id']||''} ${a.class||''}`));
    if((photoPickerCell||documentCell)&&!seen.has(a.bounds)){
      seen.add(a.bounds);out.push({bounds:a.bounds,desc,class:a.class||'',package:pkg,parsed:b,kind:photoPickerCell?'photo':'document'});
    }
    for(const c of n.children||[])walk(c);
  };
  walk(tree);
  out.sort((x,y)=>{if(x.kind!==y.kind)return x.kind==='photo'?-1:1;const xm=/\.(?:jpe?g|png|webp)\b/i.test(x.desc)?0:1,ym=/\.(?:jpe?g|png|webp)\b/i.test(y.desc)?0:1;if(xm!==ym)return xm-ym;return x.parsed.top-y.parsed.top || x.parsed.left-y.parsed.left;});
  return out;
}
...
async function autoSelectPickerMediaV38(caseNo,label,round,page){
  ...
  if(b.top<200||b.w<100||b.h<70)throw new Error('HARNESS_UNSAFE_MEDIA_BOUNDS');
  adb('shell','input','tap',String(b.x),String(b.y));
  log(`[PHOTO TAP] CASE ${caseNo}: PHOTO_01 -> ${c.kind}/${c.package||'-'} ${c.bounds}`);
...
async function main(){
  ...
  const MODES=[
    {id:'PATH_A_FS_IMAGE_FILTERED',pickerMode:'FS_IMAGE_FILTERED',label:'File System Access + 이미지 필터'},
    {id:'PATH_B_FS_ACCEPT_ALL',pickerMode:'FS_ACCEPT_ALL',label:'File System Access + accept-all 문서 선택'},
    {id:'PATH_C_LEGACY_IMAGE_INPUT',pickerMode:'LEGACY_IMAGE_INPUT',label:'기존 input[type=file] + 이미지 필터'},
  ];
  const plan=[];for(let round=1;round<=REPEATS;round++)for(const mode of MODES)plan.push({...mode,round});
  log('=== TOOL001 V41 PLATFORM PICKER A/B REAL DEVICE ===');log('[OUTPUT]',outDir);
  step('SELF-CHECK');if(adb('version').status!==0)throw new Error('ADB_NOT_AVAILABLE');if(!/\tdevice\b/.test(adbText('devices')))throw new Error('ADB_DEVICE_NOT_READY');write('self-check.txt','SELF_CHECK=PASS\nMODE=V41_PLATFORM_PICKER_10X_EACH\nTOTAL=30\n');log('[PASS] SELF-CHECK');
  ...
  step('V41 관찰기 주입');await page.addInitScript(browserInit);
  ...
  const support=await page.evaluate(()=>({ua:navigator.userAgent,showOpenFilePicker:typeof window.showOpenFilePicker==='function'}));write('v41-feature-support.json',support);log(`[FEATURE] showOpenFilePicker=${support.showOpenFilePicker}`);if(!support.showOpenFilePicker)throw new Error('V41_STABLE_PICKER_UNSUPPORTED_ON_DEVICE');
  step('플랫폼 파일선택 경로 3종 각 10회 A/B 검수');const results=[];
  for(let i=0;i<plan.length;i++){
    const item=plan[i],caseNo=i+1;log('');log(`===== V41 ${item.id} ROUND ${item.round}/${REPEATS} CASE ${caseNo}/${plan.length} =====`);
    await ensureExpectedToolPage(page,caseNo,'case-start');await page.bringToFront();
    await page.evaluate(mode=>{window.__TOOL001_V40_CAPTURE_MODE__=undefined;window.__TOOL001_V41_PICKER_MODE__=mode;},item.pickerMode);
    ...
    const r={case:caseNo,round:item.round,mode:item.id,pickerMode:item.pickerMode,label:item.label,pass:false,error:'',capture:[],observer:{},summary:null,trace:null,newCards:0,previewCount:0};
    try{
    ...
    r.v41Events=(r.capture||[]).filter(x=>String(x.phase||'').startsWith('v41-')||String(x.phase||'').startsWith('stable-')||String(x.phase||'').startsWith('picker-file-'));
    r.readPass=(r.observer?.nativeReads||[]).some(x=>x.phase==='pass') || (r.capture||[]).some(x=>/read-pass|picker-file-pass/.test(String(x.phase||'')));
    r.getFilePass=(r.observer?.handles||[]).filter(x=>x.phase==='getFile-pass').length;
    r.pickerEnv=pickerSignalSnapshot();
    ...
    const traceLines=[`CASE=${caseNo}`,`MODE=${item.id}`,`ROUND=${item.round}`,`USER_VISIBLE=${r.pass?'SUCCESS':'FAIL'}`,`LAST_SUCCESS_STEP=${r.trace.lastSuccessStep}`,`FIRST_FAIL_STEP=${r.trace.firstFailStep}`,`ERROR_NAME=${r.trace.errorName||''}`,`NOTREADABLE_EVENTS=${r.trace.notReadableEvents}`,`V41_PICKER_MODE=${item.pickerMode}`,`PICKER_PACKAGES=${(r.pickerEnv?.uiPackages||[]).join(',')}`,`RESUMED_PACKAGE=${r.pickerEnv?.resumedPackage||''}`,`GETFILE_PASS_COUNT=${r.getFilePass}`,`READ_PASS=${r.readPass}`,`CARD_ADDED=${r.trace.cardAdded}`,`VISIBLE_PREVIEW=${r.trace.visiblePreview}`];write(`case-${caseNo}-v40-trace.txt`,traceLines.join('\n'));await pcScreen(`case-${caseNo}-device.png`);
    log(`[${r.pass?'PASS':'FAIL'}] ${item.id} R${item.round} NR=${r.trace.notReadableEvents} picker=${(r.pickerEnv?.uiPackages||[]).join(',')||r.pickerEnv?.resumedPackage||'-'} getFilePass=${r.getFilePass} readPass=${r.readPass} card=${r.newCards} preview=${r.previewCount}`);results.push(r);
    if(i<plan.length-1){await page.goto(url,{waitUntil:'domcontentloaded',timeout:45000});await sleep(350);}
  }
  step('플랫폼 파일선택 경로 비교 집계');
  const modeSummary=MODES.map(m=>{const rows=results.filter(r=>r.mode===m.id);const pass=rows.filter(r=>r.pass).length;const fail=rows.length-pass;const nr=rows.reduce((a,r)=>a+(r.trace?.notReadableEvents||0),0);const readPass=rows.filter(r=>r.readPass).length;const pickerPackages=[...new Set(rows.flatMap(r=>r.pickerEnv?.uiPackages||[]))];return{id:m.id,label:m.label,total:rows.length,pass,fail,readPass,notReadableEvents:nr,pickerPackages,rate:`${pass}/${rows.length}`,candidate:pass===rows.length?'100_PERCENT_CANDIDATE':'NOT_100_PERCENT'};});
  const winners=modeSummary.filter(x=>x.pass===x.total);
  const txt=['TOOL001 V41 PLATFORM PICKER A/B',`TOTAL=${results.length}`,`PHOTO=PHOTO_01`,`REPEATS_PER_MODE=${REPEATS}`,'','MODE_SUMMARY',...modeSummary.map(x=>`${x.id} PASS=${x.pass}/${x.total} FAIL=${x.fail} READ_PASS=${x.readPass}/${x.total} NOTREADABLE_EVENTS=${x.notReadableEvents} PICKER_PACKAGES=${x.pickerPackages.join(',')||'-'} RESULT=${x.candidate}`),'',`WINNER_COUNT=${winners.length}`,`WINNERS=${winners.map(x=>x.id).join(',')||'NONE'}`,'',winners.length===1?`INTERPRETATION=ONE_PICKER_PATH_STANDS_OUT:${winners[0].id}`:winners.length>1?'INTERPRETATION=MULTIPLE_100_PERCENT_PICKER_PATHS':'INTERPRETATION=NO_100_PERCENT_PICKER_PATH_PLATFORM_BOUNDARY_REMAINS','','ROWS',...results.map(r=>`CASE_${String(r.case).padStart(2,'0')} ${r.mode} R${r.round} ${r.pass?'PASS':'FAIL'} NR=${r.trace?.notReadableEvents||0} PICKER=${(r.pickerEnv?.uiPackages||[]).join(',')||r.pickerEnv?.resumedPackage||'-'} GET=${r.getFilePass} READ=${r.readPass?'PASS':'FAIL'} ERROR=${r.trace?.errorName||'-'}`)];
  write('v41-platform-picker-summary.txt',txt.join('\n'));write('result.json',{version:'V41-PLATFORM-PICKER-AB',url,generatedAt:new Date().toISOString(),modeSummary,results});log(txt.join('\n'));
  step('결과 ZIP 생성');restoreNotificationOverlays();await context.close().catch(()=>{});if(typeof device.close==='function')await device.close().catch(()=>{});adb('shell','svc','power','stayon','false');const zip=path.join(desktop,`${path.basename(outDir)}.zip`);const pr=spawnSync('powershell.exe',['-NoProfile','-Command',`Compress-Archive -Path '${outDir.replaceAll("'","''")}\\*' -DestinationPath '${zip.replaceAll("'","''")}' -Force`],{encoding:'utf8',shell:false});if(pr.status!==0)log('[WARN] ZIP creation failed',pr.stderr||pr.stdout||'');else log('[PASS] RESULT ZIP',zip);log('SUMMARY=v41-platform-picker-summary.txt');
}
