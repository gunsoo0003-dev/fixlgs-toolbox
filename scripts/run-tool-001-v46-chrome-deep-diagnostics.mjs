...
const outDir=path.join(desktop,`TOOLBOX_001_V46_CHROME_DEEP_DIAG_REAL_DEVICE_${stamp}`);
...
function dumpUi(){const remote='/sdcard/tool001-v46-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
const MINIMAL_HTML=`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>TOOL001 V46 Chrome Byte Read</title></head><body style="font-family:sans-serif;padding:28px"><h2>TOOL001 Chrome Minimal File Read</h2><p>No TOOL001 / React / Next.js. One visible file input, one picker selection, one arrayBuffer read. Passive diagnostics only.</p><input id="file" type="file" accept="image/*" style="font-size:20px;padding:14px 4px"><pre id="out"></pre><script>
...
f.addEventListener('change',async()=>{
 ...
 if(!file){window.__diag={...window.__diag,stage:'no_file',pass:false,changeAt};paint();console.log('V46_DIAG no_file');return;}
 ...
 console.log('V46_DIAG read_start',JSON.stringify({name:file.name,size:file.size,type:file.type,lastModified:file.lastModified,changeAt,readStartAt}));
 try{
   ...
   console.log('V46_DIAG read_pass',JSON.stringify({bytes:ab.byteLength,readDurationMs:readEndAt-readStartAt}));
 }catch(e){
   const readEndAt=Date.now();
   ...
   console.error('V46_DIAG read_fail',JSON.stringify({errorName:e&&e.name||'',errorMessage:e&&e.message||String(e),readDurationMs:readEndAt-readStartAt}));
 }
});
paint();
</script></body></html>`;
...
async function runChromeMinimal(){
  log('=== PHASE A: CHROME MINIMAL HTML / PASSIVE DEEP FILE-BRIDGE DIAGNOSTICS ===');
  ...
  for(let r=1;r<=repeats;r++){
    ...
    log(`===== Chrome minimal deep diag ${r}/${repeats} =====`);
    adb('logcat','-b','all','-c');
    const row={phase:'CHROME_MINIMAL_DEEP',round:r,pass:false,harnessFail:false,error:'',diag:null,notReadable:false};
    let page=null,evidence=null;
    try{
      write(`case-${caseNo}-chrome-process-before.txt`,chromeProcessSnapshot());
...
async function main(){
  log('=== TOOL001 V46 CHROME PASSIVE DEEP DIAGNOSTICS ===');
  log(`REPEATS=${repeats}`);
  log(`PICKER_STABLE_TIMEOUT_MS=${pickerStableTimeoutMs}`);
  log('NO_NATIVE_APP=TRUE');
  log('NO_CHROME_FLAGS_OR_BEHAVIOR_MUTATION=TRUE');
  ...
  const interpretation=chrome.valid===repeats?(chrome.fail>0?'CHROME_MINIMAL_NOTREADABLE_REPRODUCED_DEEP_EVIDENCE_CAPTURED':'NO_FAILURE_REPRODUCED_THIS_RUN'):'INCONCLUSIVE_HARNESS_FAILURE';
  const summary=[
    'TOOL001 V46 CHROME PASSIVE DEEP DIAGNOSTICS',
    `REPEATS=${repeats}`,
  ...
  write('v46-chrome-deep-summary.txt',summary);
  write('result.json',{version:'V46-CHROME-PASSIVE-DEEP',repeats,chrome,interpretation,passiveComparison,chromeRows});
  log(summary);
  await makeZip();
}
