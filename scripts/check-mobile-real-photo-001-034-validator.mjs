...
const scriptPath=process.env.MOBILE_VALIDATOR_SCRIPT || path.join(root,'scripts','run-mobile-real-photo-001-034.mjs');
...
if(toolNums.length!==34)errors.push(`tool definitions != 34 (${toolNums.length})`);
for(let i=1;i<=34;i++)if(!toolNums.includes(i))errors.push(`missing TOOL${String(i).padStart(3,'0')}`);
if(new Set(toolNums).size!==toolNums.length)errors.push('duplicate tool definition');
for(const x of ['TOOLBOX_MOBILE_REALPHOTO_001_034_${stamp}','fatal-harness.json','RESULT ZIP TARGET','archiveResultFolder(outDir,zipPath)']) if(!script.includes(x))errors.push(`missing result/archive contract: ${x}`);
...
if((script.match(/inputKind:'pdf-document'/g)||[]).length<8)errors.push('TOOL027/028/029/030/031/032/033/034 PDF document-input contracts missing');
...
console.log('[PASS] mobile real-photo 001~034 validator checklist/source/static self-check');
