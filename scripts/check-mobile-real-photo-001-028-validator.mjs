...
const scriptPath=process.env.MOBILE_VALIDATOR_SCRIPT || path.join(root,'scripts','run-mobile-real-photo-001-028.mjs');
...
if(toolNums.length!==28)errors.push(`tool definitions != 28 (${toolNums.length})`);
for(let i=1;i<=28;i++)if(!toolNums.includes(i))errors.push(`missing TOOL${String(i).padStart(3,'0')}`);
if(new Set(toolNums).size!==toolNums.length)errors.push('duplicate tool definition');
for(const x of ['TOOLBOX_MOBILE_REALPHOTO_001_028_${stamp}','fatal-harness.json','RESULT ZIP TARGET','archiveResultFolder(outDir,zipPath)']) if(!script.includes(x))errors.push(`missing result/archive contract: ${x}`);
...
if((script.match(/inputKind:'pdf-document'/g)||[]).length<2)errors.push('TOOL027/028 PDF document-input contracts missing');
...
console.log('[PASS] mobile real-photo 001~028 validator checklist/source/static self-check');
