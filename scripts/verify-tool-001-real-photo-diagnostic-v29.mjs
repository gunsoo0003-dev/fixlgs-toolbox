#!/usr/bin/env node
import fs from 'fs';
const runner=fs.readFileSync(new URL('./run-tool-001-real-photo-diagnostic-v29.mjs',import.meta.url),'utf8');
...
const checks={
  version:/REAL_PHOTO_V29|DIAGNOSTIC_V29|version:'V29'/.test(runner),
  ...
  user_only_taps_photo:/사진 1장만 탭하세요/.test(runner)&&/완료\/추가\/선택은 검수기가 자동 처리합니다/.test(runner),
  no_manual_commit_request:!/직접 1회 누르세요/.test(runner)&&!/Picker가 아직 열려 있습니다/.test(runner),
  ...
  short_selection_timeout:/const selectionDeadline=Date\.now\(\)\+30000/.test(runner),
...
console.log(JSON.stringify({STATIC_SELFTEST:missing.length?'FAIL':'PASS',version:'V29',missing,...checks},null,2));
