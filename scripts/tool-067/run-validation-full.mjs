...
const txt=path.join(resultDir,`067_final_검수결과_${stamp}.txt`);
const rows=[];
...
function browser(){return isWin?run('browser','npx playwright test --config=playwright.tool067.config.ts --workers=1 --reporter=list',[],{shell:true}):run('browser','npx',['playwright','test','--config=playwright.tool067.config.ts','--workers=1','--reporter=list'])}

npmRun('static','tool067:static');
run('secret-scan',process.execPath,['scripts/tool-067/check-secret-scan.mjs']);
if(deps()){
  browser();
...
const summary=`TOOL067 MODE=final\nPASS=${pass}\nFAIL=${fail}\nMAIN_WORK=${missingDeps?1:0}\nENVIRONMENT_BLOCK=0\nSTATUS=${status}\n`;
