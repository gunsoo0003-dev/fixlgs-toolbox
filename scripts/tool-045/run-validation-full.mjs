...
const resultDir=process.env.TOOL045_RESULT_DIR?path.resolve(process.env.TOOL045_RESULT_DIR):desktop;
fs.mkdirSync(resultDir,{recursive:true});
const stamp=new Date().toISOString().replace(/[:.]/g,'-');
const txt=path.join(resultDir,`045_${mode}_검수결과_${stamp}.txt`);
const rows=[];
...
const playwright=(spec)=>{
  ...
  if(isWin){
    return run(
      spec,
      `npx playwright test tests/tool-045-${spec}.spec.ts --workers=1 --config=playwright.tool045.config.ts --reporter=list`,
      [],
      {shell:true}
    );
  }
  return run(spec,'npx',['playwright','test',`tests/tool-045-${spec}.spec.ts`,'--workers=1','--config=playwright.tool045.config.ts','--reporter=list']);
};

if(mode==='preflight'){
  run('static-self-check',process.execPath,['scripts/tool-045/run-static-validation.mjs']);
