...
const resultDir=process.env.TOOL051_RESULT_DIR?path.resolve(process.env.TOOL051_RESULT_DIR):path.join(process.env.USERPROFILE||os.homedir(),'Desktop');
fs.mkdirSync(resultDir,{recursive:true});
const stamp=new Date().toISOString().replace(/[:.]/g,'-');
const txt=path.join(resultDir,`051_${mode}_검수결과_${stamp}.txt`);
const rows=[];
