...
for(const script of scripts){
 const r=spawnSync(process.execPath,[`scripts/tool-078/${script}`],{encoding:'utf8'});
 process.stdout.write(`\n### ${script}\n${r.stdout||''}`);
 if(r.stderr)process.stderr.write(r.stderr);
 if(r.status!==0)failed++;
}
console.log(`\nTOOL078 STATIC VALIDATION: ${failed===0?'PASS':'FAIL'} (${scripts.length-failed}/${scripts.length})`);
