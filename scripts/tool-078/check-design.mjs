...
const page=fs.readFileSync('components/tool-078-stock-average-cost-calculator-page.tsx','utf8');
const css=fs.readFileSync('components/tool-078-stock-average-cost-calculator.module.css','utf8');
const seq=[
...
check('ToolNavigation current tool',page.includes('ToolNavigation')&&page.includes('currentTool={78}'));
check('expert shared wide head',page.includes('toolbox-tool-expert-post--wide-head'));
const expertTags=["TOTAL COST", "TOTAL SHARES", "AVERAGE", "BREAK-EVEN", "TARGET", "BOUNDARY"];
...
check('actual tool number',page.includes('078'));
...
console.log(`RESULT TOOL078 DESIGN PASS=${pass} FAIL=${fail}`);
