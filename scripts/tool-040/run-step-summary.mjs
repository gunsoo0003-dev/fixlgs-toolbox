...
const commands = {
  static: ['scripts/tool-040/run-static-validation.mjs', []],
  main: ['scripts/tool-040/check-main-integration.mjs', []],
  source: ['scripts/tool-040/check-source.mjs', []],
};
if (!commands[step]) { console.error(`Unknown TOOL040 step: ${step}`); process.exit(2); }

const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `tool040-summary-${step}-`));
const zipPath = path.join(desktop, `040_${step}_검수결과.zip`);
const summaryTxt = `TOOL 040 ${step}\n`;
...
try {
  ...
  const summary={tool:'040',step,status,exitCode,started,ended:new Date().toISOString(),stdout,stderr};
  ...
  fs.writeFileSync(path.join(tmp,'summary.json'),JSON.stringify({tool:'040',step,status:'FAIL',exitCode:1,error:msg},null,2),'utf8');
  try{fs.unlinkSync(zipPath)}catch{}
  zipDirectory(tmp,zipPath);
