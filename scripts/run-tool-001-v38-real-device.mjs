...
const outDir=path.join(desktop,`TOOLBOX_001_V38_STABLE_HANDLE_REAL_DEVICE_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V38__'; if(window[KEY])return;
  const s=window[KEY]={capture:[],consoleErrors:[],events:[],lifecycle:[],nativeReads:[]};
  window.addEventListener('tool001:capture-diagnostic',e=>{
    try{s.capture.push(JSON.parse(JSON.stringify(e.detail||{})));}catch{}
  },true);
  const pushLife=(kind,extra={})=>s.lifecycle.push({at:Date.now(),kind,visibility:document.visibilityState,focus:document.hasFocus(),href:location.href,...extra});
  ...
  // Passive native read observer only: never initiates a provider read.
  try{
    const proto=Blob.prototype,orig=proto.arrayBuffer;
    if(typeof orig==='function'&&!orig.__tool001v38Wrapped){const wrapped=async function(){const id=`ab-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;s.nativeReads.push({id,at:Date.now(),phase:'arraybuffer-call',size:Number(this?.size||0),type:String(this?.type||'')});try{const r=await orig.call(this);s.nativeReads.push({id,at:Date.now(),phase:'arraybuffer-load',bytes:Number(r?.byteLength||0)});return r;}catch(e){s.nativeReads.push({id,at:Date.now(),phase:'arraybuffer-error',error:`${e?.name||'Error'}:${e?.message||String(e)}`});throw e;}};Object.defineProperty(wrapped,'__tool001v38Wrapped',{value:true});proto.arrayBuffer=wrapped;}
  }catch(e){s.consoleErrors.push({at:Date.now(),message:'V38_ARRAYBUFFER_OBSERVER:'+String(e?.message||e)});}
...
async function waitV38Outcome(page,startCapture,before,timeoutMs=8000){
  const dl=Date.now()+timeoutMs;let last=null;
  while(Date.now()<dl){
    const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V38__)));
    const cap=(st.capture||[]).slice(startCapture);const dom=await domSnapshot(page);last={cap,dom,obs:st};
    const pickerSelected=cap.some(x=>x.phase==='stable-picker-selected');
    const owned=cap.some(x=>x.phase==='stable-handle-owned-pass');
    const finalFail=cap.find(x=>x.phase==='stable-handle-final-fail'||x.phase==='stable-picker-capture-fail'||x.phase==='stable-picker-error');
    const preview=dom.cards.flatMap(c=>c.imgs||[]).some(img=>img.complete&&img.naturalWidth>0&&img.naturalHeight>0);
    if((pickerSelected&&owned&&dom.cardCount>before.cardCount&&preview)||finalFail) return last;
    await sleep(180);
  }
  return last||{cap:[],dom:await domSnapshot(page),obs:await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V38__)))};
}
...
async function main(){
  ...
  step('V38 수동 관찰기 주입');await page.addInitScript(browserInit);
  ...
  const support=await page.evaluate(()=>({ua:navigator.userAgent,showOpenFilePicker:typeof window.showOpenFilePicker==='function'}));write('v38-feature-support.json',support);log(`[FEATURE] showOpenFilePicker=${support.showOpenFilePicker}`);if(!support.showOpenFilePicker)throw new Error('V38_STABLE_PICKER_UNSUPPORTED_ON_DEVICE');
  step('PHOTO_01 동일사진 10회 검수');const results=[];
  for(let i=0;i<REPEATS;i++){
    const caseNo=i+1;log('');log(`===== V38 CASE ${caseNo}/${REPEATS} =====`);await ensureExpectedToolPage(page,caseNo,'case-start');await page.bringToFront();
    const before=await domSnapshot(page);const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V38__)));adb('logcat','-c');
    const r={case:caseNo,pass:false,error:'',capture:[],summary:null,newCards:0,previewCount:0};
    try{
      ...
      const outcome=await timed(`CASE ${caseNo} product outcome`,()=>waitV38Outcome(page,(st0.capture||[]).length,before,9000),10000);
      r.capture=outcome.cap;r.after=outcome.dom;r.newCards=Math.max(0,outcome.dom.cardCount-before.cardCount);r.previewCount=outcome.dom.cards.flatMap(c=>c.imgs||[]).filter(img=>img.complete&&img.naturalWidth>0&&img.naturalHeight>0).length;r.summary=summarizeCase(r);r.pass=r.summary.ownedPass>0&&r.newCards>0&&r.previewCount>0;r.error=r.pass?'':String(r.summary.failures.at(-1)?.error||'NO_CARD_PREVIEW');
    }catch(e){r.error=String(e?.message||e);const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V38__))).catch(()=>({capture:[]}));r.capture=(st.capture||[]).slice((st0.capture||[]).length);r.summary=summarizeCase(r);}
    const raw=adbText('logcat','-d','-v','time');r.logcatEvidence=raw.split(/\r?\n/).filter(x=>/(NotReadableError|ERR_UPLOAD_FILE_CHANGED|UPLOAD_FILE_CHANGED|file could not be read|net::ERR_)/i.test(x)).slice(-150);write(`case-${caseNo}.json`,r);write(`case-${caseNo}-logcat.txt`,r.logcatEvidence.join('\n'));await pcScreen(`case-${caseNo}-device.png`);
    log(`[${r.pass?'PASS':'FAIL'}] CASE ${caseNo} attempt=${r.summary?.successAttempt??'-'} NotReadable=${r.summary?.notReadableFailures??0} cards=${r.newCards} preview=${r.previewCount} ${r.error}`);results.push(r);
    if(i<REPEATS-1){await page.goto(url,{waitUntil:'domcontentloaded',timeout:45000});await sleep(350);}
  }
  step('최종 집계');const pass=results.filter(r=>r.pass).length,fail=results.length-pass,nr=results.reduce((a,r)=>a+(r.summary?.notReadableFailures||0),0);const attempts={1:0,2:0,3:0,none:0};for(const r of results){const k=r.summary?.successAttempt;if(k===1||k===2||k===3)attempts[k]++;else attempts.none++;}
  const txt=['TOOL001 V38 STABLE FILE HANDLE REAL DEVICE',`TOTAL=${results.length}`,`PASS=${pass}`,`FAIL=${fail}`,`NOTREADABLE_EVENTS=${nr}`,`FINAL=${pass===results.length?'PASS_100_PERCENT':'FAIL_NOT_100_PERCENT'}`,`SUCCESS_ATTEMPT_1=${attempts[1]}`,`SUCCESS_ATTEMPT_2=${attempts[2]}`,`SUCCESS_ATTEMPT_3=${attempts[3]}`,`NO_SUCCESS=${attempts.none}`,'','ROWS',...results.map(r=>`CASE_${String(r.case).padStart(2,'0')} ${r.pass?'PASS':'FAIL'} successAttempt=${r.summary?.successAttempt??'-'} notReadable=${r.summary?.notReadableFailures??0} cards=${r.newCards} preview=${r.previewCount} error=${r.error||''}`)];write('v38-real-device-summary.txt',txt.join('\n'));write('result.json',{version:'V38-HARNESS',url,generatedAt:new Date().toISOString(),pass,fail,notReadableEvents:nr,attempts,results});log(txt.join('\n'));
  step('결과 ZIP 생성');restoreNotificationOverlays();await context.close().catch(()=>{});if(typeof device.close==='function')await device.close().catch(()=>{});adb('shell','svc','power','stayon','false');const zip=path.join(desktop,`${path.basename(outDir)}.zip`);const pr=spawnSync('powershell.exe',['-NoProfile','-Command',`Compress-Archive -Path '${outDir.replaceAll("'","''")}\\*' -DestinationPath '${zip.replaceAll("'","''")}' -Force`],{encoding:'utf8',shell:false});if(pr.status!==0)log('[WARN] ZIP creation failed',pr.stderr||pr.stdout||'');else log('[PASS] RESULT ZIP',zip);log('SUMMARY=v38-real-device-summary.txt');
}
