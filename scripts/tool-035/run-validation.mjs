...
const outDir=fs.mkdtempSync(path.join(os.tmpdir(),`tool035-${mode}-`));
const results=[];
...
const outBase=process.env.TOOL035_RESULT_DIR?path.resolve(process.env.TOOL035_RESULT_DIR):desktop;
fs.mkdirSync(outBase,{recursive:true});
const zipPath=path.join(outBase,`035_${mode}_검수결과.zip`);
const liveTxtPath=path.join(outBase,`035_${mode}_진행중.txt`);
const liveJsonPath=path.join(outBase,`035_${mode}_진행중.json`);

const STEP_TIMEOUT_MS={
  'preflight-runtime':180_000,
  'preflight':180_000,
  'core':300_000,
  'boundary':240_000,
  'feature':240_000,
...
const staticSteps=[['source',node,['scripts/tool-035/check-source.mjs']],['content',node,['scripts/tool-035/check-content.mjs']],['logic',node,['scripts/tool-035/check-logic.mjs']],['harness-static',node,['scripts/tool-035/check-harness.mjs']],['design-code',node,['scripts/tool-035/check-design.mjs']],['package',node,['scripts/tool-035/check-package.mjs']],['common-protection',node,['scripts/tool-035/check-common-protection.mjs']],['main-integration',node,['scripts/tool-035/check-main-integration.mjs']]];
const spec=(f,n)=>[n,node,[pw,'test',f,'--workers=1']];let steps=[...staticSteps];
if(mode==='preflight')steps.push(spec('tests/tool-035-preflight.spec.ts','preflight-runtime'));
if(mode==='core-only')steps.push(spec('tests/tool-035-core.spec.ts','core'));
if(mode==='boundary-only')steps.push(spec('tests/tool-035-boundary.spec.ts','boundary'));
if(mode==='feature-only')steps.push(spec('tests/tool-035-feature.spec.ts','feature'));
if(mode==='regression-only')steps.push(spec('tests/tool-035-regression.spec.ts','regression'));
if(mode==='limit-only')steps.push(spec('tests/tool-035-limit.spec.ts','limit'));
if(mode==='final')steps.push(['typescript',node,[tsc,'--noEmit']],['production-build',node,[next,'build']],spec('tests/tool-035-preflight.spec.ts','preflight'),spec('tests/tool-035-core.spec.ts','core'),spec('tests/tool-035-boundary.spec.ts','boundary'),spec('tests/tool-035-feature.spec.ts','feature'),spec('tests/tool-035-regression.spec.ts','regression'),spec('tests/tool-035-limit.spec.ts','limit'));
try{
  ...
  else{console.log(`TOOL 035 ${mode.toUpperCase()} | total steps=${steps.length}`);for(let i=0;i<steps.length&&!interrupted;i++){const [name,cmd,args]=steps[i];await runStep(i+1,steps.length,name,cmd,args)}}
