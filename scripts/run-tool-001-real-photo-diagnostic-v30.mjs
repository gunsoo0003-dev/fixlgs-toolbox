...
const labels=String(args.cases||'KNOWN_GOOD_1,KNOWN_GOOD_REPEAT,KNOWN_BAD_1,KNOWN_BAD_REPEAT,CAMERA_PHOTO,SCREENSHOT,DOWNLOADED_OR_EDITED,KNOWN_BAD_REPEAT_2').split(',').map(x=>x.trim()).filter(Boolean);
...
const outDir=path.join(desktop,`TOOLBOX_001_REAL_PHOTO_V30_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function dumpUi(){const remote='/sdcard/tool001-v30-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
function pickerSelectionSignals(xml){const nodes=parseUiNodes(xml);const selectedNodes=nodes.filter(n=>n.checked==='true'||n.selected==='true'||/(선택됨|selected|1개 선택|1 selected|체크됨|checked|전체 선택 해제|deselect all)/i.test(nodeHay(n)));const actionControls=photoPickerActionControls(xml);const text=nodes.map(nodeHay).join('\n');const selectedByText=/(전체 선택 해제|1개 선택|1 selected|선택됨|selected)/i.test(text);return{selectedNodes,actionControls,selectedByText,detected:selectedNodes.length>0||actionControls.length>0||selectedByText};}
...
async function autoCommitManualSelection(caseNo,page,startChange){const deadline=Date.now()+90000;let sawSelection=false;let commitTaps=0;let lastXml='';while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V30__)));if(st.changes.length>startChange)return{committed:true,mode:'picker-auto-close-change',sawSelection,commitTaps};let open=false;try{lastXml=dumpUi();open=pickerDetectedFromXml(lastXml);}catch{}if(!open){await sleep(250);const st2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V30__)));if(st2.changes.length>startChange)return{committed:true,mode:'picker-closed-change',sawSelection,commitTaps};await sleep(500);continue;}const sig=pickerSelectionSignals(lastXml);if(sig.detected){sawSelection=true;write(`case-${caseNo}-selection-observed.xml`,lastXml);const actions=photoPickerActionControls(lastXml);write(`case-${caseNo}-commit-controls.json`,actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds})));if(actions.length){for(const action of actions){const b=action.bounds;adb('shell','input','tap',String(b.x),String(b.y));commitTaps++;log(`[PHOTO_PICKER] case ${caseNo}: auto commit "${action.exact}" at ${b.x},${b.y}`);await pcScreen(`case-${caseNo}-after-auto-commit-${commitTaps}.png`);if(await pickerClosedStable(page,7000)){const st3=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V30__)));if(st3.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit',sawSelection:true,commitTaps,action:action.exact};await sleep(1500);const st4=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V30__)));if(st4.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit-delayed-change',sawSelection:true,commitTaps,action:action.exact};return{committed:false,mode:'picker-closed-no-change',sawSelection:true,commitTaps,action:action.exact};}}}}
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V30__'; if(window[KEY])return;
  const s=window[KEY]={changes:[],capture:[],consoleErrors:[],events:[],lifecycle:[],nav:[]};
...
async function ensureChromeInteractive(page,caseNo,tag='guard'){
  ...
  // V30: NEVER infer overflow menu from toolbar "새 탭".
...
function pickerMediaCandidates(xml){
  const nodes=parseUiNodes(xml).filter(n=>/com\.google\.android\.photopicker/i.test(n.package||''));
  const out=[];
  const seen=new Set();
  for(const n of nodes){
    ...
    if(b.cy<420)continue;                 // exclude top toolbar/header
    if(b.cy>2050)continue;                // exclude bottom action / gesture area
    if(w>700||h>700)continue;             // exclude full-width containers/sheets
    const ratio=w/h;
    if(ratio<0.55||ratio>1.8)continue;     // media cells are roughly rectangular/square
    const hay=nodeHay(n);
    if(/완료|추가|선택|취소|done|add|select|cancel|검색|search|album|앨범/i.test(hay))continue;
    if(/drag|handle|손잡이/i.test(hay))continue;
    const interactive=n.clickable==='true'||n.checkable==='true'||/image|frame|view/i.test(n.class||'');
    if(!interactive)continue;
    const key=n.bounds;
    if(seen.has(key))continue;
    seen.add(key);
    out.push({bounds:n.bounds,desc:n['content-desc']||'',text:n.text||'',class:n.class||'',clickable:n.clickable,checkable:n.checkable,parsed:b});
  }
  // Stable visual order: top-to-bottom, left-to-right.
  out.sort((a,b)=>a.parsed.top-b.parsed.top || a.parsed.left-b.parsed.left);
  return out;
}
function caseAutoMediaIndex(label){
  // Repeat labels intentionally reuse the same visible cell index.
  if(label==='KNOWN_GOOD_1'||label==='KNOWN_GOOD_REPEAT')return 0;
  if(label==='KNOWN_BAD_1'||label==='KNOWN_BAD_REPEAT'||label==='KNOWN_BAD_REPEAT_2')return 1;
  if(label==='CAMERA_PHOTO')return 2;
  if(label==='SCREENSHOT')return 3;
  if(label==='DOWNLOADED_OR_EDITED')return 4;
  return 0;
}
async function autoSelectPickerMedia(caseNo,label,round){
  const before=pickerSignalSnapshot();
  if(!before.detected)throw new Error('AUTO_MEDIA_PICKER_NOT_ACTIVE');
  let candidates=pickerMediaCandidates(before.xml||'');
  write(`case-${caseNo}-auto-media-candidates-r${round}.json`,candidates.map((c,i)=>({i,bounds:c.bounds,desc:c.desc,text:c.text,class:c.class,clickable:c.clickable,checkable:c.checkable})));
  if(!candidates.length){
    write(`case-${caseNo}-auto-media-no-candidate-r${round}.xml`,before.xml||'');
    await pcScreen(`case-${caseNo}-auto-media-no-candidate-r${round}.png`);
    throw new Error('AUTO_MEDIA_CELL_NOT_FOUND');
  }
  const wanted=caseAutoMediaIndex(label);
  const idx=Math.min(wanted,candidates.length-1);
  const c=candidates[idx];
  const b=c.parsed;
  // Last safety gate against prior V12 drag-handle mistake.
  if(b.cy<420||b.cy>2050||(b.right-b.left)<90||(b.bottom-b.top)<90)throw new Error('AUTO_MEDIA_UNSAFE_BOUNDS');
  adb('shell','input','tap',String(b.cx),String(b.cy));
  log(`[AUTO PHOTO] CASE ${caseNo}: media cell #${idx} 자동 탭 (${c.bounds})`);
  await sleep(220);
  await pcScreen(`case-${caseNo}-after-auto-photo-tap-r${round}.png`);

  const dl=Date.now()+5000;
  while(Date.now()<dl){
    const snap=pickerSignalSnapshot();
    if(!snap.detected){
      // One-tap auto-return is valid; caller will observe change/close.
      return {selected:true,autoClosed:true,index:idx,candidate:c};
    }
    const sig=pickerSelectionSignals(snap.xml||'');
    if(sig.detected){
      write(`case-${caseNo}-auto-photo-selected-r${round}.xml`,snap.xml||'');
      return {selected:true,autoClosed:false,index:idx,candidate:c};
    }
    await sleep(140);
  }
  const after=pickerSignalSnapshot();
  write(`case-${caseNo}-auto-photo-selection-timeout-r${round}.xml`,after.xml||'');
  await pcScreen(`case-${caseNo}-auto-photo-selection-timeout-r${round}.png`);
  throw new Error('AUTO_MEDIA_SELECTION_NOT_CONFIRMED');
}
...
async function waitForSelectionAndCommit(caseNo,label,page,startChange,context){
  ...
  async function pageState(tag){
    ...
    try{
      st=await page.evaluate(()=>({
        ...
        fileCount:Array.from(document.querySelectorAll('input[type=file]')).reduce((n,el)=>n+(el.files?.length||0),0)
      }));
    }catch(e){st={error:String(e?.message||e)}}
    write(`case-${caseNo}-${tag}-page-state-${Date.now()}.json`,st);
    return st;
  }

  async function observer(){
    return page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V30__))).catch(()=>null);
  }

  async function waitChange(ms){
    const dl=Date.now()+ms;
    while(Date.now()<dl){
      const st=await observer();
      if(st?.changes?.length>startChange)return st.changes.at(-1);
      await sleep(120);
    }
    return null;
  }

  function uniqueActionCandidates(actions){
    const seen=new Set(), out=[];
    for(const a of actions||[]){
      const b=parseBounds(a.bounds);
      if(!b)continue;
      const key=`${a.label}|${a.bounds}`;
      if(seen.has(key))continue;
      seen.add(key);
      // Positive Photo Picker action must be in the bottom half of the device.
      // This prevents toolbar/address-bar or media-cell taps.
      if(b.cy < 1000)continue;
      out.push({...a,parsed:b});
    }
    return out.sort((a,b)=>{
      const ax=(a.exact?0:1), bx=(b.exact?0:1);
      if(ax!==bx)return ax-bx;
      return b.parsed.cy-a.parsed.cy;
    });
  }

  async function tapCommitCandidate(candidate,round,rescanNo){
    const before=dumpUi();
    const controls=uniqueActionCandidates(photoPickerActionControls(before));
    const exact=controls.find(a=>a.label===candidate.label && a.bounds===candidate.bounds) || controls[0];
    if(!exact) return {tapped:false,reason:'candidate-disappeared'};
    // Revalidate current foreground is still picker immediately before tap.
    const snap=pickerSignalSnapshot();
    if(!snap.detected) return {tapped:false,reason:'picker-not-active'};
    const b=exact.parsed||parseBounds(exact.bounds);
    if(!b || b.cy<1000)return {tapped:false,reason:'unsafe-bounds'};
    write(`case-${caseNo}-auto-commit-pre-r${round}-s${rescanNo}.json`,{
      candidate:{label:exact.label,bounds:exact.bounds,exact:exact.exact},
      resumedPackage:snap.resumedPackage,
      uiPackages:snap.uiPackages
    });
    adb('shell','input','tap',String(b.cx),String(b.cy));
    await sleep(250);
    await pcScreen(`case-${caseNo}-after-auto-commit-r${round}-s${rescanNo}.png`);
    return {tapped:true,candidate:{label:exact.label,bounds:exact.bounds,exact:exact.exact}};
  }

  for(let round=1;round<=3;round++){
    await ensureExpectedToolPage(page,caseNo,`auto-before-activation-r${round}`);

    let activation;
    try{
      activation=await tapUpload(page,context,caseNo,round);
    }catch(e){
      session.push({round,activationError:String(e?.message||e)});
      write(`case-${caseNo}-activation-session.json`,session);
      if(round===3)throw e;
      continue;
    }

    session.push({round,activation:{method:activation.method,strategies:activation.strategies},mode:'AUTO_COMMIT_AFTER_USER_SELECTION'});
    write(`case-${caseNo}-activation-session.json`,session);

    log(`[PICKER READY] CASE ${caseNo} ${label}: 실제 Android Photo Picker 확인.`);
    log(`[AUTO] CASE ${caseNo}: 사진 선택부터 commit까지 무인 자동 진행합니다.`);

    const selectionDeadline=Date.now()+20000;
    let sawSelection=false;
    let selectionAt=0;
    let commitTap=null;
    let lastPickerOpen=true;
    let lastXml='';
    let rescanNo=0;
    const timeline=[];
    ...
    let autoMedia;
    try{
      autoMedia=await autoSelectPickerMedia(caseNo,label,round);
      sawSelection=!!autoMedia.selected;
      selectionAt=Date.now();
      timeline.push({at:selectionAt,event:'auto-media-tap',index:autoMedia.index,bounds:autoMedia.candidate?.bounds,autoClosed:autoMedia.autoClosed});
    }catch(e){
      session.push({round,autoMediaError:String(e?.message||e)});
      write(`case-${caseNo}-activation-session.json`,session);
      if(round===3)throw e;
      log(`[RECOVER] CASE ${caseNo}: 자동 사진 선택 실패. Picker 재오픈 (${round}/3)`);
      adb('shell','input','keyevent','KEYCODE_BACK');
      await sleep(300);
      await ensureExpectedToolPage(page,caseNo,`after-auto-media-fail-r${round}`);
      continue;
    }
    ...
    while(Date.now()<selectionDeadline){
      ...
      if(obs?.changes?.length>startChange){
        const st=await pageState(`auto-change-before-commit-r${round}`);
        timeline.push({at:now,event:'change-before-commit',pageState:st});
        write(`case-${caseNo}-auto-picker-timeline-r${round}.json`,timeline);
        return{
          committed:true,
          mode:'AUTO_RETURN_CHANGE',
          sawSelection,
          commitTaps:0,
          round,
          activation,
      ...
      const snap=pickerSignalSnapshot();
      const pickerOpen=!!snap.detected;
      lastXml=snap.xml||'';
      const sig=pickerOpen ? pickerSelectionSignals(lastXml) : {selectedNodes:[],selectedByText:false,detected:false};
      const actions=pickerOpen ? uniqueActionCandidates(photoPickerActionControls(lastXml)) : [];
      ...
      timeline.push({
        ...
        resumedPackage:snap.resumedPackage,
        uiPackages:snap.uiPackages,
        selectedNodeCount:sig.selectedNodes?.length||0,
        selectedByText:!!sig.selectedByText,
        commitControls:actions.map(a=>({label:a.label,bounds:a.bounds,exact:a.exact}))
      });
      if(timeline.length>180)timeline.shift();

      if(pickerOpen && sig.detected && !sawSelection){
        sawSelection=true;
        selectionAt=now;
        log(`[SELECTION] CASE ${caseNo}: 자동 사진 선택 확인. commit 탐색 시작.`);
        write(`case-${caseNo}-selection-observed-r${round}.xml`,lastXml);
      }

      if(sawSelection && pickerOpen && !commitTap){
        rescanNo++;
        write(`case-${caseNo}-commit-controls-r${round}-s${rescanNo}.json`,actions.map(a=>({
          label:a.label,bounds:a.bounds,exact:a.exact
        })));

        if(actions.length){
          const candidate=actions[0];
          const tapResult=await tapCommitCandidate(candidate,round,rescanNo);
          if(tapResult.tapped){
            commitTap={at:Date.now(),...tapResult.candidate};
            log(`[AUTO COMMIT] CASE ${caseNo}: "${commitTap.label}" 1회 탭 (${commitTap.bounds})`);
            const ch=await waitChange(4500);
            if(ch){
              write(`case-${caseNo}-auto-picker-timeline-r${round}.json`,timeline);
              return{
                committed:true,
                mode:'AUTO_COMMIT_CHANGE',
                sawSelection:true,
                commitTaps:1,
                commitTap,
                round,
      ...
      // Once commit was tapped, do not tap again. Only observe picker close / change.
      if(commitTap){
        const snapAfter=pickerSignalSnapshot();
        if(!snapAfter.detected){
          const st=await pageState(`auto-picker-closed-r${round}`);
          timeline.push({at:Date.now(),event:'picker-closed-after-auto-commit',pageState:st});
          log(`[PICKER CLOSED] CASE ${caseNo}: 자동 commit 후 Chrome 반환 확인.`);
          const ch=await waitChange(6500);
          if(ch){
            write(`case-${caseNo}-auto-picker-timeline-r${round}.json`,timeline);
            return{
              committed:true,
              mode:'AUTO_COMMIT_PICKER_CLOSED_CHANGE',
              sawSelection:true,
              commitTaps:1,
              commitTap,
              round,
          ...
          const after=await pageState(`auto-picker-closed-no-change-r${round}`);
          write(`case-${caseNo}-auto-picker-timeline-r${round}.json`,timeline);
          write(`case-${caseNo}-auto-picker-closed-no-change-r${round}.json`,{
            commitTap,after,observer:await observer(),foreground:foregroundSnapshot()
          });
          return{
            committed:false,
            mode:'PICKER_CLOSED_NO_CHANGE',
            sawSelection:true,
            commitTaps:1,
            commitTap,
            round,
        ...
        // Commit tap happened but picker stayed open. Give it a short settle window, never re-tap.
        if(Date.now()-commitTap.at>=5000){
          await pcScreen(`case-${caseNo}-auto-commit-picker-still-open-r${round}.png`);
          write(`case-${caseNo}-auto-commit-picker-still-open-r${round}.xml`,snapAfter.xml||'');
          write(`case-${caseNo}-auto-picker-timeline-r${round}.json`,timeline);
          return{
            committed:false,
            mode:'AUTO_COMMIT_TAPPED_PICKER_STILL_OPEN',
            sawSelection:true,
            commitTaps:1,
            commitTap,
            round,
      ...
      if(lastPickerOpen && !pickerOpen && sawSelection && !commitTap){
        // Some Android picker flows auto-close immediately after the user's selection.
        const st=await pageState(`selection-auto-closed-r${round}`);
        timeline.push({at:Date.now(),event:'selection-auto-closed',pageState:st});
        const ch=await waitChange(6500);
        write(`case-${caseNo}-auto-picker-timeline-r${round}.json`,timeline);
        if(ch){
          return{
            committed:true,
            mode:'SELECTION_AUTO_CLOSED_CHANGE',
            sawSelection:true,
            commitTaps:0,
            round,
            activation,
      ...
      if(!pickerOpen && !sawSelection){
        const ch=await waitChange(1200);
        if(ch){
          write(`case-${caseNo}-auto-picker-timeline-r${round}.json`,timeline);
          return{
            committed:true,
            mode:'AUTO_RETURN_CHANGE_WITHOUT_SELECTION_SIGNAL',
            sawSelection:false,
            commitTaps:0,
            round,
            activation,
        ...
        log(`[RECOVER] CASE ${caseNo}: 선택 전에 Picker가 닫혔습니다. 재오픈 (${round}/3)`);
        break;
      }

      // If selection is visible but the positive action has not appeared yet,
      // keep rescanning fast. This is the exact V27/V28 bottleneck.
      if(sawSelection && !commitTap && Date.now()-selectionAt>=12000){
        await pcScreen(`case-${caseNo}-commit-control-not-exposed-r${round}.png`);
        write(`case-${caseNo}-commit-control-not-exposed-r${round}.xml`,lastXml);
        write(`case-${caseNo}-auto-picker-timeline-r${round}.json`,timeline);
        return{
          committed:false,
          mode:'COMMIT_CONTROL_NOT_EXPOSED_AFTER_LIVE_RESCAN',
          sawSelection:true,
          commitTaps:0,
          round,
          activation,
    ...
    if(sawSelection){
      return{
        committed:false,
        mode:'SELECTION_TIMEOUT_AFTER_OBSERVED',
        sawSelection:true,
        commitTaps:commitTap?1:0,
        commitTap,
        round,
        activation,
        timeline
  ...
  return{
    committed:false,
    mode:'picker-lost-before-selection-after-retries',
    sawSelection:false,
    commitTaps:0
  };
}

async function waitProduct(page,startCapture,before,timeoutMs=90000){const deadline=Date.now()+timeoutMs;let snap=before,cap=[];while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V30__)));cap=st.capture.slice(startCapture);snap=await domSnapshot(page);const terminal=cap.some(x=>x.phase==='picker-file-pass'||x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail');const cardChanged=snap.cardCount!==before.cardCount;if(terminal&&(cardChanged||cap.some(x=>x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail')))break;await sleep(350);}return{capture:cap,after:snap};}
async function collectFailureEvidence(caseNo,label,page,reason){let xml='';try{xml=dumpUi();}catch{}write(`case-${caseNo}-${label}-failure-picker.xml`,xml);await pcScreen(`case-${caseNo}-${label}-failure-device.png`);await page.screenshot({path:path.join(outDir,`case-${caseNo}-${label}-failure-page.png`),fullPage:true}).catch(()=>{});const snap=await domSnapshot(page).catch(()=>null);const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V30__))).catch(()=>null);write(`case-${caseNo}-${label}-failure-evidence.json`,{reason,pickerOpen:pickerDetectedFromXml(xml),selectionSignals:pickerSelectionSignals(xml).selectedNodes.length,commitControls:photoPickerActionControls(xml).map(a=>({exact:a.exact,label:a.label,bounds:a.bounds})),foreground:foregroundSnapshot(),observer:obs,dom:snap});}


function caseInstruction(label){
  const u=label.toUpperCase();
  if(u==='KNOWN_GOOD_1')return '평소 확실히 첨부되는 실제 사진 1장을 선택하세요.';
  if(u==='KNOWN_GOOD_REPEAT')return '방금 KNOWN_GOOD_1에서 고른 같은 사진을 다시 선택하세요.';
  if(u==='KNOWN_BAD_1')return '평소 확실히 첨부 실패하는 실제 사진 1장을 선택하세요.';
  if(u==='KNOWN_BAD_REPEAT'||u==='KNOWN_BAD_REPEAT_2')return 'KNOWN_BAD_1에서 고른 같은 실패 사진을 다시 선택하세요.';
  if(u==='CAMERA_PHOTO')return 'Galaxy 카메라로 직접 촬영한 원본 사진을 선택하세요.';
  if(u==='SCREENSHOT')return 'Galaxy에서 직접 만든 실제 스크린샷을 선택하세요.';
  if(u==='DOWNLOADED_OR_EDITED')return '다운로드했거나 편집 후 저장한 실제 이미지를 선택하세요.';
  return `${label}에 해당하는 실제 이미지를 1장 선택하세요.`;
}
...
function firstDivergence(results){
  const good=results.filter(r=>r.label.includes('GOOD')&&r.pass);const bad=results.filter(r=>r.label.includes('BAD')&&!r.pass);
  if(good.length&&bad.length)return classifyStage(bad[0]);
  const stages=results.map(r=>classifyStage(r));
  return stages.find(x=>x!=='PASS')||'NONE';
}
function repeatRaceEvidence(results){
  const pairs=[['KNOWN_GOOD_1','KNOWN_GOOD_REPEAT'],['KNOWN_BAD_1','KNOWN_BAD_REPEAT'],['KNOWN_BAD_1','KNOWN_BAD_REPEAT_2']];
  const out=[];for(const [a,b] of pairs){const x=results.find(r=>r.label===a),y=results.find(r=>r.label===b);if(x&&y)out.push({a,b,aPass:x.pass,bPass:y.pass,mixed:x.pass!==y.pass,aStage:classifyStage(x),bStage:classifyStage(y)});}return out;
}
async function main(){
  log('=== TOOL001 REAL USER PHOTO DIAGNOSTIC V30 FINAL-INTEGRATED ===');log('[OUTPUT]',outDir);
  ...
  step('실사진 비교검수 안내');log('');log('*** 검수기가 테스트 사진을 만들지 않습니다. ***');log('각 CASE에서 안내된 GOOD/BAD 실제 사진을 Galaxy Photo Picker에서 1장만 탭하세요.');log('');log('8개 케이스로 반복성·출처·포맷·provider·result-return·제품검증·preview를 넓게 탐색합니다.');log('');
  const results=[];
  for(let i=0;i<labels.length;i++){
    ...
    await ensureExpectedToolPage(page,caseNo,'case-start');await page.bringToFront();const before=await domSnapshot(page);const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V30__)));let result={case:caseNo,label,pass:false,error:null,before,commit:null};
    try{
      const commit=await timed(`CASE ${caseNo} verified picker + user selection + automatic commit`,()=>waitForSelectionAndCommit(caseNo,label,page,st0.changes.length,context),65000);result.commit=commit;result.activation=commit.activation||null;
      if(!commit.committed)throw new Error(`PHOTO_PICKER_COMMIT_FAILED:${commit.mode}`);
      const changeDeadline=Date.now()+8000;let change=null;while(Date.now()<changeDeadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V30__)));if(st.changes.length>st0.changes.length){change=st.changes.at(-1);break;}await sleep(250);}if(!change)throw new Error('NO_FILE_CHANGE_AFTER_VERIFIED_COMMIT');
      const r=await timed(`CASE ${caseNo} product capture + preview`,()=>waitProduct(page,st0.capture.length,before),70000);
      ...
      const inspection=r.capture.slice().reverse().find(x=>x.phase==='product-inspection-pass')||null;const providerReceived=r.capture.find(x=>x.phase==='picker-selection-received')||null;const productFail=r.capture.slice().reverse().find(x=>x.phase==='product-add-fail'||x.phase==='product-limit-reject'||x.phase==='picker-prefilter-reject')||null;result={...result,change,capture:r.capture,readerTimeline:readers,byteSignature:sniff||null,inspection,providerReceived,productFail,after:r.after,capturePass:!!capPass,captureFail:capFail||null,newCards,previewCount,previewDims:previewImgs.map(x=>[x.naturalWidth,x.naturalHeight]),uiMessage:message,pass:!!capPass&&newCards>0&&previewCount>0};
  ...
  step('비교분석 결과 생성');
  for(const r of results)r.rootCauseStage=classifyStage(r);
  const divergence=firstDivergence(results);const repeatRace=repeatRaceEvidence(results);const timingRace=repeatRace.some(x=>x.mixed);
  const summary=['TOOL001_REAL_USER_PHOTO_DIAGNOSTIC_V30_NATIVE_USER_FLOW_BROAD_CAUSE_SWEEP',`URL=${url}`,`CASES=${results.length}`,`GOOD_BAD_FIRST_DIVERGENCE=${divergence}`,`TIMING_RACE_EVIDENCE=${timingRace}`,`REPEAT_EVIDENCE=${JSON.stringify(repeatRace)}`];
  for(const r of results){const f=r.change?.files?.[0]||{};const lastReader=r.readerTimeline?.at(-1)||{};summary.push('',`[${r.label}]`,`PASS=${r.pass}`,`ROOT_CAUSE_STAGE=${r.rootCauseStage}`,`COMMIT_MODE=${r.commit?.mode||''}`,`SELECTION_SEEN=${r.commit?.sawSelection??false}`,`FILE_NAME=${f.name||''}`,`FILE_TYPE=${f.type||''}`,`FILE_SIZE=${f.size??''}`,`LAST_MODIFIED=${f.lastModified??''}`,`TRUSTED_CHANGE=${r.change?.trusted??''}`,`CAPTURE_PASS=${r.capturePass??false}`,`CAPTURE_FAIL=${r.captureFail?.error||''}`,`LAST_READER_PHASE=${lastReader.phase||''}`,`LAST_READER=${lastReader.reader||''}`,`LAST_READER_ERROR=${lastReader.error||''}`,`SIGNATURE=${r.byteSignature?.signature||''}`,`HAS_EXIF=${r.byteSignature?.hasExif??''}`,`WIDTH=${r.inspection?.width??''}`,`HEIGHT=${r.inspection?.height??''}`,`PIXELS=${r.inspection?.pixels??''}`,`ABOVE_40MP=${r.inspection?.aboveMaxPixels??''}`,`PRODUCT_FAIL_PHASE=${r.productFail?.phase||''}`,`PRODUCT_FAIL_REASON=${r.productFail?.reason||r.productFail?.error||''}`,`NEW_CARDS=${r.newCards??0}`,`PREVIEW_COUNT=${r.previewCount??0}`,`PREVIEW_DIMS=${JSON.stringify(r.previewDims||[])}`,`UI_MESSAGE=${r.uiMessage||''}`,`ERROR=${r.error||''}`);}
  write('summary.txt',summary.join('\n'));write('root-cause-summary.json',{goodBadFirstDivergence:divergence,timingRaceEvidence:timingRace,repeatEvidence:repeatRace,stageCounts:Object.fromEntries([...new Set(results.map(r=>r.rootCauseStage))].map(k=>[k,results.filter(r=>r.rootCauseStage===k).length]))});write('result.json',{version:'V30',url,generatedAt:new Date().toISOString(),uiLimits:{has20MB:ui20,has60MB:ui60},goodBadFirstDivergence:divergence,timingRaceEvidence:timingRace,repeatEvidence:repeatRace,results});
