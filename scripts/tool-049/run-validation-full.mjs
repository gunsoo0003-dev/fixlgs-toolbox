...
const resultDir=process.env.TOOL049_RESULT_DIR?path.resolve(process.env.TOOL049_RESULT_DIR):desktop;
fs.mkdirSync(resultDir,{recursive:true});
const stamp=new Date().toISOString().replace(/[:.]/g,'-');
const txt=path.join(resultDir,`049_${mode}_검수결과_${stamp}.txt`);
const rows=[];
...
const playwright=(spec)=>{
  ...
  if(isWin){
    return run(
      spec,
      `npx playwright test tests/tool-049-${spec}.spec.ts --workers=1 --config=playwright.tool049.config.ts --reporter=list`,
      [],
      {shell:true}
    );
  }
  return run(spec,'npx',['playwright','test',`tests/tool-049-${spec}.spec.ts`,'--workers=1','--config=playwright.tool049.config.ts','--reporter=list']);
};

if(mode==='preflight'){
  run('static-self-check',process.execPath,['scripts/tool-049/run-static-validation.mjs']);
