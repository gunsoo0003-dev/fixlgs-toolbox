...
const REPEATS_PER_MODE=Math.max(1,Number(args.repeats||10));
const MODES=['FILEREADER','ARRAYBUFFER','STREAM'];
const plan=[];
for(let round=1;round<=REPEATS_PER_MODE;round++) for(const mode of MODES) plan.push({photo:'PHOTO_01',round,mode,label:`PHOTO_01_${mode}_R${String(round).padStart(2,'0')}`});
...
const outDir=path.join(desktop,`TOOLBOX_001_NOTREADABLE_READPATH_AB_V37_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V37__'; if(window[KEY])return;
  const s=window[KEY]={changes:[],capture:[],nativeReads:[],consoleErrors:[],events:[],lifecycle:[],nav:[],inputLifecycle:[],aborts:[],decodeOps:[],objectUrls:[],readConcurrency:[],domMutations:[],readPrestate:[],experiments:[],currentMode:'FILEREADER'};
  const NativeFR=window.FileReader, nativeFRRead=NativeFR.prototype.readAsArrayBuffer, nativeAB=Blob.prototype.arrayBuffer, nativeStream=Blob.prototype.stream;
  window.__TOOL001_V37_SET_MODE__=(m)=>{s.currentMode=String(m||'FILEREADER');};
  const runExperimentalRead=async(file,mode,changeAt)=>{
    const rec={id:`exp-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,mode,changeAt,startAt:Date.now(),file:{name:file?.name||'',type:file?.type||'',size:Number(file?.size||0),lastModified:Number(file?.lastModified||0)},ok:false,error:'',bytes:0};
    s.experiments.push(rec);
    try{
      if(mode==='FILEREADER'){
        await new Promise((resolve,reject)=>{const fr=new NativeFR();fr.addEventListener('load',()=>{try{rec.bytes=fr.result instanceof ArrayBuffer?fr.result.byteLength:0;}catch{}resolve();},{once:true});fr.addEventListener('error',()=>reject(fr.error||new Error('FileReader error')),{once:true});fr.addEventListener('abort',()=>reject(new DOMException('Aborted','AbortError')),{once:true});nativeFRRead.call(fr,file);});
      } else if(mode==='ARRAYBUFFER'){
        const b=await nativeAB.call(file);rec.bytes=Number(b?.byteLength||0);
      } else if(mode==='STREAM'){
        const stream=nativeStream.call(file),reader=stream.getReader();let total=0;while(true){const {done,value}=await reader.read();if(done)break;total+=Number(value?.byteLength||value?.length||0);}rec.bytes=total;
      } else throw new Error('UNKNOWN_MODE:'+mode);
      rec.ok=true;
    }catch(e){rec.error=`${e?.name||'Error'}:${e?.message||String(e)}`;}
    rec.endAt=Date.now();rec.durationMs=rec.endAt-rec.startAt;return rec;
  };
  document.addEventListener('change',e=>{
    const el=e.target;if(!(el instanceof HTMLInputElement)||el.type!=='file')return;
    const files=Array.from(el.files||[]),change={at:Date.now(),trusted:e.isTrusted,fileCount:files.length,files:files.map(f=>({name:f.name,type:f.type,size:f.size,lastModified:f.lastModified}))};s.changes.push(change);
    if(!files.length)return;
    e.stopImmediatePropagation();
    void runExperimentalRead(files[0],s.currentMode,change.at);
  },true);
  let activeReads=0; const readEnter=(kind,extra={})=>{activeReads++;s.readConcurrency.push({at:Date.now(),kind,phase:'start',activeReads,...extra});}; const readExit=(kind,extra={})=>{s.readConcurrency.push({at:Date.now(),kind,phase:'end',activeReads,...extra});activeReads=Math.max(0,activeReads-1);};
  ...
  // V37 non-product instrumentation: observe native browser reads without consuming the File ourselves.
  ...
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V37_FILEREADER_OBSERVER:'+String(e?.message||e)}); }
  try {
  ...
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V37_STREAM_OBSERVER:'+String(e?.message||e)}); }
  try {
  ...
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V37_ARRAYBUFFER_OBSERVER:'+String(e?.message||e)}); }
  try {
  ...
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V37_ABORT_OBSERVER:'+String(e?.message||e)}); }
  try {
  ...
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V37_DECODE_OBSERVER:'+String(e?.message||e)}); }
  try {
  ...
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V37_INPUT_DOM_OBSERVER:'+String(e?.message||e)}); }
...
async function autoSelectPickerMedia(caseNo,label,round,page,startChange){
  ...
  while(Date.now()<dl){
    const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V37__))).catch(()=>null);
    ...
    if(!snap.detected){
      await sleep(300);
      const obs2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V37__))).catch(()=>null);
...
async function waitProduct(page,startCapture,startNative,before,timeoutMs=6500){const deadline=Date.now()+timeoutMs;let snap=before,cap=[],nr=[];while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V37__)));cap=st.capture.slice(startCapture);nr=(st.nativeReads||[]).slice(startNative);snap=await domSnapshot(page);const preview=snap.cards.flatMap(c=>c.imgs||[]).some(img=>img.complete&&img.naturalWidth>0&&img.naturalHeight>0);const cardChanged=snap.cardCount!==before.cardCount;const capFail=cap.some(x=>x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail'||x.phase==='product-add-fail');const nativeFail=nr.some(x=>/error|throw|abort/.test(x.phase||'')&&/(NotReadableError|could not be read|UPLOAD_FILE_CHANGED|ERR_)/i.test(String(x.error||'')));if((cardChanged&&preview)||capFail||nativeFail)break;await sleep(220);}return{capture:cap,nativeReads:nr,after:snap};}
async function collectFailureEvidence(caseNo,label,page,reason){let xml='';try{xml=dumpUi();}catch{}write(`case-${caseNo}-${label}-failure-picker.xml`,xml);await pcScreen(`case-${caseNo}-${label}-failure-device.png`);await page.screenshot({path:path.join(outDir,`case-${caseNo}-${label}-failure-page.png`),fullPage:true}).catch(()=>{});const snap=await domSnapshot(page).catch(()=>null);const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V37__))).catch(()=>null);write(`case-${caseNo}-${label}-failure-evidence.json`,{reason,pickerOpen:pickerDetectedFromXml(xml),selectionSignals:pickerSelectionSignals(xml).selectedNodes.length,commitControls:photoPickerActionControls(xml).map(a=>({exact:a.exact,label:a.label,bounds:a.bounds})),foreground:foregroundSnapshot(),observer:obs,dom:snap});}
...
async function waitExperiment(page,startCount,timeoutMs=8000){
  const dl=Date.now()+timeoutMs;
  while(Date.now()<dl){
    const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V37__)));
    if((st.experiments||[]).length>startCount){const x=st.experiments.at(-1);if(x.endAt)return x;}
    await sleep(120);
  }
  throw new Error('EXPERIMENT_RESULT_TIMEOUT');
}

async function main(){
  log('=== TOOL001 NOTREADABLE READ-PATH A/B V37 ===');log('[OUTPUT]',outDir);
  step('SELF-CHECK');const av=adb('version');if(av.status!==0)throw new Error('ADB_NOT_AVAILABLE');const devs=adbText('devices');if(!/\tdevice\b/.test(devs))throw new Error('ADB_DEVICE_NOT_READY');write('self-check.txt','SELF_CHECK=PASS\nMODE=SAME_PHOTO_READ_PATH_AB\nPRODUCT_CHANGE_HANDLER_BLOCKED_DURING_EXPERIMENT=YES\n');log('[PASS] SELF-CHECK');
  ...
  step('V37 read-path 실험기 주입');await page.addInitScript(browserInit);
  ...
  step('실험 안내');log('PHOTO_01 동일 사진만 사용. 각 round마다 FILEREADER -> ARRAYBUFFER -> STREAM 순으로 교차 실행합니다.');log(`각 방식 ${REPEATS_PER_MODE}회, 총 ${plan.length}회. 제품 change handler는 실험 중 차단하여 read API끼리만 비교합니다.`);
  const results=[];
  for(let i=0;i<plan.length;i++){
    const p=plan[i],caseNo=i+1,label=p.label;step(`CASE ${caseNo}/${plan.length} ${p.mode} R${p.round}`);log('');log(`===== CASE ${caseNo}/${plan.length}: ${label} =====`);
    await ensureExpectedToolPage(page,caseNo,'case-start');await page.bringToFront();
    await page.evaluate(mode=>window.__TOOL001_V37_SET_MODE__?.(mode),p.mode);
    const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V37__)));
    adb('logcat','-c');let r={case:caseNo,label,photo:'PHOTO_01',mode:p.mode,round:p.round,pass:false,error:'',commit:null,experiment:null};
    try{
      const commit=await timed(`CASE ${caseNo} picker flow`,()=>waitForSelectionAndCommit(caseNo,'PHOTO_01',page,st0.changes.length,context),65000);r.commit=commit;if(!commit.committed)throw new Error(`PHOTO_PICKER_COMMIT_FAILED:${commit.mode}`);
      const exp=await timed(`CASE ${caseNo} ${p.mode} native read`,()=>waitExperiment(page,(st0.experiments||[]).length,8000),9000);r.experiment=exp;r.pass=!!exp.ok&&exp.bytes>0;r.error=exp.error||'';
      const rawLogcat=adbText('logcat','-d','-v','time');const logcatEvidence=rawLogcat.split(/\r?\n/).filter(x=>/(NotReadableError|ERR_UPLOAD_FILE_CHANGED|UPLOAD_FILE_CHANGED|file could not be read|net::ERR_|chromium|FileReader)/i.test(x)).slice(-250);r.logcatEvidence=logcatEvidence;write(`case-${caseNo}-${label}-logcat.txt`,logcatEvidence.join('\n'));
      log(`[${r.pass?'PASS':'FAIL'}] ${p.mode} R${p.round} bytes=${exp.bytes} duration=${exp.durationMs}ms error=${exp.error||''}`);
    }catch(e){r.error=String(e?.message||e);log(`[FAIL] ${label} ${r.error}`);await collectFailureEvidence(caseNo,label,page,r.error).catch(()=>{});}
    results.push(r);write(`case-${caseNo}-${label}.json`,r);await pcScreen(`case-${caseNo}-${label}-device.png`);
    if(i<plan.length-1){await page.goto(url,{waitUntil:'domcontentloaded',timeout:45000});await page.locator(selector).first().waitFor({state:'attached',timeout:30000});}
  }
  step('A/B 결과 집계');
  const summary={};for(const mode of MODES){const xs=results.filter(r=>r.mode===mode),pass=xs.filter(r=>r.pass).length,nr=xs.filter(r=>/NotReadableError|could not be read/i.test(r.error||r.experiment?.error||'')).length,other=xs.length-pass-nr;summary[mode]={total:xs.length,pass,notReadableFail:nr,otherFail:other,passRate:xs.length?Number((pass/xs.length*100).toFixed(1)):0};}
  const ranked=Object.entries(summary).sort((a,b)=>b[1].pass-a[1].pass||a[1].notReadableFail-b[1].notReadableFail);
  const txt=['TOOL001 NOTREADABLE READ-PATH A/B V37','PHOTO=PHOTO_01',`REPEATS_PER_MODE=${REPEATS_PER_MODE}`,`TOTAL=${results.length}`,'','MODE SUMMARY'];for(const [m,s] of ranked)txt.push(`${m}: PASS=${s.pass}/${s.total} NOTREADABLE=${s.notReadableFail} OTHER_FAIL=${s.otherFail} RATE=${s.passRate}%`);txt.push('','ROWS');for(const r of results)txt.push(`R${String(r.round).padStart(2,'0')} ${r.mode} ${r.pass?'PASS':'FAIL'} bytes=${r.experiment?.bytes??0} duration=${r.experiment?.durationMs??''} error=${r.error||r.experiment?.error||''}`);
  write('read-path-ab-comparison.txt',txt.join('\n'));write('read-path-ab-comparison.json',{version:'V37',photo:'PHOTO_01',repeatsPerMode:REPEATS_PER_MODE,summary,results});write('summary.txt',txt.join('\n'));write('result.json',{version:'V37',mode:'NOTREADABLE_READ_PATH_AB',url,repeatsPerMode:REPEATS_PER_MODE,generatedAt:new Date().toISOString(),summary,results});
  log('=== READ PATH A/B ===');for(const [m,s] of ranked)log(`${m}: PASS=${s.pass}/${s.total} NOTREADABLE=${s.notReadableFail} OTHER=${s.otherFail}`);
  const perfect=ranked.filter(([,s])=>s.pass===s.total&&s.total>0).map(([m])=>m);log(`PERFECT_10_OF_10=${perfect.length?perfect.join(','):'NONE'}`);
  step('결과 ZIP 생성');restoreNotificationOverlays();await context.close().catch(()=>{});if(typeof device.close==='function')await device.close().catch(()=>{});adb('shell','svc','power','stayon','false');const zip=path.join(desktop,`${path.basename(outDir)}.zip`);const pr=spawnSync('powershell.exe',['-NoProfile','-Command',`Compress-Archive -Path '${outDir.replaceAll("'","''")}\\*' -DestinationPath '${zip.replaceAll("'","''")}' -Force`],{encoding:'utf8',shell:false});if(pr.status!==0)log('[WARN] ZIP creation failed',pr.stderr||pr.stdout||'');else log('[PASS] RESULT ZIP',zip);log('READ_PATH_AB_COMPARISON=read-path-ab-comparison.txt');
}
