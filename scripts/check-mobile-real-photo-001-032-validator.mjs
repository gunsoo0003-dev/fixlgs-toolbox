...
const scriptPath=process.env.MOBILE_VALIDATOR_SCRIPT || path.join(root,'scripts','run-mobile-real-photo-001-032.mjs');
...
if(toolNums.length!==32)errors.push(`tool definitions != 32 (${toolNums.length})`);
for(let i=1;i<=32;i++)if(!toolNums.includes(i))errors.push(`missing TOOL${String(i).padStart(3,'0')}`);
if(new Set(toolNums).size!==toolNums.length)errors.push('duplicate tool definition');
for(const x of ['TOOLBOX_MOBILE_REALPHOTO_001_032_${stamp}','fatal-harness.json','RESULT ZIP TARGET','archiveResultFolder(outDir,zipPath)']) if(!script.includes(x))errors.push(`missing result/archive contract: ${x}`);
...
if((script.match(/inputKind:'pdf-document'/g)||[]).length<6)errors.push('TOOL027/028/029/030/031/032 PDF document-input contracts missing');
...
console.log('[PASS] mobile real-photo 001~032 validator checklist/source/static self-check');
