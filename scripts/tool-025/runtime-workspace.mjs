...
for(const e of fs.readdirSync(root,{withFileTypes:true}))if(e.isDirectory()&&e.name.startsWith('.tool025-runtime-'))fs.rmSync(path.join(root,e.name),{recursive:true,force:true});
const runtimeRoot=fs.mkdtempSync(path.join(root,'.tool025-runtime-'));
for(const name of ['app','components','lib','styles','public','icons']){const src=path.join(root,name);if(fs.existsSync(src))fs.cpSync(src,path.join(runtimeRoot,name),{recursive:true});}
...
fs.writeFileSync(path.join(runtimeRoot,'next.config.ts'),`import type { NextConfig } from "next";\nconst nextConfig: NextConfig={allowedDevOrigins:["127.0.0.1","localhost"],distDir:".next-tool025-runtime",turbopack:{root:${JSON.stringify(escapedRoot)}}};\nexport default nextConfig;\n`);
...
if(mode==='build'){let status=1;try{const r=spawnSync(process.execPath,[nextCli,'build',runtimeRoot,'--webpack'],{cwd:root,stdio:'inherit',env:{...process.env,TOOL025_RUNTIME:'1'}});status=r.status??1;}finally{cleanup();}process.exit(status)}
const child=spawn(process.execPath,[nextCli,'dev',runtimeRoot,'--webpack','--hostname','127.0.0.1','--port','3025'],{cwd:root,stdio:'inherit',env:{...process.env,TOOL025_RUNTIME:'1'}});
