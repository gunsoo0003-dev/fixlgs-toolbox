...
const outDir = fs.mkdtempSync(path.join(os.tmpdir(), `tool025-${mode}-`));
const results = [];
...
const staticSteps = [
  ['final-checklist', node, ['scripts/tool-025/check-final-checklist.mjs']],
  ['runner-contract', node, ['scripts/tool-025/check-runner-contract.mjs']],
  ['source', node, ['scripts/tool-025/check-source.mjs']],
  ['harness-static', node, ['scripts/tool-025/check-harness.mjs']],
  ['integration', node, ['scripts/tool-025/check-integration.mjs']],
  ['design-transplant', node, ['scripts/tool-025/check-design-transplant.mjs']],
  ['content-density', node, ['scripts/tool-025/check-content-density.mjs']],
  ['syntax', node, ['scripts/tool-025/check-syntax.mjs']],
  ['common-design', node, ['scripts/check-toolbox-common-design.mjs']],
];

const runtime = (spec, name) => [name, node, [pwCli, 'test', spec, '--workers=1', '--config=playwright.tool025-runtime.config.ts']];
let steps = [...staticSteps];
if (mode === 'core-only') steps.push(runtime('tests/tool-025-core.spec.ts', 'core'));
if (mode === 'boundary-only') steps.push(runtime('tests/tool-025-boundary.spec.ts', 'boundary'));
if (mode === 'feature-only') {
  steps.push(runtime('tests/tool-025-feature.spec.ts', 'feature'));
  steps.push(runtime('tests/tool-025-design-state.spec.ts', 'feature-design-state'));
}
if (mode === 'regression-only') steps.push(runtime('tests/tool-025-regression.spec.ts', 'regression'));
if (mode === 'limit-only') steps.push(runtime('tests/tool-025-limit.spec.ts', 'limit'));
if (mode === 'preflight') steps.push(runtime('tests/tool-025-core.spec.ts', 'harness-minimal-runtime'));
if (mode === 'final') {
  steps.push(['typescript', node, [tscCli, '--noEmit']]);
  steps.push(['production-build', node, ['scripts/tool-025/runtime-workspace.mjs', 'build']]);
  steps.push(runtime('tests/tool-025-core.spec.ts', 'core'));
  steps.push(runtime('tests/tool-025-boundary.spec.ts', 'boundary'));
  steps.push(runtime('tests/tool-025-feature.spec.ts', 'feature'));
  steps.push(runtime('tests/tool-025-design-state.spec.ts', 'feature-design-state'));
  steps.push(runtime('tests/tool-025-regression.spec.ts', 'regression'));
  steps.push(runtime('tests/tool-025-limit.spec.ts', 'limit'));
}
...
try {
  ...
  if (missing.length) {
    ...
    console.log(`TOOL 025 ${mode.toUpperCase()} | total steps=${steps.length}`);
...
const summary = `TOOL 025 ${mode}\nPASS=${pass}\nFAIL=${fail}\nSKIP=0\nSTATUS=${ok ? 'PASS' : 'FAIL'}\n`;
try { fs.writeFileSync(path.join(outDir, 'summary.txt'), summary); } catch {}
try { fs.writeFileSync(path.join(outDir, 'summary.json'), JSON.stringify({ tool: '025', mode, pass, fail, skip: 0, status: ok ? 'PASS' : 'FAIL', results }, null, 2)); } catch {}

const desktop = path.join(process.env.USERPROFILE || os.homedir(), 'Desktop');
const outBase = process.env.TOOL025_RESULT_DIR ? path.resolve(process.env.TOOL025_RESULT_DIR) : desktop;
let zipPath = path.join(outBase, `025_${mode}_검수결과.zip`);
try {
