#!/usr/bin/env node
import fs from 'fs';
const runner=fs.readFileSync(new URL('./run-tool-001-real-photo-diagnostic-v27.mjs',import.meta.url),'utf8');
...
const checks={
  version:/REAL_PHOTO_V27|DIAGNOSTIC_V27|version:'V27'/.test(runner),
...
console.log(JSON.stringify({STATIC_SELFTEST:missing.length?'FAIL':'PASS',version:'V27',missing,...checks},null,2));
