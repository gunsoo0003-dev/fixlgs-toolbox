...
function resolvePlaywrightCommand() {
  ...
  if (existsSync(local)) {
    return { exe: process.execPath, args: [local, 'test', 'tests/tool-015-core.spec.ts', '--workers=1'] };
  }
  const npmCli = process.env.npm_execpath;
  if (npmCli && existsSync(npmCli)) {
    return { exe: process.execPath, args: [npmCli, 'exec', '--', 'playwright', 'test', 'tests/tool-015-core.spec.ts', '--workers=1'] };
  }
  if (process.platform === 'win32') {
    return { exe: 'cmd.exe', args: ['/d', '/s', '/c', 'npm', 'exec', '--', 'playwright', 'test', 'tests/tool-015-core.spec.ts', '--workers=1'] };
  }
  return { exe: 'npm', args: ['exec', '--', 'playwright', 'test', 'tests/tool-015-core.spec.ts', '--workers=1'] };
}
...
if (pkg.zipCreated) {
  const fixed = resolve(resolveDesktopPath(), '015_core-only_검수결과.zip');
  try {
...
console.log(`015 CORE-ONLY RESULT: ${status.toUpperCase()}`);
