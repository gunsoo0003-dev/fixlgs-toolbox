...
const outDir = path.join(desktop, `TOOLBOX_001_REAL_ANDROID_DEEP_V16_${stamp}`);
...
async function assertInteractiveUnlocked(label='lock-check') {
  ...
  if (s.blocked) {
    ...
    throw new Error(`SECURE_LOCK_ACTIVE at ${label}. Unlock the Galaxy once and rerun; V16 keeps the display awake after startup.`);
  }
  return s;
}
...
async function ensureDeviceReady(page, label='device ready') {
  ...
  if (isKeyguardShowing(snap)) {
    ...
    throw new Error('Device is still on a secure lock screen. Unlock the Galaxy once, then rerun; V16 keeps it awake afterward.');
  }
...
function dumpUi() {
  const remote='/sdcard/tool001-v16-window.xml';
...
async function saveDeviceScreen(name) {
  const remote='/sdcard/tool001-v16-screen.png';
...
async function findAndTapFile(filename, attemptIndex, page=null, timeoutMs=45000) {
  ...
  while(Date.now()-started<timeoutMs){
    ...
    if((snap.packages||[]).includes('com.google.android.photopicker') ||
      ...
      // Photo Picker sorts Photos by newest first. With V16 only one canonical copy is created,
...
async function prepareDeviceFiles(count) {
  ...
  for(let i=1;i<=count;i++){
    const name=`TOOL001_V16_${token}_${String(i).padStart(2,'0')}.png`;
...
async function runSelfCheck() {
  log('[SELF-CHECK] V16 validator integrity check starting');
  ...
  write('self-check.txt', [
    'SELF_CHECK=PASS',
    'BUILD=V16_FINAL_PICKER_COMMIT_AND_RACE_HARDENED',
...
function browserInit() {
  const KEY = '__TOOL001_REAL_ANDROID_V16__';
  if (window[KEY]) return;
  const state = window[KEY] = {
    version:'V16', bornEpoch:Date.now(), bornPerf:performance.now(), seq:0,
...
async function waitForAttempt(page, expected, timeout) {
  ...
  while(Date.now()-start<timeout){
    const s=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V16__?({count:window.__TOOL001_REAL_ANDROID_V16__.attempts.length,attempts:window.__TOOL001_REAL_ANDROID_V16__.attempts.map(a=>({attemptIndex:a.attemptIndex,done:a.done,fileCount:a.fileCount})),activeProbes:window.__TOOL001_REAL_ANDROID_V16__.activeProbes}):({count:0,attempts:[],activeProbes:0})).catch(()=>({count:0,attempts:[],activeProbes:0}));
...
async function main(){
  log('=== TOOL001 REAL ANDROID DEEP DIAGNOSTIC V16 ==='); log('[OUTPUT]',outDir);
  step('검수기 SELF-CHECK');
  ...
  await page.evaluate(sel=>{if(window.__TOOL001_REAL_ANDROID_V16__)window.__TOOL001_REAL_ANDROID_V16__.selector=sel;},selector);
  ...
  for(let i=1;i<=attemptsTarget;i++){
    ...
    try{
      ...
      const p=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V16__)));
      ...
      const p=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V16__?JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V16__)):null).catch(()=>null);
  ...
  const state=await withHeartbeat('collect browser diagnostic state',()=>page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V16__))),20000);
  ...
  write('result.json',{meta:{version:'V16',generatedAt:new Date().toISOString(),url,selector,attemptsTarget,attemptsObserved:state.attempts.length,node:process.version,hostPlatform:process.platform},environment:state.environment,attempts:state.attempts,timeline:state.timeline,domFinal});
  write('timeline.json',state.timeline);write('attempts.json',state.attempts);write('dom-final.json',domFinal);
  const s=['TOOL001_REAL_ANDROID_DEEP_DIAGNOSTIC_V16',`URL=${url}`,`ATTEMPTS_TARGET=${attemptsTarget}`,`ATTEMPTS_OBSERVED=${state.attempts.length}`,`USER_AGENT=${state.environment?.userAgent||''}`,`PLATFORM=${state.environment?.platform||''}`,`MAX_TOUCH_POINTS=${state.environment?.maxTouchPoints??''}`];
...
main().catch(async e=>{
  ...
  try{adb('shell','rm','-f','/sdcard/Download/TOOL001_V16_*.png');adb('shell','rm','-f','/sdcard/Pictures/TOOL001_V16_*.png');}catch{}
