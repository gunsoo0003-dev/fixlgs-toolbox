...
const outDir = path.join(desktop, `TOOLBOX_001_REAL_ANDROID_DEEP_V10_${stamp}`);
...
async function configureStayAwake() {
  ...
  const r = adb('shell','settings','put','global','stay_on_while_plugged_in','3');
  if (r.status !== 0) throw new Error(`Failed to enable stay-awake: ${r.stderr||r.stdout||''}`);
  adb('shell','input','keyevent','KEYCODE_WAKEUP');
  await sleep(400);
  log(`[DEVICE] stay_on_while_plugged_in: ${originalStayAwake || '(unset)'} -> 3`);
}
async function restoreStayAwake() {
  if (originalStayAwake == null) return;
...
async function ensureDeviceReady(page, label='device ready') {
  ...
  if (isKeyguardShowing(snap)) {
    ...
    throw new Error('Device is still on a secure lock screen. Unlock the Galaxy once, then rerun; V10 keeps it awake afterward.');
  }
...
function dumpUi() {
  const remote='/sdcard/tool001-v10-window.xml';
...
async function saveDeviceScreen(name) {
  const remote='/sdcard/tool001-v10-screen.png';
...
async function findAndTapFile(filename, attemptIndex, timeoutMs=35000) {
  ...
  while(Date.now()-started<timeoutMs) {
    ...
    if(!usedRecentFallback){
      const mediaQ=adbText('shell','content','query','--uri','content://media/external/images/media','--projection','_id:_display_name:date_added','--where',`_display_name="${filename}"`);
...
async function prepareDeviceFiles(count) {
  ...
  for(let i=1;i<=count;i++){
    const name=`TOOL001_V10_${token}_${String(i).padStart(2,'0')}.png`;
...
async function stageDeviceFile(tf, attemptIndex) {
  ...
  const media=adbText('shell','content','query','--uri','content://media/external/images/media','--projection','_id:_display_name:date_added','--where',`_display_name="${tf.name}"`);
...
async function clickUploadButton(page, context, attemptIndex) {
  ...
  if (!geom || geom.width <= 0 || geom.height <= 0) throw new Error(`Upload button has no tappable rect: ${JSON.stringify(geom)}`);
  const cssX=Math.round(geom.x+geom.width/2), cssY=Math.round(geom.y+geom.height/2);
  const strategies=[];

  // Strategy A: Chromium DevTools trusted touch in CSS viewport coordinates.
  try{
    const cdp=await context.newCDPSession(page);
    await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:cssX,y:cssY,radiusX:1,radiusY:1,force:1,id:1}]});
    await sleep(80);
    await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});
    await cdp.detach().catch(()=>{});
    strategies.push({method:'cdp-touch',ok:true,cssX,cssY});
    log(`[TOUCH] CDP trusted touch sent button="${geom.text}" css=(${cssX},${cssY})`);
    await sleep(800);
    if(await pickerStillOpen()) {
      log('[PASS] native picker opened via CDP trusted touch');
      return {method:'cdp-touch',cssX,cssY,geom,strategies};
    }
    strategies[strategies.length-1].picker=false;
    log('[WARN] CDP touch sent but picker not detected yet; trying ADB coordinate fallback');
  }catch(e){
    strategies.push({method:'cdp-touch',ok:false,error:String(e?.message||e)});
    log(`[WARN] CDP trusted touch unavailable: ${e?.message||e}`);
  }

  // Strategy B: ADB physical tap. Estimate browser top inset from physical screen vs CSS visual viewport.
  try{
    ...
    const visibleContentPx=Math.round(geom.visualH*cssToPx);
    const topInset=Math.max(0,sh-visibleContentPx);
    ...
    log(`[TOUCH] ADB fallback button="${geom.text}" css=(${cssX},${cssY}) physical=(${ax},${ay}) screen=${sw}x${sh} dpr=${geom.dpr} topInset≈${topInset}`);
    ...
    await sleep(850);
    if(await pickerStillOpen()){
      log('[PASS] native picker opened via ADB real-device tap');
      return {method:'adb-tap',x:ax,y:ay,geom,strategies};
    }
    strategies[strategies.length-1].picker=false;
  }catch(e){
    strategies.push({method:'adb-tap',ok:false,error:String(e?.message||e)});
    log(`[WARN] ADB tap fallback failed: ${e?.message||e}`);
  }

  // Strategy C: Playwright force click as a final browser-side fallback. It may still be useful on some Chrome builds.
  try{
    await btn.click({force:true,timeout:4000,noWaitAfter:true});
    strategies.push({method:'playwright-force',ok:true});
    log('[TOUCH] Playwright force click fallback sent');
    await sleep(700);
    if(await pickerStillOpen()){
      log('[PASS] native picker opened via Playwright force-click fallback');
      return {method:'playwright-force',geom,strategies};
    }
    strategies[strategies.length-1].picker=false;
  }catch(e){
    strategies.push({method:'playwright-force',ok:false,error:String(e?.message||e)});
    log(`[WARN] Playwright force-click fallback failed: ${e?.message||e}`);
  }

  write(`attempt-${String(attemptIndex).padStart(2,'0')}-tap-strategies.json`,strategies);
  throw new Error(`All upload-button activation strategies failed to open native picker: ${JSON.stringify(strategies)}`);
}


async function runSelfCheck() {
  log('[SELF-CHECK] V10 validator integrity check starting');
...
function browserInit() {
  const KEY = '__TOOL001_REAL_ANDROID_V10__';
  if (window[KEY]) return;
  const state = window[KEY] = {
    version:'V10', bornEpoch:Date.now(), bornPerf:performance.now(), seq:0,
...
async function waitForAttempt(page, expected, timeout) {
  ...
  while(Date.now()-start<timeout){
    const s=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V10__?({count:window.__TOOL001_REAL_ANDROID_V10__.attempts.length,attempts:window.__TOOL001_REAL_ANDROID_V10__.attempts.map(a=>({attemptIndex:a.attemptIndex,done:a.done,fileCount:a.fileCount})),activeProbes:window.__TOOL001_REAL_ANDROID_V10__.activeProbes}):({count:0,attempts:[],activeProbes:0})).catch(()=>({count:0,attempts:[],activeProbes:0}));
...
async function main(){
  log('=== TOOL001 REAL ANDROID DEEP DIAGNOSTIC V10 ==='); log('[OUTPUT]',outDir);
  step('검수기 SELF-CHECK');
  ...
  await page.evaluate(sel=>{if(window.__TOOL001_REAL_ANDROID_V10__)window.__TOOL001_REAL_ANDROID_V10__.selector=sel;},selector);
  ...
  for(let i=1;i<=attemptsTarget;i++){
    ...
    try{
      ...
      const p=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V10__)));
      ...
      const p=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V10__?JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V10__)):null).catch(()=>null);
  ...
  const state=await withHeartbeat('collect browser diagnostic state',()=>page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V10__))),20000);
  ...
  write('result.json',{meta:{version:'V10',generatedAt:new Date().toISOString(),url,selector,attemptsTarget,attemptsObserved:state.attempts.length,node:process.version,hostPlatform:process.platform},environment:state.environment,attempts:state.attempts,timeline:state.timeline,domFinal});
  write('timeline.json',state.timeline);write('attempts.json',state.attempts);write('dom-final.json',domFinal);
  const s=['TOOL001_REAL_ANDROID_DEEP_DIAGNOSTIC_V10',`URL=${url}`,`ATTEMPTS_TARGET=${attemptsTarget}`,`ATTEMPTS_OBSERVED=${state.attempts.length}`,`USER_AGENT=${state.environment?.userAgent||''}`,`PLATFORM=${state.environment?.platform||''}`,`MAX_TOUCH_POINTS=${state.environment?.maxTouchPoints??''}`];
