...
const outDir=path.join(desktop,`TOOLBOX_001_REAL_PHOTO_V22_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function dumpUi(){const remote='/sdcard/tool001-v22-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
async function autoCommitManualSelection(caseNo,page,startChange){const deadline=Date.now()+90000;let sawSelection=false;let commitTaps=0;let lastXml='';while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V22__)));if(st.changes.length>startChange)return{committed:true,mode:'picker-auto-close-change',sawSelection,commitTaps};let open=false;try{lastXml=dumpUi();open=pickerDetectedFromXml(lastXml);}catch{}if(!open){await sleep(250);const st2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V22__)));if(st2.changes.length>startChange)return{committed:true,mode:'picker-closed-change',sawSelection,commitTaps};await sleep(500);continue;}const sig=pickerSelectionSignals(lastXml);if(sig.selectedNodes.length){sawSelection=true;write(`case-${caseNo}-selection-observed.xml`,lastXml);const actions=photoPickerActionControls(lastXml);write(`case-${caseNo}-commit-controls.json`,actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds})));if(actions.length){for(const action of actions){const b=action.bounds;adb('shell','input','tap',String(b.x),String(b.y));commitTaps++;log(`[PHOTO_PICKER] case ${caseNo}: auto commit "${action.exact}" at ${b.x},${b.y}`);await pcScreen(`case-${caseNo}-after-auto-commit-${commitTaps}.png`);if(await pickerClosedStable(page,7000)){const st3=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V22__)));if(st3.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit',sawSelection:true,commitTaps,action:action.exact};await sleep(1500);const st4=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V22__)));if(st4.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit-delayed-change',sawSelection:true,commitTaps,action:action.exact};return{committed:false,mode:'picker-closed-no-change',sawSelection:true,commitTaps,action:action.exact};}}}}
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V22__'; if(window[KEY])return;
...
function pickerSignalSnapshot(){
  ...
  const detected=pickerDetectedFromXml(xml)||packages.some(p=>/(photopicker|documentsui|myfiles|providers\.media|filepicker|picker)/i.test(p))||fg.packages.some(p=>/(photopicker|documentsui|myfiles|providers\.media|filepicker|picker)/i.test(p));
  return{detected,xml,fg,packages};
}
async function waitForPickerRobust(caseNo,timeoutMs=18000){
  const started=Date.now();let last=null;const history=[];
  while(Date.now()-started<timeoutMs){
    last=pickerSignalSnapshot();
    history.push({at:Date.now(),detected:last.detected,fg:last.fg.joined,packages:last.packages});
    if(last.detected){write(`case-${caseNo}-picker-detect-history.json`,history);return last.xml;}
    await sleep(250);
  }
  write(`case-${caseNo}-picker-detect-history.json`,history);
  write(`case-${caseNo}-picker-not-detected.xml`,last?.xml||'');
  throw new Error('PHOTO_PICKER_NOT_DETECTED_AFTER_ALL_ACTIVATION_STRATEGIES');
}
async function tapUpload(page,context,caseNo){
  await ensureChromeInteractive(page,caseNo,'before-upload');
  ...
  write(`case-${caseNo}-upload-geometry.json`,geom);
  const strategies=[];
  async function openedWithin(ms,method){const start=Date.now();while(Date.now()-start<ms){const s=pickerSignalSnapshot();if(s.detected){strategies.push({method,picker:true,fg:s.fg.joined,packages:s.packages});write(`case-${caseNo}-activation-strategies.json`,strategies);return true;}await sleep(220);}return false;}
  async function mark(method,extra={}){strategies.push({method,...extra});write(`case-${caseNo}-activation-strategies.json`,strategies);}

  // A. CDP trusted pointer events in CSS viewport coordinates.
  try{
    const cdp=await Promise.race([context.newCDPSession(page),new Promise((_,rej)=>setTimeout(()=>rej(new Error('CDP_SESSION_TIMEOUT')),2500))]);
    await cdp.send('Input.dispatchMouseEvent',{type:'mousePressed',x:Math.round(geom.cx),y:Math.round(geom.cy),button:'left',clickCount:1});
    await cdp.send('Input.dispatchMouseEvent',{type:'mouseReleased',x:Math.round(geom.cx),y:Math.round(geom.cy),button:'left',clickCount:1});
    await cdp.detach().catch(()=>{});await mark('cdp-mouse',{ok:true,x:Math.round(geom.cx),y:Math.round(geom.cy)});if(await openedWithin(3500,'cdp-mouse'))return{method:'cdp-mouse',geom,strategies};
  }catch(e){await mark('cdp-mouse',{ok:false,error:String(e?.message||e)});}

  // B. Physical ADB tap with visual viewport -> physical screen conversion.
  try{
    await ensureChromeInteractive(page,caseNo,'before-adb-tap');
    const wm=adbText('shell','wm','size');const m=wm.match(/(?:Physical size|Override size):\s*(\d+)x(\d+)/i)||wm.match(/(\d+)x(\d+)/);if(!m)throw new Error(`SCREEN_SIZE_PARSE_FAIL:${wm}`);
    const sw=Number(m[1]),sh=Number(m[2]);
    const cssScale=sw/Math.max(1,geom.innerW);
    const visiblePx=Math.round(geom.visualH*cssScale);
    const topInset=Math.max(0,sh-visiblePx);
    const ax=Math.max(1,Math.min(sw-1,Math.round((geom.cx+geom.visualOffsetLeft)*cssScale)));
    const ay=Math.max(1,Math.min(sh-1,Math.round(topInset+(geom.cy-geom.visualOffsetTop)*cssScale)));
    const r=adb('shell','input','tap',String(ax),String(ay));if(r.status!==0)throw new Error('ADB_TAP_FAILED');
    await mark('adb-visual-viewport',{ok:true,x:ax,y:ay,topInset,sw,sh,cssScale});if(await openedWithin(4000,'adb-visual-viewport'))return{method:'adb-visual-viewport',geom,strategies};
  }catch(e){await mark('adb-visual-viewport',{ok:false,error:String(e?.message||e)});}

  // C. Playwright force click. This preserves React handler path even when stability heuristics are flaky.
  try{await btn.click({force:true,timeout:3000,noWaitAfter:true});await mark('playwright-force',{ok:true});if(await openedWithin(3200,'playwright-force'))return{method:'playwright-force',geom,strategies};}catch(e){await mark('playwright-force',{ok:false,error:String(e?.message||e)});}

  // D. Browser-side DOM click as last activation fallback.
  try{await Promise.race([btn.evaluate(el=>el.click()),new Promise((_,rej)=>setTimeout(()=>rej(new Error('DOM_CLICK_TIMEOUT')),1500))]);await mark('dom-click',{ok:true});if(await openedWithin(3000,'dom-click'))return{method:'dom-click',geom,strategies};}catch(e){await mark('dom-click',{ok:false,error:String(e?.message||e)});}

  // E. One fresh scroll/geometry retry in case Chrome toolbar collapsed/expanded during prior attempts.
  try{
    await ensureChromeInteractive(page,caseNo,'before-fresh-retry');await btn.evaluate(el=>el.scrollIntoView({block:'center',inline:'center',behavior:'instant'}));await sleep(500);
    const b=await btn.boundingBox();if(!b)throw new Error('NO_BOUNDS_FRESH_RETRY');const x=Math.round(b.x+b.width/2),y=Math.round(b.y+b.height/2);
    const cdp=await context.newCDPSession(page);await cdp.send('Input.dispatchMouseEvent',{type:'mousePressed',x,y,button:'left',clickCount:1});await cdp.send('Input.dispatchMouseEvent',{type:'mouseReleased',x,y,button:'left',clickCount:1});await cdp.detach().catch(()=>{});
    await mark('fresh-cdp-retry',{ok:true,x,y});if(await openedWithin(4500,'fresh-cdp-retry'))return{method:'fresh-cdp-retry',geom,strategies};
  }catch(e){await mark('fresh-cdp-retry',{ok:false,error:String(e?.message||e)});}

  await pcScreen(`case-${caseNo}-activation-failed-device.png`);write(`case-${caseNo}-activation-failed-ui.xml`,dumpUi());
  throw new Error(`UPLOAD_ACTIVATION_EXHAUSTED:${JSON.stringify(strategies)}`);
}
async function waitProduct(page,startCapture,before,timeoutMs=90000){const deadline=Date.now()+timeoutMs;let snap=before,cap=[];while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V22__)));cap=st.capture.slice(startCapture);snap=await domSnapshot(page);const terminal=cap.some(x=>x.phase==='picker-file-pass'||x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail');const cardChanged=snap.cardCount!==before.cardCount;if(terminal&&(cardChanged||cap.some(x=>x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail')))break;await sleep(350);}return{capture:cap,after:snap};}
async function collectFailureEvidence(caseNo,label,page,reason){let xml='';try{xml=dumpUi();}catch{}write(`case-${caseNo}-${label}-failure-picker.xml`,xml);await pcScreen(`case-${caseNo}-${label}-failure-device.png`);await page.screenshot({path:path.join(outDir,`case-${caseNo}-${label}-failure-page.png`),fullPage:true}).catch(()=>{});const snap=await domSnapshot(page).catch(()=>null);const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V22__))).catch(()=>null);write(`case-${caseNo}-${label}-failure-evidence.json`,{reason,pickerOpen:pickerDetectedFromXml(xml),selectionSignals:pickerSelectionSignals(xml).selectedNodes.length,commitControls:photoPickerActionControls(xml).map(a=>({exact:a.exact,label:a.label,bounds:a.bounds})),foreground:foregroundSnapshot(),observer:obs,dom:snap});}

async function main(){
  log('=== TOOL001 REAL USER PHOTO DIAGNOSTIC V22 FINAL-INTEGRATED ===');log('[OUTPUT]',outDir);
  ...
  step('실사진 비교검수 안내');log('');log('*** 검수기가 테스트 사진을 만들지 않습니다. ***');log('각 CASE에서 안내된 GOOD/BAD 실제 사진을 Galaxy Photo Picker에서 1장만 탭하세요.');log('완료 버튼은 누르지 않아도 됩니다. V22이 V19 검증 로직으로 자동 commit합니다.');log('GOOD = 평소 첨부되는 실제 사진 / BAD = 평소 첨부되지 않는 실제 사진');log('');
  const results=[];
  for(let i=0;i<labels.length;i++){
    const caseNo=i+1,label=labels[i];step(`실사진 CASE ${caseNo}/${labels.length} ${label}`);log('');log(`===== CASE ${caseNo}/${labels.length}: ${label} =====`);log(`[ACTION REQUIRED] Galaxy에서 ${label.startsWith('BAD')?'평소 실패하는':'평소 성공하는'} 실제 사진/스크린샷 1장을 한 번 탭하세요. 완료는 V22이 자동 처리합니다.`);
    await page.bringToFront();const before=await domSnapshot(page);const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V22__)));let result={case:caseNo,label,pass:false,error:null,before,commit:null};
    try{
      const activation=await tapUpload(page,context,caseNo);result.activation=activation;const pickerXml=await waitForPickerRobust(caseNo,18000);write(`case-${caseNo}-picker-open.xml`,pickerXml);await pcScreen(`case-${caseNo}-picker-open.png`);
      const commit=await timed(`CASE ${caseNo} user selection + auto commit`,()=>autoCommitManualSelection(caseNo,page,st0.changes.length),120000);result.commit=commit;
      if(!commit.committed)throw new Error(`PHOTO_PICKER_COMMIT_FAILED:${commit.mode}`);
      const changeDeadline=Date.now()+15000;let change=null;while(Date.now()<changeDeadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V22__)));if(st.changes.length>st0.changes.length){change=st.changes.at(-1);break;}await sleep(250);}if(!change)throw new Error('NO_FILE_CHANGE_AFTER_VERIFIED_COMMIT');
