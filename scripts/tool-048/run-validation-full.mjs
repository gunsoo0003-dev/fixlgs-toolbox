...
const resultDir=process.env.TOOL048_RESULT_DIR?path.resolve(process.env.TOOL048_RESULT_DIR):desktop;
fs.mkdirSync(resultDir,{recursive:true});
const stamp=new Date().toISOString().replace(/[:.]/g,'-');
const txt=path.join(resultDir,`048_${mode}_검수결과_${stamp}.txt`);
const rows=[];
...
const playwright=(spec)=>{
  ...
  if(isWin){
    return run(
      spec,
      `npx playwright test tests/tool-048-${spec}.spec.ts --workers=1 --config=playwright.tool048.config.ts --reporter=list`,
      [],
      {shell:true}
    );
  }
  return run(spec,'npx',['playwright','test',`tests/tool-048-${spec}.spec.ts`,'--workers=1','--config=playwright.tool048.config.ts','--reporter=list']);
};

if(mode==='preflight'){
  run('static-self-check',process.execPath,['scripts/tool-048/run-static-validation.mjs']);
