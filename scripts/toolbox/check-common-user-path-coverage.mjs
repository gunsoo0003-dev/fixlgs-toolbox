...
const optimizedSpecPath = path.join(root, 'tests/common-user-path-full-optimized.spec.ts');
const fastSpecPath = path.join(root, 'tests/common-user-path-fast.spec.ts');
...
const optimizedSpec = fs.readFileSync(optimizedSpecPath, 'utf8');
const fastSpec = fs.readFileSync(fastSpecPath, 'utf8');
...
for (const required of ['COMMON ROUTE / LOCALE / RUNTIME / RELOAD','mobile drag/drop is N/A','probeSelectedImageFile']) {
  if (!spec.includes(required)) findings.push({ state:'COVERAGE_MISSING', tool:'COMMON', reason:`missing suite: ${required}` });
}
if (!optimizedSpec.includes('probeSelectedImageFile')) findings.push({ state:'COVERAGE_MISSING', tool:'COMMON', reason:'FULL optimized image decode probe missing' });
if (!fastSpec.includes('probeSelectedImageFile')) findings.push({ state:'COVERAGE_MISSING', tool:'COMMON', reason:'FAST image decode probe missing' });
