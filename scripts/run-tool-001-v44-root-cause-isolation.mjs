...
const outDir=path.join(desktop,`TOOLBOX_001_V44_ROOT_CAUSE_3WAY_REAL_DEVICE_${stamp}`);
...
function dumpUi(){const remote='/sdcard/tool001-v44-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
const MINIMAL_HTML=`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>TOOL001 V44 Chrome Byte Read</title></head><body style="font-family:sans-serif;padding:28px"><h2>TOOL001 Chrome Minimal File Read</h2><p>No TOOL001 / React / Next.js. One visible file input, one picker selection, one arrayBuffer read.</p><input id="file" type="file" accept="image/*" style="font-size:20px;padding:14px 4px"><pre id="out"></pre><script>
...
async function runChromeMinimal(){
  ...
  for(let r=1;r<=repeats;r++){
    ...
    log(`[${row.harnessFail?'HARNESS':row.pass?'PASS':'FAIL'}] Chrome minimal R${r} stage=${row.diag?.stage||'-'} bytes=${row.diag?.bytes||0} uriWmLock=${row.systemEvidence.uriPermissionWmLockErrors} error=${row.error||'-'}`);
...
function templateHash(){
  const a=fs.readFileSync(path.join(here,'tool-001-v44-native-template','main.dart'));
  const b=fs.readFileSync(path.join(here,'tool-001-v44-native-template','MainActivity.kt'));
  return crypto.createHash('sha256').update(a).update(b).digest('hex');
}

function prepareNativeApk(){
  const cacheRoot=path.join(os.tmpdir(),'tool001-v44-native-cache');
  ...
  if(!args['rebuild-native']&&fs.existsSync(apkCache)&&fs.existsSync(hashFile)&&fs.readFileSync(hashFile,'utf8').trim()===wantHash){
    log('[NATIVE] using cached V44 diagnostic APK');
    return apkCache;
  }
  log('[NATIVE] building V44 diagnostic APK (first run only)');
  ...
  const work=path.join(os.tmpdir(),'tool001-v44-native-build');
  ...
  fs.copyFileSync(path.join(here,'tool-001-v44-native-template','main.dart'),path.join(work,'lib','main.dart'));
  ...
  fs.copyFileSync(path.join(here,'tool-001-v44-native-template','MainActivity.kt'),kt);
...
function installNativeApk(apk){
  log('[NATIVE] installing V44 diagnostic APK');
...
async function runNativeMode(mode,label){
  ...
  for(let r=1;r<=repeats;r++){
    ...
    await sleep(180);
    const start=adb('shell','am','start','-W','--activity-new-task','--activity-clear-task','-n',`${pkg}/${act}`,'--ei','round',String(r),'--es','mode',mode);
    ...
    write(`case-${caseNo}-logcat-filtered.txt`,filteredEvidence(lc));
    await screen(`case-${caseNo}-device.png`);
    log(`[${row.harnessFail?'HARNESS':row.pass?'PASS':'FAIL'}] Android native ${mode} R${r} bytes=${row.bytes||0} uriWmLock=${row.systemEvidence.uriPermissionWmLockErrors} error=${row.error||'-'}`);
    rows.push(row);
    adb('shell','am','force-stop',pkg);
    await sleep(250);
  }
...
async function main(){
  log('=== TOOL001 V44 ROOT CAUSE 3-WAY ISOLATION ===');
  ...
  write('v44-root-cause-summary.txt',summary);
  write('result.json',{version:'V44-ROOT-CAUSE-3WAY',repeats,chrome,nativeGet,nativePick,interpretation,chromeRows,nativeGetRows,nativePickRows});
  log(summary);
  await makeZip();
}
