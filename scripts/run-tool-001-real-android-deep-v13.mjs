...
const outDir = path.join(desktop, `TOOLBOX_001_REAL_ANDROID_DEEP_V13_${stamp}`);
...
async function assertInteractiveUnlocked(label='lock-check') {
  ...
  if (s.blocked) {
    ...
    throw new Error(`SECURE_LOCK_ACTIVE at ${label}. Unlock the Galaxy once and rerun; V13 keeps the display awake after startup.`);
  }
  return s;
}
...
async function ensureDeviceReady(page, label='device ready') {
  ...
  if (isKeyguardShowing(snap)) {
    ...
    throw new Error('Device is still on a secure lock screen. Unlock the Galaxy once, then rerun; V13 keeps it awake afterward.');
  }
...
function dumpUi() {
  const remote='/sdcard/tool001-v13-window.xml';
...
async function saveDeviceScreen(name) {
  const remote='/sdcard/tool001-v13-screen.png';
...
function pickerSelectionSignals(xml) {
  ...
  const selectedNodes=nodes.filter(n=>
    n.checked==='true' ||
    (n.selected==='true' && !/content-desc="사진"/i.test(JSON.stringify(n))) ||
    /(선택됨|selected|1개 선택|1 selected|체크됨|checked)/i.test(nodeHay(n))
  );
  const confirmNodes=nodes.filter(n=>
    /(추가|완료|선택|열기|확인|add|done|select|open|choose|use)/i.test(nodeHay(n)) &&
    !/(취소|cancel|닫기|close)/i.test(nodeHay(n)) &&
    centerOfBounds(n.bounds)
  );
  return {selectedNodes,confirmNodes};
}
async function verifyAndCommitPhotoPickerSelection(attemptIndex, beforeXml, tappedCell) {
  const before=pickerSelectionSignals(beforeXml);
  const cell=tappedCell.bounds;
  const positions=[
    {name:'center',x:cell.x,y:cell.y},
    // Many multi-select pickers expose a checkbox affordance at the top-right of the tile.
  ...
  for(let pi=0;pi<positions.length;pi++){
    ...
    if(!(await pickerStillOpen())){
      log(`[PASS] Photo Picker closed after ${p.name} tap; native selection committed`);
      return {committed:true,mode:`picker-auto-close-${p.name}`};
    }
    ...
    // A selected-state change or newly surfaced confirmation control proves that the tile was selected.
    const stateChanged =
      sig.selectedNodes.length>before.selectedNodes.length ||
      sig.confirmNodes.length>before.confirmNodes.length ||
      /(1개|1 selected|선택됨|selected)/i.test(xml);

    if(stateChanged || sig.confirmNodes.length){
      log(`[PASS] Photo Picker selection state detected after ${p.name} tap; selectedSignals=${sig.selectedNodes.length} confirmSignals=${sig.confirmNodes.length}`);

      const candidates=sig.confirmNodes
        .map(n=>({n,c:centerOfBounds(n.bounds),h:nodeHay(n)}))
        .filter(x=>x.c && x.c.w>=40 && x.c.h>=40)
        .sort((a,b)=>b.c.y-a.c.y); // bottom action button first
      if(candidates.length){
        const hit=candidates[0];
        const r=adb('shell','input','tap',String(hit.c.x),String(hit.c.y));
        if(r.status===0){
          log(`[PHOTO_PICKER] tapped commit control at (${hit.c.x},${hit.c.y}) ${hit.h.slice(0,100)}`);
          const start=Date.now();
          while(Date.now()-start<5000){
            await sleep(300);
            if(!(await pickerStillOpen())){
              log('[PASS] Photo Picker closed after commit control');
              return {committed:true,mode:'selection-plus-confirm'};
            }
          }
        }
      }

      // Some builds commit immediately but animation/package transition is slow.
      await sleep(900);
      if(!(await pickerStillOpen())) return {committed:true,mode:'selection-transition'};
    }
  }

  // Final robust fallback: long-press the verified media tile to enter selection mode,
  // then look for selection/commit UI. Never long-press arbitrary picker controls.
  ...
  if(!(await pickerStillOpen())) return {committed:true,mode:'longpress-auto-close'};
  const xml=dumpUi();
  write(`attempt-${String(attemptIndex).padStart(2,'0')}-selection-longpress.xml`,xml);
  const sig=pickerSelectionSignals(xml);
  if(sig.confirmNodes.length){
    const hit=sig.confirmNodes.map(n=>({n,c:centerOfBounds(n.bounds),h:nodeHay(n)}))
      .filter(x=>x.c).sort((a,b)=>b.c.y-a.c.y)[0];
    if(hit){
      adb('shell','input','tap',String(hit.c.x),String(hit.c.y));
      log(`[PHOTO_PICKER] long-press mode commit tap (${hit.c.x},${hit.c.y})`);
      await sleep(1000);
      if(!(await pickerStillOpen())) return {committed:true,mode:'longpress-plus-confirm'};
    }
  }

  return {committed:false,mode:'no-selection-state'};
}
...
async function findAndTapFile(filename, attemptIndex, timeoutMs=30000) {
  ...
  while(Date.now()-started<timeoutMs){
    ...
    if((snap.packages||[]).includes('com.google.android.photopicker') ||
      ...
      // Photo Picker sorts Photos by newest first. With V13 only one canonical copy is created,
      ...
      const commit=await verifyAndCommitPhotoPickerSelection(attemptIndex,xml,cell);
...
async function prepareDeviceFiles(count) {
  ...
  for(let i=1;i<=count;i++){
    const name=`TOOL001_V13_${token}_${String(i).padStart(2,'0')}.png`;
...
async function runSelfCheck() {
  log('[SELF-CHECK] V13 validator integrity check starting');
  ...
  write('self-check.txt', [
    'SELF_CHECK=PASS',
    'BUILD=V13_REBUILT_FROM_V10_KNOWN_STARTABLE_BASE',
...
function browserInit() {
  const KEY = '__TOOL001_REAL_ANDROID_V13__';
  if (window[KEY]) return;
  const state = window[KEY] = {
    version:'V13', bornEpoch:Date.now(), bornPerf:performance.now(), seq:0,
...
async function waitForAttempt(page, expected, timeout) {
  ...
  while(Date.now()-start<timeout){
    const s=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V13__?({count:window.__TOOL001_REAL_ANDROID_V13__.attempts.length,attempts:window.__TOOL001_REAL_ANDROID_V13__.attempts.map(a=>({attemptIndex:a.attemptIndex,done:a.done,fileCount:a.fileCount})),activeProbes:window.__TOOL001_REAL_ANDROID_V13__.activeProbes}):({count:0,attempts:[],activeProbes:0})).catch(()=>({count:0,attempts:[],activeProbes:0}));
...
async function main(){
  log('=== TOOL001 REAL ANDROID DEEP DIAGNOSTIC V13 ==='); log('[OUTPUT]',outDir);
  step('검수기 SELF-CHECK');
  ...
  await page.evaluate(sel=>{if(window.__TOOL001_REAL_ANDROID_V13__)window.__TOOL001_REAL_ANDROID_V13__.selector=sel;},selector);
  ...
  for(let i=1;i<=attemptsTarget;i++){
    ...
    try{
      ...
      await withHeartbeat(`attempt ${i}: wait trusted change + deep file probe`,()=>waitForAttempt(page,i,Math.min(waitChangeMs,30000)),Math.min(waitChangeMs,30000)+5000,5000);
      ...
      const p=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V13__)));
      ...
      const p=await page.evaluate(()=>window.__TOOL001_REAL_ANDROID_V13__?JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V13__)):null).catch(()=>null);
  ...
  const state=await withHeartbeat('collect browser diagnostic state',()=>page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_ANDROID_V13__))),20000);
  ...
  write('result.json',{meta:{version:'V13',generatedAt:new Date().toISOString(),url,selector,attemptsTarget,attemptsObserved:state.attempts.length,node:process.version,hostPlatform:process.platform},environment:state.environment,attempts:state.attempts,timeline:state.timeline,domFinal});
  write('timeline.json',state.timeline);write('attempts.json',state.attempts);write('dom-final.json',domFinal);
  const s=['TOOL001_REAL_ANDROID_DEEP_DIAGNOSTIC_V13',`URL=${url}`,`ATTEMPTS_TARGET=${attemptsTarget}`,`ATTEMPTS_OBSERVED=${state.attempts.length}`,`USER_AGENT=${state.environment?.userAgent||''}`,`PLATFORM=${state.environment?.platform||''}`,`MAX_TOUCH_POINTS=${state.environment?.maxTouchPoints??''}`];
...
main().catch(async e=>{
  ...
  try{adb('shell','rm','-f','/sdcard/Download/TOOL001_V13_*.png');adb('shell','rm','-f','/sdcard/Pictures/TOOL001_V13_*.png');}catch{}
