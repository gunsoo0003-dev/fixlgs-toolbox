...
const labels=String(args.cases||'PHOTO_01,PHOTO_01,PHOTO_01,PHOTO_01,PHOTO_01').split(',').map(x=>x.trim()).filter(Boolean);
...
const outDir=path.join(desktop,`TOOLBOX_001_REAL_PHOTO_V33_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function dumpUi(){const remote='/sdcard/tool001-v33-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V33__'; if(window[KEY])return;
  const s=window[KEY]={changes:[],capture:[],nativeReads:[],consoleErrors:[],events:[],lifecycle:[],nav:[]};
  // V33 non-product instrumentation: observe native browser reads without consuming the File ourselves.
    ...
    if(!orig.__tool001v33Wrapped){
      const wrapped=function(blob){
        ...
        const done=(phase)=>{
          ...
          s.nativeReads.push({id,at:Date.now(),phase,error:err,bytes,readyState:this.readyState,size:meta.size,type:meta.type});
        };
        ...
        try{return orig.call(this,blob);}catch(e){
          s.nativeReads.push({id,at:Date.now(),phase:'filereader-throw',error:String(e?.name||'Error')+':'+String(e?.message||e),size:meta.size,type:meta.type});
          throw e;
        }
      };
      Object.defineProperty(wrapped,'__tool001v33Wrapped',{value:true});
      proto.readAsArrayBuffer=wrapped;
    }
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V33_FILEREADER_OBSERVER:'+String(e?.message||e)}); }
  try {
    const proto=Blob.prototype, orig=proto.stream;
    if(typeof orig==='function'&&!orig.__tool001v33Wrapped){
  ...
  } catch(e) { s.consoleErrors.push({at:Date.now(),message:'V33_STREAM_OBSERVER:'+String(e?.message||e)}); }
...
async function ensureChromeInteractive(page,caseNo,tag='guard'){
  ...
  // V33: NEVER infer overflow menu from toolbar "새 탭".
...
async function autoSelectPickerMedia(caseNo,label,round,page,startChange){
  ...
  while(Date.now()<dl){
    const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V33__))).catch(()=>null);
    ...
    if(!snap.detected){
      await sleep(300);
      const obs2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V33__))).catch(()=>null);
...
async function main(){
  log('=== TOOL001 REAL PHOTO HARNESS V33 FACTS-ONLY ===');log('[OUTPUT]',outDir);
  ...
  step('실기기 사용자흐름 검수 안내');log('');log('*** 검수기는 사용자 흐름만 자동 재현하고 사실만 기록합니다. ***');log('사용자 입력 없음: Photo Picker의 PHOTO_01 동일 사진을 기본 5회 연속 자동 선택합니다.');log('');log('GOOD/BAD를 미리 가정하지 않습니다. 동일 사진의 회차별 SUCCESS/FAIL과 최초 분기 증거만 기록합니다.');log('');
  const results=[];
  for(let i=0;i<labels.length;i++){
    ...
    await ensureExpectedToolPage(page,caseNo,'case-start');await page.bringToFront();const before=await domSnapshot(page);const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V33__))); adb('logcat','-c'); let result={case:caseNo,label,pass:false,error:null,before,commit:null};
    try{
      ...
      const changeDeadline=Date.now()+8000;let change=null;while(Date.now()<changeDeadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V33__)));if(st.changes.length>st0.changes.length){change=st.changes.at(-1);break;}await sleep(250);}if(!change)throw new Error('NO_FILE_CHANGE_AFTER_VERIFIED_COMMIT');
      ...
      const obsNow=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V33__)));
      const nativeReads=(obsNow.nativeReads||[]).slice((st0.nativeReads||[]).length);
      ...
      write(`case-${caseNo}-${label}-native-reads.json`,nativeReads);
      write(`case-${caseNo}-${label}-logcat-evidence.txt`,logcatEvidence.join('\n'));
      const userPass=newCards>0&&previewCount>0;
      result={...result,change,capture:r.capture,nativeReads,logcatEvidence,readerTimeline:readers,byteSignature:sniff||null,inspection,providerReceived,productFail,after:r.after,capturePass:!!capPass,captureFail:capFail||null,newCards,previewCount,previewDims:previewImgs.map(x=>[x.naturalWidth,x.naturalHeight]),uiMessage:message,pass:userPass};
  ...
  step('검수 사실기록 생성');
  const summary=['TOOL001_REAL_PHOTO_HARNESS_V33_FACTS_ONLY',`URL=${url}`,`CASES=${results.length}`];
  ...
  write('result.json',{version:'V33',mode:'HARNESS_FACTS_ONLY',url,generatedAt:new Date().toISOString(),results});
  step('최종 사실표');
  const allPass=results.every(r=>r.pass);
  write('verdict.json',{
    allFlowPass:allPass,
    repeatedPhoto:'PHOTO_01',
    firstDivergence:firstDivergence(results),
