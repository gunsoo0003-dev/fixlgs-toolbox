#!/usr/bin/env node
import fs from 'fs';
const runner=fs.readFileSync(new URL('./run-tool-001-real-photo-diagnostic-v26.mjs',import.meta.url),'utf8');
...
const checks={
  version:/REAL_PHOTO_V26|DIAGNOSTIC_V26|version:'V26'/.test(runner),
  ...
  overflow_menu_recovery:/Chrome overflow menu detected/.test(runner),
  ...
  short_selection_timeout:/selectionDeadline=Date\.now\(\)\+30000/.test(runner),
...
console.log(JSON.stringify({STATIC_SELFTEST:missing.length?'FAIL':'PASS',version:'V26',missing,...checks},null,2));
