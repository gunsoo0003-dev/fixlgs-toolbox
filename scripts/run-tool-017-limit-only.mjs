...
const full = [...cmd.prefix, 'run', 'test:toolbox:017-limit'];
...
const steps = [{ name: 'limit-only', status, exitCode: code, durationSeconds: Math.round((Date.now() - started) / 100) / 10, counts: parseCounts(text), command: `${cmd.exe} ${full.join(' ')}`, stdout, stderr: run.error ? `${stderr}\n${run.error.message}` : stderr }];
const pkg = createValidationResultPackage({ toolNumber: '017', validationType: 'limit-only', status, startedAt, endedAt: new Date(), steps });
cleanupProjectValidationArtifacts();
let finalZip = pkg.zipPath;
if (pkg.zipCreated) {
  const fixed = resolve(resolveDesktopPath(), '017_limit-only_검수결과.zip');
...
console.log(`017 LIMIT-ONLY RESULT: ${status.toUpperCase()}`);
