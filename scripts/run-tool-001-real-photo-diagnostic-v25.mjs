...
const outDir=path.join(desktop,`TOOLBOX_001_REAL_PHOTO_V25_${stamp}`);fs.mkdirSync(outDir,{recursive:true});
const lines=[];
...
function dumpUi(){const remote='/sdcard/tool001-v25-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
async function autoCommitManualSelection(caseNo,page,startChange){const deadline=Date.now()+90000;let sawSelection=false;let commitTaps=0;let lastXml='';while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__)));if(st.changes.length>startChange)return{committed:true,mode:'picker-auto-close-change',sawSelection,commitTaps};let open=false;try{lastXml=dumpUi();open=pickerDetectedFromXml(lastXml);}catch{}if(!open){await sleep(250);const st2=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__)));if(st2.changes.length>startChange)return{committed:true,mode:'picker-closed-change',sawSelection,commitTaps};await sleep(500);continue;}const sig=pickerSelectionSignals(lastXml);if(sig.detected){sawSelection=true;write(`case-${caseNo}-selection-observed.xml`,lastXml);const actions=photoPickerActionControls(lastXml);write(`case-${caseNo}-commit-controls.json`,actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds})));if(actions.length){for(const action of actions){const b=action.bounds;adb('shell','input','tap',String(b.x),String(b.y));commitTaps++;log(`[PHOTO_PICKER] case ${caseNo}: auto commit "${action.exact}" at ${b.x},${b.y}`);await pcScreen(`case-${caseNo}-after-auto-commit-${commitTaps}.png`);if(await pickerClosedStable(page,7000)){const st3=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__)));if(st3.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit',sawSelection:true,commitTaps,action:action.exact};await sleep(1500);const st4=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__)));if(st4.changes.length>startChange)return{committed:true,mode:'selected-plus-auto-commit-delayed-change',sawSelection:true,commitTaps,action:action.exact};return{committed:false,mode:'picker-closed-no-change',sawSelection:true,commitTaps,action:action.exact};}}}}
...
function browserInit(){
  const KEY='__TOOL001_REAL_PHOTO_V25__'; if(window[KEY])return;
...
async function tapUpload(page,context,caseNo,round=1){
  ...
  const btn=page.locator('[data-testid="converter-upload-button"], [data-testid="converter-add-button"]').first();
  await btn.waitFor({state:'visible',timeout:15000});
  await btn.evaluate(el=>el.scrollIntoView({block:'center',inline:'center',behavior:'instant'}));await sleep(300);
  const geom=await btn.evaluate(el=>{const r=el.getBoundingClientRect();const cx=r.left+r.width/2,cy=r.top+r.height/2;const hit=document.elementFromPoint(cx,cy);return{x:r.left,y:r.top,width:r.width,height:r.height,cx,cy,buttonText:(el.textContent||'').trim(),buttonTestId:el.getAttribute('data-testid')||'',hitTag:hit?.tagName||'',hitText:(hit?.textContent||'').trim().slice(0,120),hitTestId:hit?.getAttribute?.('data-testid')||'',sameNode:hit===el||el.contains(hit)};});
  write(`case-${caseNo}-round-${round}-upload-geometry.json`,geom);
  if(!geom.sameNode || !/^converter-(upload|add)-button$/.test(geom.buttonTestId))throw new Error(`UPLOAD_TARGET_WHITELIST_FAIL:${JSON.stringify(geom)}`);
  const strategies=[];
  async function attempt(method,fire){
    ...
    try{
      ...
      const xml=await waitStablePickerOpen(caseNo,`r${round}-${method}`,5500);strategies[strategies.length-1].stablePicker=!!xml;
    ...
    await ensureChromeInteractive(page,caseNo,`recover-${method}-r${round}`).catch(()=>{});await sleep(250);return null;
  }
  // Safe path 1: CDP event on the verified DOM button center. No device-screen coordinate mapping.
  let r=await attempt('cdp-whitelisted-button',async()=>{const cdp=await context.newCDPSession(page);try{await cdp.send('Input.dispatchMouseEvent',{type:'mousePressed',x:Math.round(geom.cx),y:Math.round(geom.cy),button:'left',clickCount:1});await cdp.send('Input.dispatchMouseEvent',{type:'mouseReleased',x:Math.round(geom.cx),y:Math.round(geom.cy),button:'left',clickCount:1});}finally{await cdp.detach().catch(()=>{});}});if(r)return r;
  // Safe path 2: exact UIAutomator clickable node whose label is the upload/add action.
  r=await attempt('uiautomator-exact-upload-control',async()=>{const xml=dumpUi();const cs=chromeUploadActionControls(xml).filter(c=>/(이미지 선택|이미지 추가|choose image|add image|画像を選択)/i.test(c.label));write(`case-${caseNo}-round-${round}-chrome-upload-controls.json`,cs);if(cs.length!==1)throw new Error(`UPLOAD_CONTROL_AMBIGUOUS:${cs.length}`);const b=cs[0].bounds;const ar=adb('shell','input','tap',String(b.x),String(b.y));if(ar.status!==0)throw new Error('ADB_EXACT_UPLOAD_TAP_FAILED');});if(r)return r;
  // Safe path 3: Playwright clicks only the verified testid button. No force and no generic role/coordinate fallback.
  r=await attempt('playwright-whitelisted-button',async()=>{await btn.click({timeout:3000,noWaitAfter:true});});if(r)return r;
...
async function waitForSelectionAndCommit(caseNo,label,page,startChange,context){
  ...
  async function pageState(tag){
    let st=null;try{st=await page.evaluate(()=>({href:location.href,visibility:document.visibilityState,focus:document.hasFocus(),activeTag:document.activeElement?.tagName||'',activeType:document.activeElement?.type||'',fileCount:Array.from(document.querySelectorAll('input[type=file]')).reduce((n,el)=>n+(el.files?.length||0),0)}));}catch(e){st={error:String(e?.message||e)}}
    write(`case-${caseNo}-${tag}-page-state-${Date.now()}.json`,st);return st;
  }
  async function urlGuard(tag){
    const st=await pageState(`urlguard-${tag}`);if(!st?.href)return st;
    let actual;try{actual=new URL(st.href);}catch{return st;}
    const ok=actual.origin===expected.origin&&actual.pathname===expected.pathname;
    if(!ok){
      write(`case-${caseNo}-${tag}-unexpected-navigation.json`,{expected:expectedUrl,actual:st.href,foreground:foregroundSnapshot()});
      log(`[URL_GUARD] CASE ${caseNo}: unexpected navigation ${st.href} -> restoring TOOL001`);
      await page.goto(expectedUrl,{waitUntil:'domcontentloaded',timeout:30000}).catch(()=>{});
      await page.locator(selector).first().waitFor({state:'attached',timeout:15000}).catch(()=>{});
    }
    return st;
  }
  async function waitChange(ms){
    const dl=Date.now()+ms;while(Date.now()<dl){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__)));if(st.changes.length>startChange)return st.changes.at(-1);await sleep(200);}return null;
  }
  for(let round=1;round<=3;round++){
    await urlGuard(`before-activation-r${round}`);
    let activation;
    ...
    session.push({round,activation:{method:activation.method,strategies:activation.strategies}});write(`case-${caseNo}-activation-session.json`,session);
    log(`[PICKER READY] CASE ${caseNo} ${label}: Photo Picker가 실제로 열린 것을 확인했습니다.`);
    log(`[ACTION REQUIRED] ${caseInstruction(label)} 완료 버튼은 누르지 마세요. V25가 자동 처리합니다.`);

    const selectionDeadline=Date.now()+30000;let sawSelection=false,commitTaps=0,lastXml='',selectionAt=0;
    while(Date.now()<selectionDeadline){
      const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__)));if(st.changes.length>startChange)return{committed:true,mode:'picker-auto-close-change',sawSelection,commitTaps,round,activation};
      const snap=pickerSignalSnapshot();lastXml=snap.xml;
      if(!snap.detected){
        const ch=await waitChange(1800);if(ch)return{committed:true,mode:'picker-closed-change',sawSelection,commitTaps,round,activation};
        await urlGuard(`picker-lost-r${round}`);
        if(!sawSelection){log(`[RECOVER] CASE ${caseNo}: 선택 전에 Photo Picker가 닫혔습니다. 자동 재오픈 (${round}/3)`);session.push({round,pickerLostBeforeSelection:true});write(`case-${caseNo}-activation-session.json`,session);break;}
        await pcScreen(`case-${caseNo}-picker-closed-no-change-r${round}.png`);write(`case-${caseNo}-picker-closed-no-change-r${round}.xml`,lastXml);
        await pageState(`picker-closed-no-change-r${round}`);
        return{committed:false,mode:'PICKER_CLOSED_NO_CHANGE',sawSelection:true,commitTaps,round,activation};
      }
      const sig=pickerSelectionSignals(lastXml);
      if(sig.selectedNodes.length){
        if(!sawSelection){sawSelection=true;selectionAt=Date.now();write(`case-${caseNo}-selection-observed-r${round}.xml`,lastXml);log(`[SELECTION] CASE ${caseNo}: selection observed; commit control rescan started`);}
        // Re-dump every cycle because Android 15 often exposes Done asynchronously after selection animation.
        const actions=photoPickerActionControls(lastXml);write(`case-${caseNo}-commit-controls-r${round}-${Date.now()}.json`,actions.map(a=>({label:a.label,exact:a.exact,bounds:a.bounds})));
        if(actions.length){
          const action=actions[0],b=action.bounds;
          adb('shell','input','tap',String(b.x),String(b.y));commitTaps++;
          log(`[PHOTO_PICKER] CASE ${caseNo}: auto commit "${action.exact}" at ${b.x},${b.y} (tap ${commitTaps})`);
          await pcScreen(`case-${caseNo}-after-auto-commit-r${round}-${commitTaps}.png`);
          const change=await waitChange(3500);if(change)return{committed:true,mode:'selected-plus-auto-commit-change',sawSelection:true,commitTaps,round,activation,action:action.exact};
          if(await pickerClosedStable(page,3500)){
            const delayed=await waitChange(5000);if(delayed)return{committed:true,mode:'selected-plus-auto-commit-delayed-change',sawSelection:true,commitTaps,round,activation,action:action.exact};
            await pcScreen(`case-${caseNo}-picker-closed-no-change-after-commit-r${round}.png`);await pageState(`closed-no-change-after-commit-r${round}`);await urlGuard(`after-commit-r${round}`);
            return{committed:false,mode:'PICKER_CLOSED_NO_CHANGE',sawSelection:true,commitTaps,round,activation,action:action.exact};
          }
        }
        // Do not wait 90-110 s. If selection is visible but no commit control materializes in 12 s, capture evidence and fail this case.
        if(Date.now()-selectionAt>12000){
          const fresh=pickerSignalSnapshot();write(`case-${caseNo}-commit-control-timeout-r${round}.xml`,fresh.xml);await pcScreen(`case-${caseNo}-commit-control-timeout-r${round}.png`);await pageState(`commit-control-timeout-r${round}`);
          return{committed:false,mode:'COMMIT_CONTROL_NOT_EXPOSED',sawSelection:true,commitTaps,round,activation};
        }
      }
      await sleep(350);
    }
    if(sawSelection)return{committed:false,mode:'COMMIT_CONTROL_NOT_EXPOSED',sawSelection,commitTaps,round,activation};
  }
  return{committed:false,mode:'picker-lost-before-selection-after-retries',sawSelection:false,commitTaps:0};
}

async function waitProduct(page,startCapture,before,timeoutMs=90000){const deadline=Date.now()+timeoutMs;let snap=before,cap=[];while(Date.now()<deadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__)));cap=st.capture.slice(startCapture);snap=await domSnapshot(page);const terminal=cap.some(x=>x.phase==='picker-file-pass'||x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail');const cardChanged=snap.cardCount!==before.cardCount;if(terminal&&(cardChanged||cap.some(x=>x.phase==='picker-file-fail'||x.phase==='capture-bytes-fail')))break;await sleep(350);}return{capture:cap,after:snap};}
async function collectFailureEvidence(caseNo,label,page,reason){let xml='';try{xml=dumpUi();}catch{}write(`case-${caseNo}-${label}-failure-picker.xml`,xml);await pcScreen(`case-${caseNo}-${label}-failure-device.png`);await page.screenshot({path:path.join(outDir,`case-${caseNo}-${label}-failure-page.png`),fullPage:true}).catch(()=>{});const snap=await domSnapshot(page).catch(()=>null);const obs=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__))).catch(()=>null);write(`case-${caseNo}-${label}-failure-evidence.json`,{reason,pickerOpen:pickerDetectedFromXml(xml),selectionSignals:pickerSelectionSignals(xml).selectedNodes.length,commitControls:photoPickerActionControls(xml).map(a=>({exact:a.exact,label:a.label,bounds:a.bounds})),foreground:foregroundSnapshot(),observer:obs,dom:snap});}
...
function classifyStage(r){
  if(r.activationError||/UPLOAD_ACTIVATION|WRONG_TARGET|URL_GUARD/.test(r.error||''))return 'UPLOAD_ACTIVATION';
...
async function main(){
  log('=== TOOL001 REAL USER PHOTO DIAGNOSTIC V25 FINAL-INTEGRATED ===');log('[OUTPUT]',outDir);
  ...
  step('실사진 비교검수 안내');log('');log('*** 검수기가 테스트 사진을 만들지 않습니다. ***');log('각 CASE에서 안내된 GOOD/BAD 실제 사진을 Galaxy Photo Picker에서 1장만 탭하세요.');log('완료 버튼은 누르지 않아도 됩니다. V25이 V19 검증 로직으로 자동 commit합니다.');log('8개 케이스로 반복성·출처·포맷·provider·result-return·제품검증·preview를 넓게 탐색합니다.');log('');
  const results=[];
  for(let i=0;i<labels.length;i++){
    ...
    await page.bringToFront();const before=await domSnapshot(page);const st0=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__)));let result={case:caseNo,label,pass:false,error:null,before,commit:null};
    try{
      const commit=await timed(`CASE ${caseNo} verified picker + user selection + auto commit`,()=>waitForSelectionAndCommit(caseNo,label,page,st0.changes.length,context),65000);result.commit=commit;result.activation=commit.activation||null;
      if(!commit.committed)throw new Error(`PHOTO_PICKER_COMMIT_FAILED:${commit.mode}`);
      const changeDeadline=Date.now()+8000;let change=null;while(Date.now()<changeDeadline){const st=await page.evaluate(()=>JSON.parse(JSON.stringify(window.__TOOL001_REAL_PHOTO_V25__)));if(st.changes.length>st0.changes.length){change=st.changes.at(-1);break;}await sleep(250);}if(!change)throw new Error('NO_FILE_CHANGE_AFTER_VERIFIED_COMMIT');
  ...
  const summary=['TOOL001_REAL_USER_PHOTO_DIAGNOSTIC_V25_BROAD_CAUSE_SWEEP',`URL=${url}`,`CASES=${results.length}`,`GOOD_BAD_FIRST_DIVERGENCE=${divergence}`,`TIMING_RACE_EVIDENCE=${timingRace}`,`REPEAT_EVIDENCE=${JSON.stringify(repeatRace)}`];
  ...
  write('summary.txt',summary.join('\n'));write('root-cause-summary.json',{goodBadFirstDivergence:divergence,timingRaceEvidence:timingRace,repeatEvidence:repeatRace,stageCounts:Object.fromEntries([...new Set(results.map(r=>r.rootCauseStage))].map(k=>[k,results.filter(r=>r.rootCauseStage===k).length]))});write('result.json',{version:'V25',url,generatedAt:new Date().toISOString(),uiLimits:{has20MB:ui20,has60MB:ui60},goodBadFirstDivergence:divergence,timingRaceEvidence:timingRace,repeatEvidence:repeatRace,results});
