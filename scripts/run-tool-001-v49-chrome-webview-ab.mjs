...
const outDir=path.join(desktop,`TOOLBOX_001_V49_CHROME_WEBVIEW_AB_REAL_DEVICE_${stamp}`);
...
function dumpUi(){const remote='/sdcard/tool001-v49-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
const MINIMAL_HTML=`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>TOOL001 V49 Chrome Byte Read</title></head><body style="font-family:sans-serif;padding:28px"><h2>TOOL001 Chrome Minimal File Read</h2><p>No TOOL001 / React / Next.js. One visible file input, one picker selection, one arrayBuffer read. Passive diagnostics only.</p><input id="file" type="file" accept="image/*" style="font-size:20px;padding:14px 4px"><pre id="out"></pre><script>
...
f.addEventListener('change',async()=>{
 ...
 if(!file){window.__diag={...window.__diag,stage:'no_file',pass:false,changeAt};paint();console.log('V49_DIAG no_file');return;}
 ...
 console.log('V49_DIAG read_start',JSON.stringify({name:file.name,size:file.size,type:file.type,lastModified:file.lastModified,changeAt,readStartAt}));
 try{
   ...
   console.log('V49_DIAG read_pass',JSON.stringify({bytes:ab.byteLength,readDurationMs:readEndAt-readStartAt}));
 }catch(e){
   const readEndAt=Date.now();
   ...
   console.error('V49_DIAG read_fail',JSON.stringify({errorName:e&&e.name||'',errorMessage:e&&e.message||String(e),readDurationMs:readEndAt-readStartAt}));
 }
});
paint();
</script></body></html>`;
        ...
        webView.webViewClient = object : WebViewClient() {
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                view?.evaluateJavascript(
                    "(function(){var f=document.getElementById('file');return f?('READY:'+window.innerWidth+'x'+window.innerHeight):'MISSING';})()"
                ) { value ->
                    Log.i(TAG, "WEBVIEW_INPUT_READY round=$round value=\${safe(value)}")
                }
            }
        }
...
<body style="font-family:sans-serif;margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;background:#fff">
<div style="text-align:center;padding:28px;pointer-events:none"><h2>TOOL001 WebView Minimal File Read</h2><p>Tap once to open the web file input. One picker selection, one arrayBuffer read.</p></div>
<input id="file" type="file" accept="image/*" aria-label="OPEN_WEB_FILE" style="position:fixed;inset:0;width:100vw;height:100vh;opacity:0.01;z-index:9999;margin:0;padding:0">
<pre id="out" style="position:fixed;left:8px;right:8px;bottom:8px;max-height:35vh;overflow:auto;pointer-events:none"></pre>
<script>
window.__diag={stage:'idle',pass:null,pageLoadedAt:Date.now(),ua:navigator.userAgent};
const f=document.getElementById('file'),o=document.getElementById('out');
const paint=()=>o.textContent=JSON.stringify(window.__diag,null,2);
...
function prepareWebViewApk(){
  ...
  webviewRuntimeDir=path.join(os.tmpdir(),`fixlgs-tool001-v49-webview-${process.pid}`);
...
function tapWebViewFullscreenInputOnce(){
  ...
  if(r.status!==0)throw new Error(`HARNESS_WEBVIEW_INPUT_TAP_FAILED:${r.stderr||r.stdout||''}`);
  return{method:'ADB_REAL_TOUCH_FULLSCREEN_FILE_INPUT',x,y,width,height};
}
...
async function runWebViewMinimal(){
  ...
  for(let r=1;r<=repeats;r++){
    ...
    try{
      ...
      const inputReady=waitLogLine(new RegExp(`WEBVIEW_INPUT_READY round=${r}\\b.*READY:`),5000);
      if(!inputReady)throw new Error('HARNESS_WEBVIEW_INPUT_NOT_READY');
      write(`case-${caseNo}-input-ready-line.txt`,inputReady);
      await screen(`case-${caseNo}-before-input-tap.png`);
      row.openControl=tapWebViewFullscreenInputOnce();
...
async function main(){
  log('=== TOOL001 V49 CHROME vs ANDROID SYSTEM WEBVIEW FILE-BRIDGE A/B ===');
  ...
  const summary=[
    'TOOL001 V49 CHROME vs ANDROID SYSTEM WEBVIEW FILE-BRIDGE A/B',
    ...
    'RULE_5=WebView phase uses a temporary diagnostic APK only to host Android System WebView; the harness performs one real center-screen touch on a fullscreen web file input and never reads the provider URI directly.',
  ...
  write('v49-chrome-webview-ab-summary.txt',summary);
  write('result.json',{version:'V49-CHROME-WEBVIEW-FILE-BRIDGE-AB',repeats,chrome,webview,interpretation,chromeSignals,webviewSignals,chromeRows,webviewRows});
  log(summary);
  await makeZip();
  cleanupWebViewApk();
}
