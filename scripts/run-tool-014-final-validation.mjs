...
const definitions = [
  ['validator', ['run', 'check:tool014-validator']],
  ['source', ['run', 'check:tool014-source']],
  ['ja-terms', ['run', 'check:ja-terms']],
  ['build', ['run', 'build']],
  ['common', ['run', 'test:toolbox:common-additive']],
  ['core', ['run', 'test:toolbox:014-core']],
  ['boundary', ['run', 'test:toolbox:014-boundary']],
  ['regression', ['run', 'test:toolbox:014-regression']],
  ['service-limit', ['exec', 'playwright', 'test', 'tests/tool-014-limit.spec.ts', '--workers=1', '--project=desktop-chromium']],
];
...
const pkg = createValidationResultPackage({
  ...
  steps,
  extraFiles: [{
    source: 'test-results/tool-014-limit-report.json',
    destination: 'limit/tool-014-limit-report.json',
  }],
});
cleanupProjectValidationArtifacts();
console.log(`014 FINAL RESULT: ${status.toUpperCase()}`);
