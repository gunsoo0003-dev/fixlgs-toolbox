...
const outDir=path.join(desktop,`TOOLBOX_001_V51_CHROME_WEBVIEW_AB_REAL_DEVICE_${stamp}`);
...
function dumpUi(){const remote='/sdcard/tool001-v51-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
const MINIMAL_HTML=`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>TOOL001 V51 Chrome Byte Read</title></head><body style="font-family:sans-serif;padding:28px"><h2>TOOL001 Chrome Minimal File Read</h2><p>No TOOL001 / React / Next.js. One visible file input, one picker selection, one arrayBuffer read. Passive diagnostics only.</p><input id="file" type="file" accept="image/*" style="font-size:20px;padding:14px 4px"><pre id="out"></pre><script>
...
f.addEventListener('change',async()=>{
 ...
 if(!file){window.__diag={...window.__diag,stage:'no_file',pass:false,changeAt};paint();console.log('V51_DIAG no_file');return;}
 ...
 console.log('V51_DIAG read_start',JSON.stringify({name:file.name,size:file.size,type:file.type,lastModified:file.lastModified,changeAt,readStartAt}));
 try{
   ...
   console.log('V51_DIAG read_pass',JSON.stringify({bytes:ab.byteLength,readDurationMs:readEndAt-readStartAt}));
 }catch(e){
   const readEndAt=Date.now();
   ...
   console.error('V51_DIAG read_fail',JSON.stringify({errorName:e&&e.name||'',errorMessage:e&&e.message||String(e),readDurationMs:readEndAt-readStartAt}));
 }
});
paint();
</script></body></html>`;
...
function prepareWebViewApk(){
  ...
  webviewRuntimeDir=path.join(os.tmpdir(),`fixlgs-tool001-v51-webview-${process.pid}`);
...
async function main(){
  log('=== TOOL001 V51 CHROME vs ANDROID SYSTEM WEBVIEW FILE-BRIDGE A/B ===');
  ...
  const summary=[
    'TOOL001 V51 CHROME vs ANDROID SYSTEM WEBVIEW FILE-BRIDGE A/B',
  ...
  write('v51-chrome-webview-ab-summary.txt',summary);
  write('result.json',{version:'V51-CHROME-WEBVIEW-FILE-BRIDGE-AB',repeats,chrome,webview,interpretation,chromeSignals,webviewSignals,chromeRows,webviewRows});
  log(summary);
  await makeZip();
  cleanupWebViewApk();
}
