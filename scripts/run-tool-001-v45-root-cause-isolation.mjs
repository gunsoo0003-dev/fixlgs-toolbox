...
const outDir=path.join(desktop,`TOOLBOX_001_V45_ROOT_CAUSE_3WAY_REAL_DEVICE_${stamp}`);
...
function dumpUi(){const remote='/sdcard/tool001-v45-window.xml';adb('shell','uiautomator','dump','--compressed',remote);return adbText('shell','cat',remote);}
...
function filteredEvidence(text){return String(text||'').split(/\r?\n/).filter(x=>/(TOOL001_NATIVE_DIAG|chromium|chrome|photopicker|MediaProvider|fuse|ContentResolver|openFileDescriptor|ContentProviderHelper|UriGrantsManagerService|permission denied|EACCES|ENOENT|NotReadableError|ERR_UPLOAD_FILE_CHANGED|file could not be read|FileReader|ACTION_GET_CONTENT|ACTION_PICK_IMAGES)/i.test(x)).slice(-1800).join('\n');}
...
function evidenceFromLogcat(text){
  ...
  return{
    ...
    nativeDiag:lines.filter(x=>/TOOL001_NATIVE_DIAG/i.test(x)).slice(-80)
  };
}
...
const MINIMAL_HTML=`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>TOOL001 V45 Chrome Byte Read</title></head><body style="font-family:sans-serif;padding:28px"><h2>TOOL001 Chrome Minimal File Read</h2><p>No TOOL001 / React / Next.js. One visible file input, one picker selection, one arrayBuffer read.</p><input id="file" type="file" accept="image/*" style="font-size:20px;padding:14px 4px"><pre id="out"></pre><script>
...
async function runChromeMinimal(){
  log('=== PHASE A: CHROME MINIMAL HTML / BROWSER FILE BRIDGE ===');
...
function templateHash(){
  const a=fs.readFileSync(path.join(here,'tool-001-v45-native-template','main.dart'));
  const b=fs.readFileSync(path.join(here,'tool-001-v45-native-template','MainActivity.kt'));
  return crypto.createHash('sha256').update(a).update(b).digest('hex');
}

function prepareNativeApk(){
  const cacheRoot=path.join(os.tmpdir(),'tool001-v45-native-cache');
  ...
  if(!args['rebuild-native']&&fs.existsSync(apkCache)&&fs.existsSync(hashFile)&&fs.readFileSync(hashFile,'utf8').trim()===wantHash){
    log('[NATIVE] using cached V45 diagnostic APK');
    return apkCache;
  }
  log('[NATIVE] building V45 diagnostic APK (first run only)');
  ...
  const work=path.join(os.tmpdir(),'tool001-v45-native-build');
  ...
  fs.copyFileSync(path.join(here,'tool-001-v45-native-template','main.dart'),path.join(work,'lib','main.dart'));
  ...
  fs.copyFileSync(path.join(here,'tool-001-v45-native-template','MainActivity.kt'),kt);
...
function installNativeApk(apk){
  log('[NATIVE] installing V45 diagnostic APK');
...
function parseNativeResult(logText,round,mode){
  const lines=String(logText||'').split(/\r?\n/).filter(x=>x.includes('TOOL001_NATIVE_DIAG'));
  const target=lines.filter(x=>new RegExp(`round=${round}(?:\\s|$)`).test(x)&&x.includes(`mode=${mode}`));
  const h=[...target].reverse().find(x=>x.includes('NATIVE_HARNESS_FAIL'));
...
async function waitNativeAppStarted(round,mode,timeoutMs=7000){
  const deadline=Date.now()+timeoutMs;
  while(Date.now()<deadline){
    const t=fullLogcat();
    if(t.split(/\r?\n/).some(x=>x.includes('TOOL001_NATIVE_DIAG')&&x.includes(`NATIVE_APP_START round=${round} mode=${mode}`)))return true;
    await sleep(150);
  }
  return false;
}

async function runNativeMode(mode,label){
  log(`=== ${label} ===`);
  const pkg='com.fixlgs.tool001_native_provider_diag';
  const act='com.fixlgs.tool001_native_provider_diag.MainActivity';
  const rows=[];
  for(let r=1;r<=repeats;r++){
    const caseNo=`NATIVE-${mode}-${r}`;
    log(`===== Android native ${mode} ${r}/${repeats} =====`);
    adb('logcat','-c');
    adb('shell','am','force-stop',pkg);
    await sleep(180);
    // Android 15 `am start` on this device rejects the long-form activity flags.
    // Use the standard intent flag mask instead: NEW_TASK | CLEAR_TASK.
    const start=adb('shell','am','start','-W','-S','-f','0x10008000','-n',`${pkg}/${act}`,'--ei','round',String(r),'--es','mode',mode);
    const row={phase:`ANDROID_NATIVE_${mode}`,mode,round:r,pass:false,harnessFail:false,error:'',bytes:0};
    if(start.status!==0){
      row.harnessFail=true;
      row.error=`HARNESS_NATIVE_START_FAILED:${start.stderr||start.stdout||''}`;
    }else{
      const started=await waitNativeAppStarted(r,mode,7000);
      if(!started){
        row.harnessFail=true;
        row.error='HARNESS_NATIVE_APP_START_NOT_OBSERVED';
      }else{
        try{
          row.selected=await selectPhoto01(caseNo);
          ...
          while(Date.now()<deadline){
            const t=fullLogcat();
            const p=parseNativeResult(t,r,mode);
            if(p.done){Object.assign(row,p);break;}
            await sleep(180);
          }
          if(!row.pass&&!row.harnessFail&&!row.error){
            row.harnessFail=true;
            row.error='HARNESS_NATIVE_RESULT_TIMEOUT';
          }
        }catch(e){
          row.error=String(e?.message||e);
          row.harnessFail=/HARNESS_/i.test(row.error);
          if(!row.harnessFail)row.harnessFail=true;
        }
      }
    }
    const lc=fullLogcat();
    ...
    write(`case-${caseNo}-logcat-filtered.txt`,filteredEvidence(lc));
    await screen(`case-${caseNo}-device.png`);
    log(`[${row.harnessFail?'HARNESS':row.pass?'PASS':'FAIL'}] Android native ${mode} R${r} bytes=${row.bytes||0} uriWmLock=${row.systemEvidence.uriPermissionWmLockErrors} lowerFsOpens=${row.systemEvidence.mediaProviderLowerFsOpens} noPersistWrite=${row.systemEvidence.chromeNoPersistedWritePermission} error=${row.error||'-'}`);
    rows.push(row);
    adb('shell','am','force-stop',pkg);
    await sleep(250);
  }
...
function classify(chromeRows,getRows,pickRows){
  const c=summarize(chromeRows),g=summarize(getRows),p=summarize(pickRows);
  if(c.valid!==repeats||g.valid!==repeats||p.valid!==repeats)return 'INCONCLUSIVE_HARNESS_FAILURE';
  if(c.fail>0&&g.fail===0&&p.fail===0)return 'CHROME_FILE_BRIDGE_STRONGLY_SUSPECTED';
  if(c.fail>0&&g.fail>0&&p.fail===0)return 'GET_CONTENT_URI_GRANT_OR_COMPAT_PATH_STRONGLY_SUSPECTED';
  if(c.fail>0&&g.fail>0&&p.fail>0)return 'ANDROID_PROVIDER_OR_LOWER_NATIVE_BOUNDARY_STRONGLY_SUSPECTED';
  if(c.fail===0&&g.fail===0&&p.fail===0)return 'NO_FAILURE_REPRODUCED_THIS_RUN';
  if(c.fail===0&&(g.fail>0||p.fail>0))return 'NATIVE_ONLY_FAILURE_INCONCLUSIVE';
  if(c.fail>0&&g.fail===0&&p.fail>0)return 'MIXED_NATIVE_PICK_IMAGES_FAILURE_INCONCLUSIVE';
  return 'INCONCLUSIVE';
}
...
function cleanupNativeApk(){
  const pkg='com.fixlgs.tool001_native_provider_diag';
  const r=adb('uninstall',pkg);
  const text=`${r.stdout||''}${r.stderr||''}`.trim();
  if(r.status===0)console.log(`[CLEANUP] diagnostic APK removed: ${pkg}`);
  else if(/Unknown package|not installed/i.test(text))console.log(`[CLEANUP] diagnostic APK already absent: ${pkg}`);
  else console.log(`[CLEANUP_WARN] diagnostic APK uninstall failed: ${text||`status=${r.status}`}`);
}

async function main(){
  log('=== TOOL001 V45 ROOT CAUSE 3-WAY ISOLATION ===');
  log(`REPEATS_PER_PATH=${repeats}`);
  ...
  const nativeGetRows=await runNativeMode('GET_CONTENT','PHASE B: ANDROID NATIVE GET_CONTENT + CONTENTRESOLVER DIRECT READ');
  const nativePickRows=await runNativeMode('PICK_IMAGES','PHASE C: ANDROID NATIVE PICK_IMAGES + CONTENTRESOLVER DIRECT READ');

  const chrome=summarize(chromeRows),nativeGet=summarize(nativeGetRows),nativePick=summarize(nativePickRows);
  const interpretation=classify(chromeRows,nativeGetRows,nativePickRows);
  const summary=[
    'TOOL001 V45 ROOT CAUSE 3-WAY ISOLATION',
    `REPEATS_PER_PATH=${repeats}`,
    '',
    `A_CHROME_MINIMAL VALID=${chrome.valid}/${chrome.total} PASS=${chrome.pass}/${chrome.valid} FAIL=${chrome.fail} HARNESS_FAIL=${chrome.harnessFail}`,
    `B_NATIVE_GET_CONTENT VALID=${nativeGet.valid}/${nativeGet.total} PASS=${nativeGet.pass}/${nativeGet.valid} FAIL=${nativeGet.fail} HARNESS_FAIL=${nativeGet.harnessFail}`,
    `C_NATIVE_PICK_IMAGES VALID=${nativePick.valid}/${nativePick.total} PASS=${nativePick.pass}/${nativePick.valid} FAIL=${nativePick.fail} HARNESS_FAIL=${nativePick.harnessFail}`,
    '',
    `INTERPRETATION=${interpretation}`,
    '',
    'RULE_1=A fails, B+C all-pass => Chrome browser File/URI bridge strongly suspected.',
    'RULE_2=A+B fail, C all-pass => ACTION_GET_CONTENT / Photo Picker compatibility URI-grant path strongly suspected.',
    'RULE_3=A+B+C fail => Android Provider / URI / lower native file-open boundary strongly suspected.',
    'RULE_4=All three all-pass => failure not reproduced; repeat same harness only.',
    'RULE_5=ANY HARNESS_FAIL => automatic root-cause classification stays INCONCLUSIVE.',
    'RULE_6=One round = one picker open, PHOTO01 one selection, one byte-read attempt. No retry/recovery/fallback.',
    'RULE_7=Samsung Internet is not used.'
  ].join('\n');
  write('v45-root-cause-summary.txt',summary);
  write('result.json',{version:'V45-ROOT-CAUSE-3WAY',repeats,chrome,nativeGet,nativePick,interpretation,chromeRows,nativeGetRows,nativePickRows});
  log(summary);
  // User-visible result files and ZIP are completed before removing the temporary diagnostic app.
  await makeZip();
  cleanupNativeApk();
}
...
main().catch(async e=>{
  ...
  // Preserve failure evidence first, then remove the temporary diagnostic app.
  await makeZip();
  cleanupNativeApk();
  process.exitCode=1;
});
